! function() {
    const ca = a => {
        let b = new ArrayBuffer(65536),
            c = new Uint8Array(b),
            f = a({
                Uint8Array
            }, null, b),
            e = 0,
            h = 0;
        f.reset();
        return n => {
            n = new Uint8Array(n);
            let r = 0,
                z = n.length;
            for (; 0 < z;) {
                var x = c.length - h;
                x = x < z ? x : z;
                c.set(n.subarray(r, r + x), h);
                h += x;
                r += x;
                z -= x;
                e += f.update(e, h - e);
                e === h && (h = e = 0)
            }
            f.digest(e, h - e, 0);
            n = c.slice(0, 32).buffer;
            h = e = 0;
            f.reset();
            return n
        }
    };
    let da = ca(new Function("s,f,b", "'use asm';var A=0,B=0,C=0,D=0,E=0,F=0,G=0,H=0,I=0,J=0;var K=new s.Uint8Array(b);function z(j){j=j|0;var w0=0,w1=0,w2=0,w3=0,w4=0,w5=0,w6=0,w7=0,w8=0,w9=0,w10=0,w11=0,w12=0,w13=0,w14=0,w15=0;var a=0,b=0,c=0,d=0,e=0,f=0,g=0,h=0;w0=K[j|0]<<24|K[j|1]<<16|K[j|2]<<8|K[j|3];w1=K[j|4]<<24|K[j|5]<<16|K[j|6]<<8|K[j|7];w2=K[j|8]<<24|K[j|9]<<16|K[j|10]<<8|K[j|11];w3=K[j|12]<<24|K[j|13]<<16|K[j|14]<<8|K[j|15];w4=K[j|16]<<24|K[j|17]<<16|K[j|18]<<8|K[j|19];w5=K[j|20]<<24|K[j|21]<<16|K[j|22]<<8|K[j|23];w6=K[j|24]<<24|K[j|25]<<16|K[j|26]<<8|K[j|27];w7=K[j|28]<<24|K[j|29]<<16|K[j|30]<<8|K[j|31];w8=K[j|32]<<24|K[j|33]<<16|K[j|34]<<8|K[j|35];w9=K[j|36]<<24|K[j|37]<<16|K[j|38]<<8|K[j|39];w10=K[j|40]<<24|K[j|41]<<16|K[j|42]<<8|K[j|43];w11=K[j|44]<<24|K[j|45]<<16|K[j|46]<<8|K[j|47];w12=K[j|48]<<24|K[j|49]<<16|K[j|50]<<8|K[j|51];w13=K[j|52]<<24|K[j|53]<<16|K[j|54]<<8|K[j|55];w14=K[j|56]<<24|K[j|57]<<16|K[j|58]<<8|K[j|59];w15=K[j|60]<<24|K[j|61]<<16|K[j|62]<<8|K[j|63];a=A;b=B;c=C;d=D;e=E;f=F;g=G;h=H;h=(w0+h+(e>>>6^e>>>11^e>>>25^e<<26^e<<21^e<<7)+(g^e&(f^g))+0x428a2f98)|0;d=(d+h)|0;h=(h+((a&b)^(c&(a^b)))+(a>>>2^a>>>13^a>>>22^a<<30^a<<19^a<<10))|0;g=(w1+g+(d>>>6^d>>>11^d>>>25^d<<26^d<<21^d<<7)+(f^d&(e^f))+0x71374491)|0;c=(c+g)|0;g=(g+((h&a)^(b&(h^a)))+(h>>>2^h>>>13^h>>>22^h<<30^h<<19^h<<10))|0;f=(w2+f+(c>>>6^c>>>11^c>>>25^c<<26^c<<21^c<<7)+(e^c&(d^e))+0xb5c0fbcf)|0;b=(b+f)|0;f=(f+((g&h)^(a&(g^h)))+(g>>>2^g>>>13^g>>>22^g<<30^g<<19^g<<10))|0;e=(w3+e+(b>>>6^b>>>11^b>>>25^b<<26^b<<21^b<<7)+(d^b&(c^d))+0xe9b5dba5)|0;a=(a+e)|0;e=(e+((f&g)^(h&(f^g)))+(f>>>2^f>>>13^f>>>22^f<<30^f<<19^f<<10))|0;d=(w4+d+(a>>>6^a>>>11^a>>>25^a<<26^a<<21^a<<7)+(c^a&(b^c))+0x3956c25b)|0;h=(h+d)|0;d=(d+((e&f)^(g&(e^f)))+(e>>>2^e>>>13^e>>>22^e<<30^e<<19^e<<10))|0;c=(w5+c+(h>>>6^h>>>11^h>>>25^h<<26^h<<21^h<<7)+(b^h&(a^b))+0x59f111f1)|0;g=(g+c)|0;c=(c+((d&e)^(f&(d^e)))+(d>>>2^d>>>13^d>>>22^d<<30^d<<19^d<<10))|0;b=(w6+b+(g>>>6^g>>>11^g>>>25^g<<26^g<<21^g<<7)+(a^g&(h^a))+0x923f82a4)|0;f=(f+b)|0;b=(b+((c&d)^(e&(c^d)))+(c>>>2^c>>>13^c>>>22^c<<30^c<<19^c<<10))|0;a=(w7+a+(f>>>6^f>>>11^f>>>25^f<<26^f<<21^f<<7)+(h^f&(g^h))+0xab1c5ed5)|0;e=(e+a)|0;a=(a+((b&c)^(d&(b^c)))+(b>>>2^b>>>13^b>>>22^b<<30^b<<19^b<<10))|0;h=(w8+h+(e>>>6^e>>>11^e>>>25^e<<26^e<<21^e<<7)+(g^e&(f^g))+0xd807aa98)|0;d=(d+h)|0;h=(h+((a&b)^(c&(a^b)))+(a>>>2^a>>>13^a>>>22^a<<30^a<<19^a<<10))|0;g=(w9+g+(d>>>6^d>>>11^d>>>25^d<<26^d<<21^d<<7)+(f^d&(e^f))+0x12835b01)|0;c=(c+g)|0;g=(g+((h&a)^(b&(h^a)))+(h>>>2^h>>>13^h>>>22^h<<30^h<<19^h<<10))|0;f=(w10+f+(c>>>6^c>>>11^c>>>25^c<<26^c<<21^c<<7)+(e^c&(d^e))+0x243185be)|0;b=(b+f)|0;f=(f+((g&h)^(a&(g^h)))+(g>>>2^g>>>13^g>>>22^g<<30^g<<19^g<<10))|0;e=(w11+e+(b>>>6^b>>>11^b>>>25^b<<26^b<<21^b<<7)+(d^b&(c^d))+0x550c7dc3)|0;a=(a+e)|0;e=(e+((f&g)^(h&(f^g)))+(f>>>2^f>>>13^f>>>22^f<<30^f<<19^f<<10))|0;d=(w12+d+(a>>>6^a>>>11^a>>>25^a<<26^a<<21^a<<7)+(c^a&(b^c))+0x72be5d74)|0;h=(h+d)|0;d=(d+((e&f)^(g&(e^f)))+(e>>>2^e>>>13^e>>>22^e<<30^e<<19^e<<10))|0;c=(w13+c+(h>>>6^h>>>11^h>>>25^h<<26^h<<21^h<<7)+(b^h&(a^b))+0x80deb1fe)|0;g=(g+c)|0;c=(c+((d&e)^(f&(d^e)))+(d>>>2^d>>>13^d>>>22^d<<30^d<<19^d<<10))|0;b=(w14+b+(g>>>6^g>>>11^g>>>25^g<<26^g<<21^g<<7)+(a^g&(h^a))+0x9bdc06a7)|0;f=(f+b)|0;b=(b+((c&d)^(e&(c^d)))+(c>>>2^c>>>13^c>>>22^c<<30^c<<19^c<<10))|0;a=(w15+a+(f>>>6^f>>>11^f>>>25^f<<26^f<<21^f<<7)+(h^f&(g^h))+0xc19bf174)|0;e=(e+a)|0;a=(a+((b&c)^(d&(b^c)))+(b>>>2^b>>>13^b>>>22^b<<30^b<<19^b<<10))|0;w0=((w1>>>7^w1>>>18^w1>>>3^w1<<25^w1<<14)+(w14>>>17^w14>>>19^w14>>>10^w14<<15^w14<<13)+w0+w9)|0;h=(w0+h+(e>>>6^e>>>11^e>>>25^e<<26^e<<21^e<<7)+(g^e&(f^g))+0xe49b69c1)|0;d=(d+h)|0;h=(h+((a&b)^(c&(a^b)))+(a>>>2^a>>>13^a>>>22^a<<30^a<<19^a<<10))|0;w1=((w2>>>7^w2>>>18^w2>>>3^w2<<25^w2<<14)+(w15>>>17^w15>>>19^w15>>>10^w15<<15^w15<<13)+w1+w10)|0;g=(w1+g+(d>>>6^d>>>11^d>>>25^d<<26^d<<21^d<<7)+(f^d&(e^f))+0xefbe4786)|0;c=(c+g)|0;g=(g+((h&a)^(b&(h^a)))+(h>>>2^h>>>13^h>>>22^h<<30^h<<19^h<<10))|0;w2=((w3>>>7^w3>>>18^w3>>>3^w3<<25^w3<<14)+(w0>>>17^w0>>>19^w0>>>10^w0<<15^w0<<13)+w2+w11)|0;f=(w2+f+(c>>>6^c>>>11^c>>>25^c<<26^c<<21^c<<7)+(e^c&(d^e))+0x0fc19dc6)|0;b=(b+f)|0;f=(f+((g&h)^(a&(g^h)))+(g>>>2^g>>>13^g>>>22^g<<30^g<<19^g<<10))|0;w3=((w4>>>7^w4>>>18^w4>>>3^w4<<25^w4<<14)+(w1>>>17^w1>>>19^w1>>>10^w1<<15^w1<<13)+w3+w12)|0;e=(w3+e+(b>>>6^b>>>11^b>>>25^b<<26^b<<21^b<<7)+(d^b&(c^d))+0x240ca1cc)|0;a=(a+e)|0;e=(e+((f&g)^(h&(f^g)))+(f>>>2^f>>>13^f>>>22^f<<30^f<<19^f<<10))|0;w4=((w5>>>7^w5>>>18^w5>>>3^w5<<25^w5<<14)+(w2>>>17^w2>>>19^w2>>>10^w2<<15^w2<<13)+w4+w13)|0;d=(w4+d+(a>>>6^a>>>11^a>>>25^a<<26^a<<21^a<<7)+(c^a&(b^c))+0x2de92c6f)|0;h=(h+d)|0;d=(d+((e&f)^(g&(e^f)))+(e>>>2^e>>>13^e>>>22^e<<30^e<<19^e<<10))|0;w5=((w6>>>7^w6>>>18^w6>>>3^w6<<25^w6<<14)+(w3>>>17^w3>>>19^w3>>>10^w3<<15^w3<<13)+w5+w14)|0;c=(w5+c+(h>>>6^h>>>11^h>>>25^h<<26^h<<21^h<<7)+(b^h&(a^b))+0x4a7484aa)|0;g=(g+c)|0;c=(c+((d&e)^(f&(d^e)))+(d>>>2^d>>>13^d>>>22^d<<30^d<<19^d<<10))|0;w6=((w7>>>7^w7>>>18^w7>>>3^w7<<25^w7<<14)+(w4>>>17^w4>>>19^w4>>>10^w4<<15^w4<<13)+w6+w15)|0;b=(w6+b+(g>>>6^g>>>11^g>>>25^g<<26^g<<21^g<<7)+(a^g&(h^a))+0x5cb0a9dc)|0;f=(f+b)|0;b=(b+((c&d)^(e&(c^d)))+(c>>>2^c>>>13^c>>>22^c<<30^c<<19^c<<10))|0;w7=((w8>>>7^w8>>>18^w8>>>3^w8<<25^w8<<14)+(w5>>>17^w5>>>19^w5>>>10^w5<<15^w5<<13)+w7+w0)|0;a=(w7+a+(f>>>6^f>>>11^f>>>25^f<<26^f<<21^f<<7)+(h^f&(g^h))+0x76f988da)|0;e=(e+a)|0;a=(a+((b&c)^(d&(b^c)))+(b>>>2^b>>>13^b>>>22^b<<30^b<<19^b<<10))|0;w8=((w9>>>7^w9>>>18^w9>>>3^w9<<25^w9<<14)+(w6>>>17^w6>>>19^w6>>>10^w6<<15^w6<<13)+w8+w1)|0;h=(w8+h+(e>>>6^e>>>11^e>>>25^e<<26^e<<21^e<<7)+(g^e&(f^g))+0x983e5152)|0;d=(d+h)|0;h=(h+((a&b)^(c&(a^b)))+(a>>>2^a>>>13^a>>>22^a<<30^a<<19^a<<10))|0;w9=((w10>>>7^w10>>>18^w10>>>3^w10<<25^w10<<14)+(w7>>>17^w7>>>19^w7>>>10^w7<<15^w7<<13)+w9+w2)|0;g=(w9+g+(d>>>6^d>>>11^d>>>25^d<<26^d<<21^d<<7)+(f^d&(e^f))+0xa831c66d)|0;c=(c+g)|0;g=(g+((h&a)^(b&(h^a)))+(h>>>2^h>>>13^h>>>22^h<<30^h<<19^h<<10))|0;w10=((w11>>>7^w11>>>18^w11>>>3^w11<<25^w11<<14)+(w8>>>17^w8>>>19^w8>>>10^w8<<15^w8<<13)+w10+w3)|0;f=(w10+f+(c>>>6^c>>>11^c>>>25^c<<26^c<<21^c<<7)+(e^c&(d^e))+0xb00327c8)|0;b=(b+f)|0;f=(f+((g&h)^(a&(g^h)))+(g>>>2^g>>>13^g>>>22^g<<30^g<<19^g<<10))|0;w11=((w12>>>7^w12>>>18^w12>>>3^w12<<25^w12<<14)+(w9>>>17^w9>>>19^w9>>>10^w9<<15^w9<<13)+w11+w4)|0;e=(w11+e+(b>>>6^b>>>11^b>>>25^b<<26^b<<21^b<<7)+(d^b&(c^d))+0xbf597fc7)|0;a=(a+e)|0;e=(e+((f&g)^(h&(f^g)))+(f>>>2^f>>>13^f>>>22^f<<30^f<<19^f<<10))|0;w12=((w13>>>7^w13>>>18^w13>>>3^w13<<25^w13<<14)+(w10>>>17^w10>>>19^w10>>>10^w10<<15^w10<<13)+w12+w5)|0;d=(w12+d+(a>>>6^a>>>11^a>>>25^a<<26^a<<21^a<<7)+(c^a&(b^c))+0xc6e00bf3)|0;h=(h+d)|0;d=(d+((e&f)^(g&(e^f)))+(e>>>2^e>>>13^e>>>22^e<<30^e<<19^e<<10))|0;w13=((w14>>>7^w14>>>18^w14>>>3^w14<<25^w14<<14)+(w11>>>17^w11>>>19^w11>>>10^w11<<15^w11<<13)+w13+w6)|0;c=(w13+c+(h>>>6^h>>>11^h>>>25^h<<26^h<<21^h<<7)+(b^h&(a^b))+0xd5a79147)|0;g=(g+c)|0;c=(c+((d&e)^(f&(d^e)))+(d>>>2^d>>>13^d>>>22^d<<30^d<<19^d<<10))|0;w14=((w15>>>7^w15>>>18^w15>>>3^w15<<25^w15<<14)+(w12>>>17^w12>>>19^w12>>>10^w12<<15^w12<<13)+w14+w7)|0;b=(w14+b+(g>>>6^g>>>11^g>>>25^g<<26^g<<21^g<<7)+(a^g&(h^a))+0x06ca6351)|0;f=(f+b)|0;b=(b+((c&d)^(e&(c^d)))+(c>>>2^c>>>13^c>>>22^c<<30^c<<19^c<<10))|0;w15=((w0>>>7^w0>>>18^w0>>>3^w0<<25^w0<<14)+(w13>>>17^w13>>>19^w13>>>10^w13<<15^w13<<13)+w15+w8)|0;a=(w15+a+(f>>>6^f>>>11^f>>>25^f<<26^f<<21^f<<7)+(h^f&(g^h))+0x14292967)|0;e=(e+a)|0;a=(a+((b&c)^(d&(b^c)))+(b>>>2^b>>>13^b>>>22^b<<30^b<<19^b<<10))|0;w0=((w1>>>7^w1>>>18^w1>>>3^w1<<25^w1<<14)+(w14>>>17^w14>>>19^w14>>>10^w14<<15^w14<<13)+w0+w9)|0;h=(w0+h+(e>>>6^e>>>11^e>>>25^e<<26^e<<21^e<<7)+(g^e&(f^g))+0x27b70a85)|0;d=(d+h)|0;h=(h+((a&b)^(c&(a^b)))+(a>>>2^a>>>13^a>>>22^a<<30^a<<19^a<<10))|0;w1=((w2>>>7^w2>>>18^w2>>>3^w2<<25^w2<<14)+(w15>>>17^w15>>>19^w15>>>10^w15<<15^w15<<13)+w1+w10)|0;g=(w1+g+(d>>>6^d>>>11^d>>>25^d<<26^d<<21^d<<7)+(f^d&(e^f))+0x2e1b2138)|0;c=(c+g)|0;g=(g+((h&a)^(b&(h^a)))+(h>>>2^h>>>13^h>>>22^h<<30^h<<19^h<<10))|0;w2=((w3>>>7^w3>>>18^w3>>>3^w3<<25^w3<<14)+(w0>>>17^w0>>>19^w0>>>10^w0<<15^w0<<13)+w2+w11)|0;f=(w2+f+(c>>>6^c>>>11^c>>>25^c<<26^c<<21^c<<7)+(e^c&(d^e))+0x4d2c6dfc)|0;b=(b+f)|0;f=(f+((g&h)^(a&(g^h)))+(g>>>2^g>>>13^g>>>22^g<<30^g<<19^g<<10))|0;w3=((w4>>>7^w4>>>18^w4>>>3^w4<<25^w4<<14)+(w1>>>17^w1>>>19^w1>>>10^w1<<15^w1<<13)+w3+w12)|0;e=(w3+e+(b>>>6^b>>>11^b>>>25^b<<26^b<<21^b<<7)+(d^b&(c^d))+0x53380d13)|0;a=(a+e)|0;e=(e+((f&g)^(h&(f^g)))+(f>>>2^f>>>13^f>>>22^f<<30^f<<19^f<<10))|0;w4=((w5>>>7^w5>>>18^w5>>>3^w5<<25^w5<<14)+(w2>>>17^w2>>>19^w2>>>10^w2<<15^w2<<13)+w4+w13)|0;d=(w4+d+(a>>>6^a>>>11^a>>>25^a<<26^a<<21^a<<7)+(c^a&(b^c))+0x650a7354)|0;h=(h+d)|0;d=(d+((e&f)^(g&(e^f)))+(e>>>2^e>>>13^e>>>22^e<<30^e<<19^e<<10))|0;w5=((w6>>>7^w6>>>18^w6>>>3^w6<<25^w6<<14)+(w3>>>17^w3>>>19^w3>>>10^w3<<15^w3<<13)+w5+w14)|0;c=(w5+c+(h>>>6^h>>>11^h>>>25^h<<26^h<<21^h<<7)+(b^h&(a^b))+0x766a0abb)|0;g=(g+c)|0;c=(c+((d&e)^(f&(d^e)))+(d>>>2^d>>>13^d>>>22^d<<30^d<<19^d<<10))|0;w6=((w7>>>7^w7>>>18^w7>>>3^w7<<25^w7<<14)+(w4>>>17^w4>>>19^w4>>>10^w4<<15^w4<<13)+w6+w15)|0;b=(w6+b+(g>>>6^g>>>11^g>>>25^g<<26^g<<21^g<<7)+(a^g&(h^a))+0x81c2c92e)|0;f=(f+b)|0;b=(b+((c&d)^(e&(c^d)))+(c>>>2^c>>>13^c>>>22^c<<30^c<<19^c<<10))|0;w7=((w8>>>7^w8>>>18^w8>>>3^w8<<25^w8<<14)+(w5>>>17^w5>>>19^w5>>>10^w5<<15^w5<<13)+w7+w0)|0;a=(w7+a+(f>>>6^f>>>11^f>>>25^f<<26^f<<21^f<<7)+(h^f&(g^h))+0x92722c85)|0;e=(e+a)|0;a=(a+((b&c)^(d&(b^c)))+(b>>>2^b>>>13^b>>>22^b<<30^b<<19^b<<10))|0;w8=((w9>>>7^w9>>>18^w9>>>3^w9<<25^w9<<14)+(w6>>>17^w6>>>19^w6>>>10^w6<<15^w6<<13)+w8+w1)|0;h=(w8+h+(e>>>6^e>>>11^e>>>25^e<<26^e<<21^e<<7)+(g^e&(f^g))+0xa2bfe8a1)|0;d=(d+h)|0;h=(h+((a&b)^(c&(a^b)))+(a>>>2^a>>>13^a>>>22^a<<30^a<<19^a<<10))|0;w9=((w10>>>7^w10>>>18^w10>>>3^w10<<25^w10<<14)+(w7>>>17^w7>>>19^w7>>>10^w7<<15^w7<<13)+w9+w2)|0;g=(w9+g+(d>>>6^d>>>11^d>>>25^d<<26^d<<21^d<<7)+(f^d&(e^f))+0xa81a664b)|0;c=(c+g)|0;g=(g+((h&a)^(b&(h^a)))+(h>>>2^h>>>13^h>>>22^h<<30^h<<19^h<<10))|0;w10=((w11>>>7^w11>>>18^w11>>>3^w11<<25^w11<<14)+(w8>>>17^w8>>>19^w8>>>10^w8<<15^w8<<13)+w10+w3)|0;f=(w10+f+(c>>>6^c>>>11^c>>>25^c<<26^c<<21^c<<7)+(e^c&(d^e))+0xc24b8b70)|0;b=(b+f)|0;f=(f+((g&h)^(a&(g^h)))+(g>>>2^g>>>13^g>>>22^g<<30^g<<19^g<<10))|0;w11=((w12>>>7^w12>>>18^w12>>>3^w12<<25^w12<<14)+(w9>>>17^w9>>>19^w9>>>10^w9<<15^w9<<13)+w11+w4)|0;e=(w11+e+(b>>>6^b>>>11^b>>>25^b<<26^b<<21^b<<7)+(d^b&(c^d))+0xc76c51a3)|0;a=(a+e)|0;e=(e+((f&g)^(h&(f^g)))+(f>>>2^f>>>13^f>>>22^f<<30^f<<19^f<<10))|0;w12=((w13>>>7^w13>>>18^w13>>>3^w13<<25^w13<<14)+(w10>>>17^w10>>>19^w10>>>10^w10<<15^w10<<13)+w12+w5)|0;d=(w12+d+(a>>>6^a>>>11^a>>>25^a<<26^a<<21^a<<7)+(c^a&(b^c))+0xd192e819)|0;h=(h+d)|0;d=(d+((e&f)^(g&(e^f)))+(e>>>2^e>>>13^e>>>22^e<<30^e<<19^e<<10))|0;w13=((w14>>>7^w14>>>18^w14>>>3^w14<<25^w14<<14)+(w11>>>17^w11>>>19^w11>>>10^w11<<15^w11<<13)+w13+w6)|0;c=(w13+c+(h>>>6^h>>>11^h>>>25^h<<26^h<<21^h<<7)+(b^h&(a^b))+0xd6990624)|0;g=(g+c)|0;c=(c+((d&e)^(f&(d^e)))+(d>>>2^d>>>13^d>>>22^d<<30^d<<19^d<<10))|0;w14=((w15>>>7^w15>>>18^w15>>>3^w15<<25^w15<<14)+(w12>>>17^w12>>>19^w12>>>10^w12<<15^w12<<13)+w14+w7)|0;b=(w14+b+(g>>>6^g>>>11^g>>>25^g<<26^g<<21^g<<7)+(a^g&(h^a))+0xf40e3585)|0;f=(f+b)|0;b=(b+((c&d)^(e&(c^d)))+(c>>>2^c>>>13^c>>>22^c<<30^c<<19^c<<10))|0;w15=((w0>>>7^w0>>>18^w0>>>3^w0<<25^w0<<14)+(w13>>>17^w13>>>19^w13>>>10^w13<<15^w13<<13)+w15+w8)|0;a=(w15+a+(f>>>6^f>>>11^f>>>25^f<<26^f<<21^f<<7)+(h^f&(g^h))+0x106aa070)|0;e=(e+a)|0;a=(a+((b&c)^(d&(b^c)))+(b>>>2^b>>>13^b>>>22^b<<30^b<<19^b<<10))|0;w0=((w1>>>7^w1>>>18^w1>>>3^w1<<25^w1<<14)+(w14>>>17^w14>>>19^w14>>>10^w14<<15^w14<<13)+w0+w9)|0;h=(w0+h+(e>>>6^e>>>11^e>>>25^e<<26^e<<21^e<<7)+(g^e&(f^g))+0x19a4c116)|0;d=(d+h)|0;h=(h+((a&b)^(c&(a^b)))+(a>>>2^a>>>13^a>>>22^a<<30^a<<19^a<<10))|0;w1=((w2>>>7^w2>>>18^w2>>>3^w2<<25^w2<<14)+(w15>>>17^w15>>>19^w15>>>10^w15<<15^w15<<13)+w1+w10)|0;g=(w1+g+(d>>>6^d>>>11^d>>>25^d<<26^d<<21^d<<7)+(f^d&(e^f))+0x1e376c08)|0;c=(c+g)|0;g=(g+((h&a)^(b&(h^a)))+(h>>>2^h>>>13^h>>>22^h<<30^h<<19^h<<10))|0;w2=((w3>>>7^w3>>>18^w3>>>3^w3<<25^w3<<14)+(w0>>>17^w0>>>19^w0>>>10^w0<<15^w0<<13)+w2+w11)|0;f=(w2+f+(c>>>6^c>>>11^c>>>25^c<<26^c<<21^c<<7)+(e^c&(d^e))+0x2748774c)|0;b=(b+f)|0;f=(f+((g&h)^(a&(g^h)))+(g>>>2^g>>>13^g>>>22^g<<30^g<<19^g<<10))|0;w3=((w4>>>7^w4>>>18^w4>>>3^w4<<25^w4<<14)+(w1>>>17^w1>>>19^w1>>>10^w1<<15^w1<<13)+w3+w12)|0;e=(w3+e+(b>>>6^b>>>11^b>>>25^b<<26^b<<21^b<<7)+(d^b&(c^d))+0x34b0bcb5)|0;a=(a+e)|0;e=(e+((f&g)^(h&(f^g)))+(f>>>2^f>>>13^f>>>22^f<<30^f<<19^f<<10))|0;w4=((w5>>>7^w5>>>18^w5>>>3^w5<<25^w5<<14)+(w2>>>17^w2>>>19^w2>>>10^w2<<15^w2<<13)+w4+w13)|0;d=(w4+d+(a>>>6^a>>>11^a>>>25^a<<26^a<<21^a<<7)+(c^a&(b^c))+0x391c0cb3)|0;h=(h+d)|0;d=(d+((e&f)^(g&(e^f)))+(e>>>2^e>>>13^e>>>22^e<<30^e<<19^e<<10))|0;w5=((w6>>>7^w6>>>18^w6>>>3^w6<<25^w6<<14)+(w3>>>17^w3>>>19^w3>>>10^w3<<15^w3<<13)+w5+w14)|0;c=(w5+c+(h>>>6^h>>>11^h>>>25^h<<26^h<<21^h<<7)+(b^h&(a^b))+0x4ed8aa4a)|0;g=(g+c)|0;c=(c+((d&e)^(f&(d^e)))+(d>>>2^d>>>13^d>>>22^d<<30^d<<19^d<<10))|0;w6=((w7>>>7^w7>>>18^w7>>>3^w7<<25^w7<<14)+(w4>>>17^w4>>>19^w4>>>10^w4<<15^w4<<13)+w6+w15)|0;b=(w6+b+(g>>>6^g>>>11^g>>>25^g<<26^g<<21^g<<7)+(a^g&(h^a))+0x5b9cca4f)|0;f=(f+b)|0;b=(b+((c&d)^(e&(c^d)))+(c>>>2^c>>>13^c>>>22^c<<30^c<<19^c<<10))|0;w7=((w8>>>7^w8>>>18^w8>>>3^w8<<25^w8<<14)+(w5>>>17^w5>>>19^w5>>>10^w5<<15^w5<<13)+w7+w0)|0;a=(w7+a+(f>>>6^f>>>11^f>>>25^f<<26^f<<21^f<<7)+(h^f&(g^h))+0x682e6ff3)|0;e=(e+a)|0;a=(a+((b&c)^(d&(b^c)))+(b>>>2^b>>>13^b>>>22^b<<30^b<<19^b<<10))|0;w8=((w9>>>7^w9>>>18^w9>>>3^w9<<25^w9<<14)+(w6>>>17^w6>>>19^w6>>>10^w6<<15^w6<<13)+w8+w1)|0;h=(w8+h+(e>>>6^e>>>11^e>>>25^e<<26^e<<21^e<<7)+(g^e&(f^g))+0x748f82ee)|0;d=(d+h)|0;h=(h+((a&b)^(c&(a^b)))+(a>>>2^a>>>13^a>>>22^a<<30^a<<19^a<<10))|0;w9=((w10>>>7^w10>>>18^w10>>>3^w10<<25^w10<<14)+(w7>>>17^w7>>>19^w7>>>10^w7<<15^w7<<13)+w9+w2)|0;g=(w9+g+(d>>>6^d>>>11^d>>>25^d<<26^d<<21^d<<7)+(f^d&(e^f))+0x78a5636f)|0;c=(c+g)|0;g=(g+((h&a)^(b&(h^a)))+(h>>>2^h>>>13^h>>>22^h<<30^h<<19^h<<10))|0;w10=((w11>>>7^w11>>>18^w11>>>3^w11<<25^w11<<14)+(w8>>>17^w8>>>19^w8>>>10^w8<<15^w8<<13)+w10+w3)|0;f=(w10+f+(c>>>6^c>>>11^c>>>25^c<<26^c<<21^c<<7)+(e^c&(d^e))+0x84c87814)|0;b=(b+f)|0;f=(f+((g&h)^(a&(g^h)))+(g>>>2^g>>>13^g>>>22^g<<30^g<<19^g<<10))|0;w11=((w12>>>7^w12>>>18^w12>>>3^w12<<25^w12<<14)+(w9>>>17^w9>>>19^w9>>>10^w9<<15^w9<<13)+w11+w4)|0;e=(w11+e+(b>>>6^b>>>11^b>>>25^b<<26^b<<21^b<<7)+(d^b&(c^d))+0x8cc70208)|0;a=(a+e)|0;e=(e+((f&g)^(h&(f^g)))+(f>>>2^f>>>13^f>>>22^f<<30^f<<19^f<<10))|0;w12=((w13>>>7^w13>>>18^w13>>>3^w13<<25^w13<<14)+(w10>>>17^w10>>>19^w10>>>10^w10<<15^w10<<13)+w12+w5)|0;d=(w12+d+(a>>>6^a>>>11^a>>>25^a<<26^a<<21^a<<7)+(c^a&(b^c))+0x90befffa)|0;h=(h+d)|0;d=(d+((e&f)^(g&(e^f)))+(e>>>2^e>>>13^e>>>22^e<<30^e<<19^e<<10))|0;w13=((w14>>>7^w14>>>18^w14>>>3^w14<<25^w14<<14)+(w11>>>17^w11>>>19^w11>>>10^w11<<15^w11<<13)+w13+w6)|0;c=(w13+c+(h>>>6^h>>>11^h>>>25^h<<26^h<<21^h<<7)+(b^h&(a^b))+0xa4506ceb)|0;g=(g+c)|0;c=(c+((d&e)^(f&(d^e)))+(d>>>2^d>>>13^d>>>22^d<<30^d<<19^d<<10))|0;w14=((w15>>>7^w15>>>18^w15>>>3^w15<<25^w15<<14)+(w12>>>17^w12>>>19^w12>>>10^w12<<15^w12<<13)+w14+w7)|0;b=(w14+b+(g>>>6^g>>>11^g>>>25^g<<26^g<<21^g<<7)+(a^g&(h^a))+0xbef9a3f7)|0;f=(f+b)|0;b=(b+((c&d)^(e&(c^d)))+(c>>>2^c>>>13^c>>>22^c<<30^c<<19^c<<10))|0;w15=((w0>>>7^w0>>>18^w0>>>3^w0<<25^w0<<14)+(w13>>>17^w13>>>19^w13>>>10^w13<<15^w13<<13)+w15+w8)|0;a=(w15+a+(f>>>6^f>>>11^f>>>25^f<<26^f<<21^f<<7)+(h^f&(g^h))+0xc67178f2)|0;e=(e+a)|0;a=(a+((b&c)^(d&(b^c)))+(b>>>2^b>>>13^b>>>22^b<<30^b<<19^b<<10))|0;A=(A+a)|0;B=(B+b)|0;C=(C+c)|0;D=(D+d)|0;E=(E+e)|0;F=(F+f)|0;G=(G+g)|0;H=(H+h)|0}function reset(){A=0x6a09e667;B=0xbb67ae85;C=0x3c6ef372;D=0xa54ff53a;E=0x510e527f;F=0x9b05688c;G=0x1f83d9ab;H=0x5be0cd19;I=J=0}function update(j,l){j=j|0;l=l|0;var h=0;if(j&63)return-1;while((l|0)>=64){z(j);j=(j+64)|0;l=(l-64)|0;h=(h+64)|0}I=(I+h)|0;if(I>>>0<h>>>0)J=(J+1)|0;return h|0}function digest(j,l,o){j=j|0;l=l|0;o=o|0;var h=0,i=0;if(j&63)return-1;if(~o)if(o&31)return-1;if((l|0)>=64){h=update(j,l)|0;if((h|0)==-1)return-1;j=(j+h)|0;l=(l-h)|0}h=(h+l)|0;I=(I+l)|0;if(I>>>0<l>>>0)J=(J+1)|0;K[j|l]=128;if((l|0)>=56){for(i=(l+1)|0;(i|0)<64;i=(i+1)|0)K[j|i]=0;z(j);l=0;K[j|0]=0}for(i=(l+1)|0;(i|0)<59;i=(i+1)|0)K[j|i]=0;K[j|56]=J>>>21&255;K[j|57]=J>>>13&255;K[j|58]=J>>>5&255;K[j|59]=J<<3&255|I>>>29;K[j|60]=I>>>21&255;K[j|61]=I>>>13&255;K[j|62]=I>>>5&255;K[j|63]=I<<3&255;z(j);if(~o){o=o|0;K[o|0]=A>>>24;K[o|1]=A>>>16&255;K[o|2]=A>>>8&255;K[o|3]=A&255;K[o|4]=B>>>24;K[o|5]=B>>>16&255;K[o|6]=B>>>8&255;K[o|7]=B&255;K[o|8]=C>>>24;K[o|9]=C>>>16&255;K[o|10]=C>>>8&255;K[o|11]=C&255;K[o|12]=D>>>24;K[o|13]=D>>>16&255;K[o|14]=D>>>8&255;K[o|15]=D&255;K[o|16]=E>>>24;K[o|17]=E>>>16&255;K[o|18]=E>>>8&255;K[o|19]=E&255;K[o|20]=F>>>24;K[o|21]=F>>>16&255;K[o|22]=F>>>8&255;K[o|23]=F&255;K[o|24]=G>>>24;K[o|25]=G>>>16&255;K[o|26]=G>>>8&255;K[o|27]=G&255;K[o|28]=H>>>24;K[o|29]=H>>>16&255;K[o|30]=H>>>8&255;K[o|31]=H&255}return h|0}return{reset:reset,update:update,digest:digest}"));
    const fa = (a, b) => {
            if ("string" === typeof a)
                if ("latin1" === b || "base64" === b) {
                    b = "base64" === b ? atob(a) : a;
                    a = new Uint8Array(b.length);
                    for (var c = 0; c < b.length; c++) a[c] = b.charCodeAt(c)
                } else if ("hex" === b) {
                b = new Uint8Array(a.length / 2);
                for (c = 0; c < a.length; c += 2) b[c >> 1] = parseInt(a.slice(c, c + 2), 16);
                a = b
            }
            return a.buffer || a
        },
        ha = a => {
            a = new Uint8Array(a);
            return btoa(String.fromCharCode.apply(String, a))
        };

    function ia(a, b = "latin1") {
        a = fa(a, b);
        if (!(a instanceof ArrayBuffer)) throw new TypeError("Invalid data type");
        a = da(a);
        return a instanceof Promise ? a.then(c => ha(c)) : ha(a)
    }
    var ma = (() => {
        let a = URL.createObjectURL(new Blob(["!function(m,x){x=x(m);self.onmessage=function(d){for(var b=d.data[1],a=new Uint8Array(6+b.length),c=0;c<b.length;c++)a[c+6]=b.charCodeAt(c);for(b=0;1073741824>b;b++)for(a[0]=48+(63&b>>24),a[1]=48+(63&b>>18),a[2]=48+(63&b>>12),a[3]=48+(63&b>>6),a[4]=48+(63&b),c=0;64>c;c++)if(a[5]=48+c,0===(new Uint16Array(x(a.buffer)))[0]){postMessage([d.data[0],String.fromCharCode(a[0],a[1],a[2],a[3],a[4],a[5])]);return}}}(function(", "s,f,b", "){", "'use asm';var A=0,B=0,C=0,D=0,E=0,F=0,G=0,H=0,I=0,J=0;var K=new s.Uint8Array(b);function z(j){j=j|0;var w0=0,w1=0,w2=0,w3=0,w4=0,w5=0,w6=0,w7=0,w8=0,w9=0,w10=0,w11=0,w12=0,w13=0,w14=0,w15=0;var a=0,b=0,c=0,d=0,e=0,f=0,g=0,h=0;w0=K[j|0]<<24|K[j|1]<<16|K[j|2]<<8|K[j|3];w1=K[j|4]<<24|K[j|5]<<16|K[j|6]<<8|K[j|7];w2=K[j|8]<<24|K[j|9]<<16|K[j|10]<<8|K[j|11];w3=K[j|12]<<24|K[j|13]<<16|K[j|14]<<8|K[j|15];w4=K[j|16]<<24|K[j|17]<<16|K[j|18]<<8|K[j|19];w5=K[j|20]<<24|K[j|21]<<16|K[j|22]<<8|K[j|23];w6=K[j|24]<<24|K[j|25]<<16|K[j|26]<<8|K[j|27];w7=K[j|28]<<24|K[j|29]<<16|K[j|30]<<8|K[j|31];w8=K[j|32]<<24|K[j|33]<<16|K[j|34]<<8|K[j|35];w9=K[j|36]<<24|K[j|37]<<16|K[j|38]<<8|K[j|39];w10=K[j|40]<<24|K[j|41]<<16|K[j|42]<<8|K[j|43];w11=K[j|44]<<24|K[j|45]<<16|K[j|46]<<8|K[j|47];w12=K[j|48]<<24|K[j|49]<<16|K[j|50]<<8|K[j|51];w13=K[j|52]<<24|K[j|53]<<16|K[j|54]<<8|K[j|55];w14=K[j|56]<<24|K[j|57]<<16|K[j|58]<<8|K[j|59];w15=K[j|60]<<24|K[j|61]<<16|K[j|62]<<8|K[j|63];a=A;b=B;c=C;d=D;e=E;f=F;g=G;h=H;h=(w0+h+(e>>>6^e>>>11^e>>>25^e<<26^e<<21^e<<7)+(g^e&(f^g))+0x428a2f98)|0;d=(d+h)|0;h=(h+((a&b)^(c&(a^b)))+(a>>>2^a>>>13^a>>>22^a<<30^a<<19^a<<10))|0;g=(w1+g+(d>>>6^d>>>11^d>>>25^d<<26^d<<21^d<<7)+(f^d&(e^f))+0x71374491)|0;c=(c+g)|0;g=(g+((h&a)^(b&(h^a)))+(h>>>2^h>>>13^h>>>22^h<<30^h<<19^h<<10))|0;f=(w2+f+(c>>>6^c>>>11^c>>>25^c<<26^c<<21^c<<7)+(e^c&(d^e))+0xb5c0fbcf)|0;b=(b+f)|0;f=(f+((g&h)^(a&(g^h)))+(g>>>2^g>>>13^g>>>22^g<<30^g<<19^g<<10))|0;e=(w3+e+(b>>>6^b>>>11^b>>>25^b<<26^b<<21^b<<7)+(d^b&(c^d))+0xe9b5dba5)|0;a=(a+e)|0;e=(e+((f&g)^(h&(f^g)))+(f>>>2^f>>>13^f>>>22^f<<30^f<<19^f<<10))|0;d=(w4+d+(a>>>6^a>>>11^a>>>25^a<<26^a<<21^a<<7)+(c^a&(b^c))+0x3956c25b)|0;h=(h+d)|0;d=(d+((e&f)^(g&(e^f)))+(e>>>2^e>>>13^e>>>22^e<<30^e<<19^e<<10))|0;c=(w5+c+(h>>>6^h>>>11^h>>>25^h<<26^h<<21^h<<7)+(b^h&(a^b))+0x59f111f1)|0;g=(g+c)|0;c=(c+((d&e)^(f&(d^e)))+(d>>>2^d>>>13^d>>>22^d<<30^d<<19^d<<10))|0;b=(w6+b+(g>>>6^g>>>11^g>>>25^g<<26^g<<21^g<<7)+(a^g&(h^a))+0x923f82a4)|0;f=(f+b)|0;b=(b+((c&d)^(e&(c^d)))+(c>>>2^c>>>13^c>>>22^c<<30^c<<19^c<<10))|0;a=(w7+a+(f>>>6^f>>>11^f>>>25^f<<26^f<<21^f<<7)+(h^f&(g^h))+0xab1c5ed5)|0;e=(e+a)|0;a=(a+((b&c)^(d&(b^c)))+(b>>>2^b>>>13^b>>>22^b<<30^b<<19^b<<10))|0;h=(w8+h+(e>>>6^e>>>11^e>>>25^e<<26^e<<21^e<<7)+(g^e&(f^g))+0xd807aa98)|0;d=(d+h)|0;h=(h+((a&b)^(c&(a^b)))+(a>>>2^a>>>13^a>>>22^a<<30^a<<19^a<<10))|0;g=(w9+g+(d>>>6^d>>>11^d>>>25^d<<26^d<<21^d<<7)+(f^d&(e^f))+0x12835b01)|0;c=(c+g)|0;g=(g+((h&a)^(b&(h^a)))+(h>>>2^h>>>13^h>>>22^h<<30^h<<19^h<<10))|0;f=(w10+f+(c>>>6^c>>>11^c>>>25^c<<26^c<<21^c<<7)+(e^c&(d^e))+0x243185be)|0;b=(b+f)|0;f=(f+((g&h)^(a&(g^h)))+(g>>>2^g>>>13^g>>>22^g<<30^g<<19^g<<10))|0;e=(w11+e+(b>>>6^b>>>11^b>>>25^b<<26^b<<21^b<<7)+(d^b&(c^d))+0x550c7dc3)|0;a=(a+e)|0;e=(e+((f&g)^(h&(f^g)))+(f>>>2^f>>>13^f>>>22^f<<30^f<<19^f<<10))|0;d=(w12+d+(a>>>6^a>>>11^a>>>25^a<<26^a<<21^a<<7)+(c^a&(b^c))+0x72be5d74)|0;h=(h+d)|0;d=(d+((e&f)^(g&(e^f)))+(e>>>2^e>>>13^e>>>22^e<<30^e<<19^e<<10))|0;c=(w13+c+(h>>>6^h>>>11^h>>>25^h<<26^h<<21^h<<7)+(b^h&(a^b))+0x80deb1fe)|0;g=(g+c)|0;c=(c+((d&e)^(f&(d^e)))+(d>>>2^d>>>13^d>>>22^d<<30^d<<19^d<<10))|0;b=(w14+b+(g>>>6^g>>>11^g>>>25^g<<26^g<<21^g<<7)+(a^g&(h^a))+0x9bdc06a7)|0;f=(f+b)|0;b=(b+((c&d)^(e&(c^d)))+(c>>>2^c>>>13^c>>>22^c<<30^c<<19^c<<10))|0;a=(w15+a+(f>>>6^f>>>11^f>>>25^f<<26^f<<21^f<<7)+(h^f&(g^h))+0xc19bf174)|0;e=(e+a)|0;a=(a+((b&c)^(d&(b^c)))+(b>>>2^b>>>13^b>>>22^b<<30^b<<19^b<<10))|0;w0=((w1>>>7^w1>>>18^w1>>>3^w1<<25^w1<<14)+(w14>>>17^w14>>>19^w14>>>10^w14<<15^w14<<13)+w0+w9)|0;h=(w0+h+(e>>>6^e>>>11^e>>>25^e<<26^e<<21^e<<7)+(g^e&(f^g))+0xe49b69c1)|0;d=(d+h)|0;h=(h+((a&b)^(c&(a^b)))+(a>>>2^a>>>13^a>>>22^a<<30^a<<19^a<<10))|0;w1=((w2>>>7^w2>>>18^w2>>>3^w2<<25^w2<<14)+(w15>>>17^w15>>>19^w15>>>10^w15<<15^w15<<13)+w1+w10)|0;g=(w1+g+(d>>>6^d>>>11^d>>>25^d<<26^d<<21^d<<7)+(f^d&(e^f))+0xefbe4786)|0;c=(c+g)|0;g=(g+((h&a)^(b&(h^a)))+(h>>>2^h>>>13^h>>>22^h<<30^h<<19^h<<10))|0;w2=((w3>>>7^w3>>>18^w3>>>3^w3<<25^w3<<14)+(w0>>>17^w0>>>19^w0>>>10^w0<<15^w0<<13)+w2+w11)|0;f=(w2+f+(c>>>6^c>>>11^c>>>25^c<<26^c<<21^c<<7)+(e^c&(d^e))+0x0fc19dc6)|0;b=(b+f)|0;f=(f+((g&h)^(a&(g^h)))+(g>>>2^g>>>13^g>>>22^g<<30^g<<19^g<<10))|0;w3=((w4>>>7^w4>>>18^w4>>>3^w4<<25^w4<<14)+(w1>>>17^w1>>>19^w1>>>10^w1<<15^w1<<13)+w3+w12)|0;e=(w3+e+(b>>>6^b>>>11^b>>>25^b<<26^b<<21^b<<7)+(d^b&(c^d))+0x240ca1cc)|0;a=(a+e)|0;e=(e+((f&g)^(h&(f^g)))+(f>>>2^f>>>13^f>>>22^f<<30^f<<19^f<<10))|0;w4=((w5>>>7^w5>>>18^w5>>>3^w5<<25^w5<<14)+(w2>>>17^w2>>>19^w2>>>10^w2<<15^w2<<13)+w4+w13)|0;d=(w4+d+(a>>>6^a>>>11^a>>>25^a<<26^a<<21^a<<7)+(c^a&(b^c))+0x2de92c6f)|0;h=(h+d)|0;d=(d+((e&f)^(g&(e^f)))+(e>>>2^e>>>13^e>>>22^e<<30^e<<19^e<<10))|0;w5=((w6>>>7^w6>>>18^w6>>>3^w6<<25^w6<<14)+(w3>>>17^w3>>>19^w3>>>10^w3<<15^w3<<13)+w5+w14)|0;c=(w5+c+(h>>>6^h>>>11^h>>>25^h<<26^h<<21^h<<7)+(b^h&(a^b))+0x4a7484aa)|0;g=(g+c)|0;c=(c+((d&e)^(f&(d^e)))+(d>>>2^d>>>13^d>>>22^d<<30^d<<19^d<<10))|0;w6=((w7>>>7^w7>>>18^w7>>>3^w7<<25^w7<<14)+(w4>>>17^w4>>>19^w4>>>10^w4<<15^w4<<13)+w6+w15)|0;b=(w6+b+(g>>>6^g>>>11^g>>>25^g<<26^g<<21^g<<7)+(a^g&(h^a))+0x5cb0a9dc)|0;f=(f+b)|0;b=(b+((c&d)^(e&(c^d)))+(c>>>2^c>>>13^c>>>22^c<<30^c<<19^c<<10))|0;w7=((w8>>>7^w8>>>18^w8>>>3^w8<<25^w8<<14)+(w5>>>17^w5>>>19^w5>>>10^w5<<15^w5<<13)+w7+w0)|0;a=(w7+a+(f>>>6^f>>>11^f>>>25^f<<26^f<<21^f<<7)+(h^f&(g^h))+0x76f988da)|0;e=(e+a)|0;a=(a+((b&c)^(d&(b^c)))+(b>>>2^b>>>13^b>>>22^b<<30^b<<19^b<<10))|0;w8=((w9>>>7^w9>>>18^w9>>>3^w9<<25^w9<<14)+(w6>>>17^w6>>>19^w6>>>10^w6<<15^w6<<13)+w8+w1)|0;h=(w8+h+(e>>>6^e>>>11^e>>>25^e<<26^e<<21^e<<7)+(g^e&(f^g))+0x983e5152)|0;d=(d+h)|0;h=(h+((a&b)^(c&(a^b)))+(a>>>2^a>>>13^a>>>22^a<<30^a<<19^a<<10))|0;w9=((w10>>>7^w10>>>18^w10>>>3^w10<<25^w10<<14)+(w7>>>17^w7>>>19^w7>>>10^w7<<15^w7<<13)+w9+w2)|0;g=(w9+g+(d>>>6^d>>>11^d>>>25^d<<26^d<<21^d<<7)+(f^d&(e^f))+0xa831c66d)|0;c=(c+g)|0;g=(g+((h&a)^(b&(h^a)))+(h>>>2^h>>>13^h>>>22^h<<30^h<<19^h<<10))|0;w10=((w11>>>7^w11>>>18^w11>>>3^w11<<25^w11<<14)+(w8>>>17^w8>>>19^w8>>>10^w8<<15^w8<<13)+w10+w3)|0;f=(w10+f+(c>>>6^c>>>11^c>>>25^c<<26^c<<21^c<<7)+(e^c&(d^e))+0xb00327c8)|0;b=(b+f)|0;f=(f+((g&h)^(a&(g^h)))+(g>>>2^g>>>13^g>>>22^g<<30^g<<19^g<<10))|0;w11=((w12>>>7^w12>>>18^w12>>>3^w12<<25^w12<<14)+(w9>>>17^w9>>>19^w9>>>10^w9<<15^w9<<13)+w11+w4)|0;e=(w11+e+(b>>>6^b>>>11^b>>>25^b<<26^b<<21^b<<7)+(d^b&(c^d))+0xbf597fc7)|0;a=(a+e)|0;e=(e+((f&g)^(h&(f^g)))+(f>>>2^f>>>13^f>>>22^f<<30^f<<19^f<<10))|0;w12=((w13>>>7^w13>>>18^w13>>>3^w13<<25^w13<<14)+(w10>>>17^w10>>>19^w10>>>10^w10<<15^w10<<13)+w12+w5)|0;d=(w12+d+(a>>>6^a>>>11^a>>>25^a<<26^a<<21^a<<7)+(c^a&(b^c))+0xc6e00bf3)|0;h=(h+d)|0;d=(d+((e&f)^(g&(e^f)))+(e>>>2^e>>>13^e>>>22^e<<30^e<<19^e<<10))|0;w13=((w14>>>7^w14>>>18^w14>>>3^w14<<25^w14<<14)+(w11>>>17^w11>>>19^w11>>>10^w11<<15^w11<<13)+w13+w6)|0;c=(w13+c+(h>>>6^h>>>11^h>>>25^h<<26^h<<21^h<<7)+(b^h&(a^b))+0xd5a79147)|0;g=(g+c)|0;c=(c+((d&e)^(f&(d^e)))+(d>>>2^d>>>13^d>>>22^d<<30^d<<19^d<<10))|0;w14=((w15>>>7^w15>>>18^w15>>>3^w15<<25^w15<<14)+(w12>>>17^w12>>>19^w12>>>10^w12<<15^w12<<13)+w14+w7)|0;b=(w14+b+(g>>>6^g>>>11^g>>>25^g<<26^g<<21^g<<7)+(a^g&(h^a))+0x06ca6351)|0;f=(f+b)|0;b=(b+((c&d)^(e&(c^d)))+(c>>>2^c>>>13^c>>>22^c<<30^c<<19^c<<10))|0;w15=((w0>>>7^w0>>>18^w0>>>3^w0<<25^w0<<14)+(w13>>>17^w13>>>19^w13>>>10^w13<<15^w13<<13)+w15+w8)|0;a=(w15+a+(f>>>6^f>>>11^f>>>25^f<<26^f<<21^f<<7)+(h^f&(g^h))+0x14292967)|0;e=(e+a)|0;a=(a+((b&c)^(d&(b^c)))+(b>>>2^b>>>13^b>>>22^b<<30^b<<19^b<<10))|0;w0=((w1>>>7^w1>>>18^w1>>>3^w1<<25^w1<<14)+(w14>>>17^w14>>>19^w14>>>10^w14<<15^w14<<13)+w0+w9)|0;h=(w0+h+(e>>>6^e>>>11^e>>>25^e<<26^e<<21^e<<7)+(g^e&(f^g))+0x27b70a85)|0;d=(d+h)|0;h=(h+((a&b)^(c&(a^b)))+(a>>>2^a>>>13^a>>>22^a<<30^a<<19^a<<10))|0;w1=((w2>>>7^w2>>>18^w2>>>3^w2<<25^w2<<14)+(w15>>>17^w15>>>19^w15>>>10^w15<<15^w15<<13)+w1+w10)|0;g=(w1+g+(d>>>6^d>>>11^d>>>25^d<<26^d<<21^d<<7)+(f^d&(e^f))+0x2e1b2138)|0;c=(c+g)|0;g=(g+((h&a)^(b&(h^a)))+(h>>>2^h>>>13^h>>>22^h<<30^h<<19^h<<10))|0;w2=((w3>>>7^w3>>>18^w3>>>3^w3<<25^w3<<14)+(w0>>>17^w0>>>19^w0>>>10^w0<<15^w0<<13)+w2+w11)|0;f=(w2+f+(c>>>6^c>>>11^c>>>25^c<<26^c<<21^c<<7)+(e^c&(d^e))+0x4d2c6dfc)|0;b=(b+f)|0;f=(f+((g&h)^(a&(g^h)))+(g>>>2^g>>>13^g>>>22^g<<30^g<<19^g<<10))|0;w3=((w4>>>7^w4>>>18^w4>>>3^w4<<25^w4<<14)+(w1>>>17^w1>>>19^w1>>>10^w1<<15^w1<<13)+w3+w12)|0;e=(w3+e+(b>>>6^b>>>11^b>>>25^b<<26^b<<21^b<<7)+(d^b&(c^d))+0x53380d13)|0;a=(a+e)|0;e=(e+((f&g)^(h&(f^g)))+(f>>>2^f>>>13^f>>>22^f<<30^f<<19^f<<10))|0;w4=((w5>>>7^w5>>>18^w5>>>3^w5<<25^w5<<14)+(w2>>>17^w2>>>19^w2>>>10^w2<<15^w2<<13)+w4+w13)|0;d=(w4+d+(a>>>6^a>>>11^a>>>25^a<<26^a<<21^a<<7)+(c^a&(b^c))+0x650a7354)|0;h=(h+d)|0;d=(d+((e&f)^(g&(e^f)))+(e>>>2^e>>>13^e>>>22^e<<30^e<<19^e<<10))|0;w5=((w6>>>7^w6>>>18^w6>>>3^w6<<25^w6<<14)+(w3>>>17^w3>>>19^w3>>>10^w3<<15^w3<<13)+w5+w14)|0;c=(w5+c+(h>>>6^h>>>11^h>>>25^h<<26^h<<21^h<<7)+(b^h&(a^b))+0x766a0abb)|0;g=(g+c)|0;c=(c+((d&e)^(f&(d^e)))+(d>>>2^d>>>13^d>>>22^d<<30^d<<19^d<<10))|0;w6=((w7>>>7^w7>>>18^w7>>>3^w7<<25^w7<<14)+(w4>>>17^w4>>>19^w4>>>10^w4<<15^w4<<13)+w6+w15)|0;b=(w6+b+(g>>>6^g>>>11^g>>>25^g<<26^g<<21^g<<7)+(a^g&(h^a))+0x81c2c92e)|0;f=(f+b)|0;b=(b+((c&d)^(e&(c^d)))+(c>>>2^c>>>13^c>>>22^c<<30^c<<19^c<<10))|0;w7=((w8>>>7^w8>>>18^w8>>>3^w8<<25^w8<<14)+(w5>>>17^w5>>>19^w5>>>10^w5<<15^w5<<13)+w7+w0)|0;a=(w7+a+(f>>>6^f>>>11^f>>>25^f<<26^f<<21^f<<7)+(h^f&(g^h))+0x92722c85)|0;e=(e+a)|0;a=(a+((b&c)^(d&(b^c)))+(b>>>2^b>>>13^b>>>22^b<<30^b<<19^b<<10))|0;w8=((w9>>>7^w9>>>18^w9>>>3^w9<<25^w9<<14)+(w6>>>17^w6>>>19^w6>>>10^w6<<15^w6<<13)+w8+w1)|0;h=(w8+h+(e>>>6^e>>>11^e>>>25^e<<26^e<<21^e<<7)+(g^e&(f^g))+0xa2bfe8a1)|0;d=(d+h)|0;h=(h+((a&b)^(c&(a^b)))+(a>>>2^a>>>13^a>>>22^a<<30^a<<19^a<<10))|0;w9=((w10>>>7^w10>>>18^w10>>>3^w10<<25^w10<<14)+(w7>>>17^w7>>>19^w7>>>10^w7<<15^w7<<13)+w9+w2)|0;g=(w9+g+(d>>>6^d>>>11^d>>>25^d<<26^d<<21^d<<7)+(f^d&(e^f))+0xa81a664b)|0;c=(c+g)|0;g=(g+((h&a)^(b&(h^a)))+(h>>>2^h>>>13^h>>>22^h<<30^h<<19^h<<10))|0;w10=((w11>>>7^w11>>>18^w11>>>3^w11<<25^w11<<14)+(w8>>>17^w8>>>19^w8>>>10^w8<<15^w8<<13)+w10+w3)|0;f=(w10+f+(c>>>6^c>>>11^c>>>25^c<<26^c<<21^c<<7)+(e^c&(d^e))+0xc24b8b70)|0;b=(b+f)|0;f=(f+((g&h)^(a&(g^h)))+(g>>>2^g>>>13^g>>>22^g<<30^g<<19^g<<10))|0;w11=((w12>>>7^w12>>>18^w12>>>3^w12<<25^w12<<14)+(w9>>>17^w9>>>19^w9>>>10^w9<<15^w9<<13)+w11+w4)|0;e=(w11+e+(b>>>6^b>>>11^b>>>25^b<<26^b<<21^b<<7)+(d^b&(c^d))+0xc76c51a3)|0;a=(a+e)|0;e=(e+((f&g)^(h&(f^g)))+(f>>>2^f>>>13^f>>>22^f<<30^f<<19^f<<10))|0;w12=((w13>>>7^w13>>>18^w13>>>3^w13<<25^w13<<14)+(w10>>>17^w10>>>19^w10>>>10^w10<<15^w10<<13)+w12+w5)|0;d=(w12+d+(a>>>6^a>>>11^a>>>25^a<<26^a<<21^a<<7)+(c^a&(b^c))+0xd192e819)|0;h=(h+d)|0;d=(d+((e&f)^(g&(e^f)))+(e>>>2^e>>>13^e>>>22^e<<30^e<<19^e<<10))|0;w13=((w14>>>7^w14>>>18^w14>>>3^w14<<25^w14<<14)+(w11>>>17^w11>>>19^w11>>>10^w11<<15^w11<<13)+w13+w6)|0;c=(w13+c+(h>>>6^h>>>11^h>>>25^h<<26^h<<21^h<<7)+(b^h&(a^b))+0xd6990624)|0;g=(g+c)|0;c=(c+((d&e)^(f&(d^e)))+(d>>>2^d>>>13^d>>>22^d<<30^d<<19^d<<10))|0;w14=((w15>>>7^w15>>>18^w15>>>3^w15<<25^w15<<14)+(w12>>>17^w12>>>19^w12>>>10^w12<<15^w12<<13)+w14+w7)|0;b=(w14+b+(g>>>6^g>>>11^g>>>25^g<<26^g<<21^g<<7)+(a^g&(h^a))+0xf40e3585)|0;f=(f+b)|0;b=(b+((c&d)^(e&(c^d)))+(c>>>2^c>>>13^c>>>22^c<<30^c<<19^c<<10))|0;w15=((w0>>>7^w0>>>18^w0>>>3^w0<<25^w0<<14)+(w13>>>17^w13>>>19^w13>>>10^w13<<15^w13<<13)+w15+w8)|0;a=(w15+a+(f>>>6^f>>>11^f>>>25^f<<26^f<<21^f<<7)+(h^f&(g^h))+0x106aa070)|0;e=(e+a)|0;a=(a+((b&c)^(d&(b^c)))+(b>>>2^b>>>13^b>>>22^b<<30^b<<19^b<<10))|0;w0=((w1>>>7^w1>>>18^w1>>>3^w1<<25^w1<<14)+(w14>>>17^w14>>>19^w14>>>10^w14<<15^w14<<13)+w0+w9)|0;h=(w0+h+(e>>>6^e>>>11^e>>>25^e<<26^e<<21^e<<7)+(g^e&(f^g))+0x19a4c116)|0;d=(d+h)|0;h=(h+((a&b)^(c&(a^b)))+(a>>>2^a>>>13^a>>>22^a<<30^a<<19^a<<10))|0;w1=((w2>>>7^w2>>>18^w2>>>3^w2<<25^w2<<14)+(w15>>>17^w15>>>19^w15>>>10^w15<<15^w15<<13)+w1+w10)|0;g=(w1+g+(d>>>6^d>>>11^d>>>25^d<<26^d<<21^d<<7)+(f^d&(e^f))+0x1e376c08)|0;c=(c+g)|0;g=(g+((h&a)^(b&(h^a)))+(h>>>2^h>>>13^h>>>22^h<<30^h<<19^h<<10))|0;w2=((w3>>>7^w3>>>18^w3>>>3^w3<<25^w3<<14)+(w0>>>17^w0>>>19^w0>>>10^w0<<15^w0<<13)+w2+w11)|0;f=(w2+f+(c>>>6^c>>>11^c>>>25^c<<26^c<<21^c<<7)+(e^c&(d^e))+0x2748774c)|0;b=(b+f)|0;f=(f+((g&h)^(a&(g^h)))+(g>>>2^g>>>13^g>>>22^g<<30^g<<19^g<<10))|0;w3=((w4>>>7^w4>>>18^w4>>>3^w4<<25^w4<<14)+(w1>>>17^w1>>>19^w1>>>10^w1<<15^w1<<13)+w3+w12)|0;e=(w3+e+(b>>>6^b>>>11^b>>>25^b<<26^b<<21^b<<7)+(d^b&(c^d))+0x34b0bcb5)|0;a=(a+e)|0;e=(e+((f&g)^(h&(f^g)))+(f>>>2^f>>>13^f>>>22^f<<30^f<<19^f<<10))|0;w4=((w5>>>7^w5>>>18^w5>>>3^w5<<25^w5<<14)+(w2>>>17^w2>>>19^w2>>>10^w2<<15^w2<<13)+w4+w13)|0;d=(w4+d+(a>>>6^a>>>11^a>>>25^a<<26^a<<21^a<<7)+(c^a&(b^c))+0x391c0cb3)|0;h=(h+d)|0;d=(d+((e&f)^(g&(e^f)))+(e>>>2^e>>>13^e>>>22^e<<30^e<<19^e<<10))|0;w5=((w6>>>7^w6>>>18^w6>>>3^w6<<25^w6<<14)+(w3>>>17^w3>>>19^w3>>>10^w3<<15^w3<<13)+w5+w14)|0;c=(w5+c+(h>>>6^h>>>11^h>>>25^h<<26^h<<21^h<<7)+(b^h&(a^b))+0x4ed8aa4a)|0;g=(g+c)|0;c=(c+((d&e)^(f&(d^e)))+(d>>>2^d>>>13^d>>>22^d<<30^d<<19^d<<10))|0;w6=((w7>>>7^w7>>>18^w7>>>3^w7<<25^w7<<14)+(w4>>>17^w4>>>19^w4>>>10^w4<<15^w4<<13)+w6+w15)|0;b=(w6+b+(g>>>6^g>>>11^g>>>25^g<<26^g<<21^g<<7)+(a^g&(h^a))+0x5b9cca4f)|0;f=(f+b)|0;b=(b+((c&d)^(e&(c^d)))+(c>>>2^c>>>13^c>>>22^c<<30^c<<19^c<<10))|0;w7=((w8>>>7^w8>>>18^w8>>>3^w8<<25^w8<<14)+(w5>>>17^w5>>>19^w5>>>10^w5<<15^w5<<13)+w7+w0)|0;a=(w7+a+(f>>>6^f>>>11^f>>>25^f<<26^f<<21^f<<7)+(h^f&(g^h))+0x682e6ff3)|0;e=(e+a)|0;a=(a+((b&c)^(d&(b^c)))+(b>>>2^b>>>13^b>>>22^b<<30^b<<19^b<<10))|0;w8=((w9>>>7^w9>>>18^w9>>>3^w9<<25^w9<<14)+(w6>>>17^w6>>>19^w6>>>10^w6<<15^w6<<13)+w8+w1)|0;h=(w8+h+(e>>>6^e>>>11^e>>>25^e<<26^e<<21^e<<7)+(g^e&(f^g))+0x748f82ee)|0;d=(d+h)|0;h=(h+((a&b)^(c&(a^b)))+(a>>>2^a>>>13^a>>>22^a<<30^a<<19^a<<10))|0;w9=((w10>>>7^w10>>>18^w10>>>3^w10<<25^w10<<14)+(w7>>>17^w7>>>19^w7>>>10^w7<<15^w7<<13)+w9+w2)|0;g=(w9+g+(d>>>6^d>>>11^d>>>25^d<<26^d<<21^d<<7)+(f^d&(e^f))+0x78a5636f)|0;c=(c+g)|0;g=(g+((h&a)^(b&(h^a)))+(h>>>2^h>>>13^h>>>22^h<<30^h<<19^h<<10))|0;w10=((w11>>>7^w11>>>18^w11>>>3^w11<<25^w11<<14)+(w8>>>17^w8>>>19^w8>>>10^w8<<15^w8<<13)+w10+w3)|0;f=(w10+f+(c>>>6^c>>>11^c>>>25^c<<26^c<<21^c<<7)+(e^c&(d^e))+0x84c87814)|0;b=(b+f)|0;f=(f+((g&h)^(a&(g^h)))+(g>>>2^g>>>13^g>>>22^g<<30^g<<19^g<<10))|0;w11=((w12>>>7^w12>>>18^w12>>>3^w12<<25^w12<<14)+(w9>>>17^w9>>>19^w9>>>10^w9<<15^w9<<13)+w11+w4)|0;e=(w11+e+(b>>>6^b>>>11^b>>>25^b<<26^b<<21^b<<7)+(d^b&(c^d))+0x8cc70208)|0;a=(a+e)|0;e=(e+((f&g)^(h&(f^g)))+(f>>>2^f>>>13^f>>>22^f<<30^f<<19^f<<10))|0;w12=((w13>>>7^w13>>>18^w13>>>3^w13<<25^w13<<14)+(w10>>>17^w10>>>19^w10>>>10^w10<<15^w10<<13)+w12+w5)|0;d=(w12+d+(a>>>6^a>>>11^a>>>25^a<<26^a<<21^a<<7)+(c^a&(b^c))+0x90befffa)|0;h=(h+d)|0;d=(d+((e&f)^(g&(e^f)))+(e>>>2^e>>>13^e>>>22^e<<30^e<<19^e<<10))|0;w13=((w14>>>7^w14>>>18^w14>>>3^w14<<25^w14<<14)+(w11>>>17^w11>>>19^w11>>>10^w11<<15^w11<<13)+w13+w6)|0;c=(w13+c+(h>>>6^h>>>11^h>>>25^h<<26^h<<21^h<<7)+(b^h&(a^b))+0xa4506ceb)|0;g=(g+c)|0;c=(c+((d&e)^(f&(d^e)))+(d>>>2^d>>>13^d>>>22^d<<30^d<<19^d<<10))|0;w14=((w15>>>7^w15>>>18^w15>>>3^w15<<25^w15<<14)+(w12>>>17^w12>>>19^w12>>>10^w12<<15^w12<<13)+w14+w7)|0;b=(w14+b+(g>>>6^g>>>11^g>>>25^g<<26^g<<21^g<<7)+(a^g&(h^a))+0xbef9a3f7)|0;f=(f+b)|0;b=(b+((c&d)^(e&(c^d)))+(c>>>2^c>>>13^c>>>22^c<<30^c<<19^c<<10))|0;w15=((w0>>>7^w0>>>18^w0>>>3^w0<<25^w0<<14)+(w13>>>17^w13>>>19^w13>>>10^w13<<15^w13<<13)+w15+w8)|0;a=(w15+a+(f>>>6^f>>>11^f>>>25^f<<26^f<<21^f<<7)+(h^f&(g^h))+0xc67178f2)|0;e=(e+a)|0;a=(a+((b&c)^(d&(b^c)))+(b>>>2^b>>>13^b>>>22^b<<30^b<<19^b<<10))|0;A=(A+a)|0;B=(B+b)|0;C=(C+c)|0;D=(D+d)|0;E=(E+e)|0;F=(F+f)|0;G=(G+g)|0;H=(H+h)|0}function reset(){A=0x6a09e667;B=0xbb67ae85;C=0x3c6ef372;D=0xa54ff53a;E=0x510e527f;F=0x9b05688c;G=0x1f83d9ab;H=0x5be0cd19;I=J=0}function update(j,l){j=j|0;l=l|0;var h=0;if(j&63)return-1;while((l|0)>=64){z(j);j=(j+64)|0;l=(l-64)|0;h=(h+64)|0}I=(I+h)|0;if(I>>>0<h>>>0)J=(J+1)|0;return h|0}function digest(j,l,o){j=j|0;l=l|0;o=o|0;var h=0,i=0;if(j&63)return-1;if(~o)if(o&31)return-1;if((l|0)>=64){h=update(j,l)|0;if((h|0)==-1)return-1;j=(j+h)|0;l=(l-h)|0}h=(h+l)|0;I=(I+l)|0;if(I>>>0<l>>>0)J=(J+1)|0;K[j|l]=128;if((l|0)>=56){for(i=(l+1)|0;(i|0)<64;i=(i+1)|0)K[j|i]=0;z(j);l=0;K[j|0]=0}for(i=(l+1)|0;(i|0)<59;i=(i+1)|0)K[j|i]=0;K[j|56]=J>>>21&255;K[j|57]=J>>>13&255;K[j|58]=J>>>5&255;K[j|59]=J<<3&255|I>>>29;K[j|60]=I>>>21&255;K[j|61]=I>>>13&255;K[j|62]=I>>>5&255;K[j|63]=I<<3&255;z(j);if(~o){o=o|0;K[o|0]=A>>>24;K[o|1]=A>>>16&255;K[o|2]=A>>>8&255;K[o|3]=A&255;K[o|4]=B>>>24;K[o|5]=B>>>16&255;K[o|6]=B>>>8&255;K[o|7]=B&255;K[o|8]=C>>>24;K[o|9]=C>>>16&255;K[o|10]=C>>>8&255;K[o|11]=C&255;K[o|12]=D>>>24;K[o|13]=D>>>16&255;K[o|14]=D>>>8&255;K[o|15]=D&255;K[o|16]=E>>>24;K[o|17]=E>>>16&255;K[o|18]=E>>>8&255;K[o|19]=E&255;K[o|20]=F>>>24;K[o|21]=F>>>16&255;K[o|22]=F>>>8&255;K[o|23]=F&255;K[o|24]=G>>>24;K[o|25]=G>>>16&255;K[o|26]=G>>>8&255;K[o|27]=G&255;K[o|28]=H>>>24;K[o|29]=H>>>16&255;K[o|30]=H>>>8&255;K[o|31]=H&255}return h|0}return{reset:reset,update:update,digest:digest}",
                "},", ca.toString(), ")"
            ], {
                type: "application/javascript"
            })),
            b = new Worker(a);
        URL.revokeObjectURL(a);
        let c = 0,
            f = {};
        b.onmessage = e => {
            let [h, n] = e.data;
            f[h] && (f[h](n), delete f[h])
        };
        return {
            cd(e) {
                return new Promise(h => {
                    let n = c++;
                    f[n] = h;
                    b.postMessage([n, e])
                })
            }
        }
    })();
    const d = {
        K: {
            KEY_OVERRIDE: "R",
            KEY_LEVEL_UP: "N",
            KEY_ABILITY: "F",
            KEY_PAUSE: "G",
            KEY_CHOOSE_1: "Y",
            KEY_CHOOSE_2: "U",
            KEY_CHOOSE_3: "I",
            KEY_CHOOSE_4: "H",
            KEY_CHOOSE_5: "J",
            KEY_CHOOSE_6: "K",
            KEY_SPECIAL: "`"
        },
        KEY_AUTO_FIRE: 69,
        KEY_AUTO_SPIN: 67,
        KEY_OVERRIDE: 82,
        KEY_LEVEL_UP: 78,
        KEY_ABILITY: 70,
        KEY_PAUSE: 71,
        KEY_REVERSE_TANK: 86,
        KEY_REVERSE_MOUSE: 66,
        KEY_SCREENSHOT: 81,
        KEY_UPGRADE_MAX: 77,
        KEY_CLASS_TREE: 84,
        KEY_RECORD: 90,
        KEY_UP: 87,
        KEY_PING: 76,
        KEY_LEFT: 65,
        KEY_DOWN: 83,
        KEY_RIGHT: 68,
        KEY_CHOOSE_1: 89,
        KEY_CHOOSE_2: 85,
        KEY_CHOOSE_3: 73,
        KEY_CHOOSE_4: 72,
        KEY_CHOOSE_5: 74,
        KEY_CHOOSE_6: 75,
        KEY_CHOOSE_7: -1,
        KEY_CHOOSE_8: -1,
        KEY_CHOOSE_9: -1,
        KEY_ENTER: 13,
        KEY_SPAWN: 13,
        KEY_LEFT_ARROW: 37,
        KEY_UP_ARROW: 38,
        KEY_RIGHT_ARROW: 39,
        KEY_DOWN_ARROW: 40,
        KEY_UPGRADE_ATK: 49,
        KEY_UPGRADE_HLT: 50,
        KEY_UPGRADE_SPD: 51,
        KEY_UPGRADE_STR: 52,
        KEY_UPGRADE_PEN: 53,
        KEY_UPGRADE_DAM: 54,
        KEY_UPGRADE_RLD: 55,
        KEY_UPGRADE_MOB: 56,
        KEY_UPGRADE_RGN: 57,
        KEY_UPGRADE_SHI: 48,
        KEY_MOUSE_0: 32,
        KEY_MOUSE_1: 9,
        KEY_MOUSE_2: 16,
        KEY_SPECIAL: 192,
        KEY_SPECIAL_ALT: 223,
        KEY_SUICIDE: 79,
        i: 1920,
        j: 1080,
        ya: !1,
        la: !1,
        N: !1,
        cb: 0,
        Ub: !1,
        Sa: [{
                z: ["Private", null],
                x: ["Local", null],
                glitch: ["Glitch", 10],
                buyvm: ["BuyVM", 15],
                extravm: ["ExtraVM", 40],
                hetzner: ["Hetzner", 50],
                ovh: ["OVH", 45],
                wsi: ["WSI", 50],
                vultr: ["Vultr", 30]
            }, {
                xyz: ["Local", "Localhost", null],
                unk: ["Unknown", "Unknown", null],
                svx: ["US West", "Silicon Valley, CA, US", -7],
                lax: ["US West", "Los Angeles, CA, US", -7],
                dal: ["USA", "Dallas, TX, US", -5],
                kci: ["USA", "Kansas City, MO, US", -5],
                vin: ["US East", "Vint Hill, VA, US", -4],
                mtl: ["US East", "Montreal, CA", -4],
                lon: ["Europe", "London, UK",
                    1
                ],
                fra: ["Europe", "Frankfurt, DE", 2],
                fsn: ["Europe", "Falkenstein, DE", 2],
                sgp: ["Asia", "Singapore", 8]
            },
            [
                [{
                    id: "x",
                    I: "Private"
                }],
                [{
                    id: "e",
                    Ib: "word"
                }],
                [{
                    id: "w",
                    Ib: "words"
                }],
                [{
                    id: "p",
                    I: "Portal"
                }],
                [{
                    id: "o",
                    I: "Open"
                }],
                [{
                    id: "m",
                    I: "Maze",
                    delay: !0,
                    remove: "f"
                }],
                [{
                    id: "f",
                    I: "FFA"
                }, {
                    id: "d",
                    I: "Duos"
                }, {
                    id: "s",
                    I: "Squads"
                }, {
                    id: "1",
                    I: "1 Team",
                    advance: !0
                }, {
                    id: "2",
                    I: "2 Team",
                    advance: !0,
                    end: "2TDM"
                }, {
                    id: "3",
                    I: "3 Team",
                    advance: !0,
                    end: "3TDM"
                }, {
                    id: "4",
                    I: "4 Team",
                    advance: !0,
                    end: "4TDM"
                }],
                [{
                    id: "d",
                    I: "Domination"
                }, {
                    id: "m",
                    I: "Mothership",
                    remove: "2"
                }, {
                    id: "a",
                    I: "Assault",
                    remove: ["2", "m"]
                }, {
                    id: "s",
                    I: "Siege",
                    remove: "1"
                }, {
                    id: "t",
                    I: "Tag",
                    remove: ["o", "4"]
                }, {
                    id: "p",
                    I: "Pandemic",
                    remove: ["o", "2"]
                }, {
                    id: "z",
                    I: "Sandbox"
                }]
            ]
        ],
        pa: (new Date).getTimezoneOffset() / -60,
        ua: [{
            visible: 1,
            id: "c",
            code: "extravm-dal-x",
            host: "ext.arras.cx:5000"
        }, {
            visible: 2,
            id: "x",
            code: "x-xyz-x",
            host: "localhost:5000"
        }, {
            visible: 3,
            id: "sb",
            code: "extravm-dal-2",
            host: "ext.arras.cx:5001",
            qb: !0
        }, {
            visible: 3,
            id: "sc",
            code: "extravm-dal-f",
            host: "ext.arras.cx:5002",
            qb: !0
        }],
        D: null,
        pb: 0,
        md: null,
        mobile: "ontouchstart" in document.body && /android|mobi/i.test(navigator.userAgent)
    };
    let oa = !1;
    window.Arras = a => true /*Patched*/ /*"string" === typeof a && (oa = oa || "X8KDa199MTItU+gyqYWU+GIExelEY1yYwZU0xzRF104=" === ia(a, "latin1"))*/ ? d : {
        get graphical() {
            let b = d.Eb.o;
            return {
                get screenshotMode() {
                    return b.Oa
                },
                set screenshotMode(c) {
                    b.Oa = c
                },
                get borderChunk() {
                    return b.Ra
                },
                set borderChunk(c) {
                    b.Ra = c
                },
                get compensationScale() {
                    return b.Db
                },
                set compensationScale(c) {
                    b.Db = c
                },
                get lowGraphics() {
                    return b.Za
                },
                set lowGraphics(c) {
                    b.Za = c
                },
                get alphaAnimations() {
                    return b.gb
                },
                set alphaAnimations(c) {
                    b.gb = c
                },
                get inversedRender() {
                    return b.Wa
                },
                set inversedRender(c) {
                    b.Wa =
                        c
                },
                get miterStars() {
                    return b.ab
                },
                set miterStars(c) {
                    b.ab = c
                },
                get miter() {
                    return b.$a
                },
                set miter(c) {
                    b.$a = c
                },
                get fontSizeOffset() {
                    return b.Ua
                },
                set fontSizeOffset(c) {
                    b.Ua = c
                },
                get shieldbars() {
                    return b.vb
                },
                set shieldbars(c) {
                    b.vb = c
                },
                get renderGrid() {
                    return b.sb
                },
                set renderGrid(c) {
                    b.sb = c
                },
                get renderNames() {
                    return b.Tb
                },
                set renderNames(c) {
                    b.Tb = c
                },
                get censorNames() {
                    return b.jb
                },
                set censorNames(c) {
                    b.jb = c
                },
                get darkBorders() {
                    return b.xa
                },
                set darkBorders(c) {
                    b.xa = c
                },
                get neon() {
                    return b.Ca
                },
                set neon(c) {
                    b.Ca = c
                },
                get alternateBorder() {
                    return b.Qa
                },
                set alternateBorder(c) {
                    b.Qa = c
                },
                get coloredNest() {
                    return b.Cb
                },
                set coloredNest(c) {
                    b.Cb = c
                }
            }
        },
        get gui() {
            let b = d.Eb.O;
            return {
                get enabled() {
                    return b.enabled
                },
                set enabled(c) {
                    b.enabled = c
                },
                get scale() {
                    return b.scale
                },
                set scale(c) {
                    b.scale = c
                },
                get alcoveSize() {
                    return b.zb
                },
                set alcoveSize(c) {
                    b.zb = c
                },
                get spacing() {
                    return b.Wb
                },
                set spacing(c) {
                    b.Wb = c
                },
                get leaderboard() {
                    return b.Lb
                },
                set leaderboard(c) {
                    b.Lb = c
                },
                get barChunk() {
                    return b.va
                },
                set barChunk(c) {
                    b.va = c
                }
            }
        },
        get themeColor() {
            return d.gd
        },
        get debug() {
            if (oa) {
                var b =
                    d.Eb.debug;
                return {
                    get showId() {
                        return b.wb
                    },
                    set showId(c) {
                        b.wb = c
                    }
                }
            }
        }
    };
    let pa = window.localStorage || {};
    const ya = a => {
            let b = document.getElementById(a);
            "text" === b.type || "select-one" === b.type ? pa.setItem(a, b.value || "") : ("checkbox" === b.type || "radio" === b.type) && pa.setItem(a, b.checked || "false");
            pa.removeItem(`${a}Value`);
            pa.removeItem(`${a}Checked`)
        },
        za = (a, b = !1) => {
            let c = document.getElementById(a);
            if ("text" === c.type || "select-one" === c.type) c.value = pa.getItem(a) || "";
            else if ("checkbox" === c.type || "radio" === c.type) c.checked = "true" === (pa.getItem(a) || b.toString())
        },
        Aa = a => {
            if (0 === a.length) return "";
            if (1 === a.length) return a[0];
            let b = `${"and"} ${a[a.length-1]}`;
            return [...a.slice(0, -1), b].join(2 === a.length ? " " : ", ")
        },
        Ca = () => {
            var a = Math.round(d.A.time);
            a = Math.floor(a);
            if (0 >= a) return "less than a second";
            let b = [];
            for (let [c, f] of [
                    [60, "second"],
                    [60, "minute"],
                    [24, "hour"],
                    [null, "day"]
                ]) {
                let e = c ? a % c : a;
                a = c ? Math.floor(a / c) : 0;
                b.unshift(`${e} ${f}${1===e?"":"s"}`);
                if (0 === a) break
            }
            return Aa(b)
        },
        Da = (a, b = !1) => b && 0 >= a ? "" : 1E3 > a ? a.toFixed(0) : 1E6 > a ? (a / 1E3).toFixed(2) + "k" : 1E9 > a ? (a / 1E6).toFixed(2) + "m" : 1E12 > a ? (a / 1E9).toFixed(2) + "b" : 1E15 > a ? (a / 1E12).toFixed(2) +
        "t" : 1E18 > a ? (a / 1E15).toFixed(2) + "q" : 0 < a ? "\u221e" : "\u2014",
        Ea = a => 1E18 > a ? a.toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ",") : 0 < a ? "\u221e" : "\u2014";

    function Fa() {
        return Math.max(0, Math.min(Math.floor(Date.now() / 2E3) - 757382400, 268435455))
    }

    function Ia(a) {
        a.startTime = Fa();
        a.H = {
            Vc: 0,
            disconnect: 0
        };
        a.h = {
            status: !0,
            Ia: [0, 0, 0, 0, 0, 0, 0]
        };
        a.u = {
            status: !!navigator.onLine,
            Ia: [0, 0, 0, 0]
        };
        a.Y = !1;
        a.R = !1;
        let b = new Image;
        Object.defineProperty(b, "id", {
            get: () => {
                a.R = !0;
                return ""
            }
        });
        let c = () => {};
        c.toString = () => {
            a.R = !0;
            return "color:rgba(0,0,0,0)"
        };
        setTimeout(console.log.bind(null, "%c%s", c, b))
    }

    function Ja(a) {
        var b = Ka;
        b.H[a] = b.time()
    }

    function La(a) {
        var b = Ka;
        b.h.status !== a && (b.h.status = a, b.h.Ia.shift(), b.h.Ia.push(b.time()))
    }

    function Ma(a) {
        var b = Ka,
            c = k;
        b = [b.h.status | b.u.status << 1 | b.Y << 2 | b.R << 3 | (b.startTime & 4095) << 4, b.startTime >>> 12, b.H.Vc, b.H.disconnect, ...b.h.Ia, ...b.u.Ia, b.time()];
        var f = b.reduce((r, z) => r ^ z, 0);
        b = [57005, f, ...b, f ^ 65535, 48879];
        var e = parseInt(a.slice(1), 16);
        a = e >> 16 & 224;
        f = e >> 8 & 224;
        e &= 192;
        let h = Math.floor(c.canvas.width / 2) - 10,
            n = Math.floor(c.canvas.height / 3);
        for (let r = 0; 20 > r; r++) {
            let z = b[r];
            c.fillStyle = `rgb(${a|(z&63488)>>>11},${f|(z&1984)>>>6},${e|z&63})`;
            c.fillRect(h + r, n, 1, 1)
        }
    }
    var Ka = new class {
        constructor() {
            this.startTime = 0;
            this.u = this.h = this.H = null;
            this.R = this.Y = !1;
            Ia(this);
            setInterval(() => {
                var a = !!navigator.onLine;
                this.u.status !== a && (this.u.status = a, this.u.Ia.shift(), this.u.Ia.push(this.time()))
            }, 2E3)
        }
        time() {
            return Math.max(0, Math.min(Fa() - this.startTime, 65535))
        }
    };
    let Na = a => {
        d.bd.push({
            text: a,
            time: Date.now(),
            duration: 1E4
        })
    };

    function Oa(a) {
        let b = navigator.getGamepads();
        if (-1 !== a.Ba) {
            var c = b[a.Ba];
            if (c && "standard" === c.mapping && c.connected) return c;
            a.Ba = -1
        }
        for (c = 0; c < b.length; c++) {
            let f = b[c];
            if (f && "standard" === f.mapping && f.connected) return a.Ba = c, f
        }
        return null
    }

    function Sa(a, b, c) {
        a.set(0, -.3826834323650898 > c);
        a.set(1, .3826834323650898 < c);
        a.set(2, -.3826834323650898 > b);
        a.set(3, .3826834323650898 < b)
    }

    function Ta(a) {
        if (!a.bc) {
            a.bc = !0;
            var b = !1,
                c = !1,
                f = !1,
                e = !1,
                h = !1;
            setInterval(() => {
                let n = Oa(a);
                h !== !!n && (h = !!n, Na(h ? "Controller connected." : "Controller disconnected."));
                if (n) {
                    var r = A => A < n.axes.length ? n.axes[A] : 0,
                        z = A => A < n.buttons.length && n.buttons[A].pressed;
                    Sa(a, r(0), r(1));
                    var x = r(2);
                    r = r(3);
                    if (.01 < x * x + r * r) {
                        let A = .6 * Math.max(a.h.width, a.h.height);
                        a.setPosition(x * A, r * A)
                    }
                    a.set(4, z(5) || z(7));
                    a.set(6, z(4) || z(6));
                    x = z(1);
                    b !== x && (x && a.l("t", 2), b = x);
                    x = z(2);
                    c !== x && (x && a.v.l("0"), c = x);
                    x = z(10);
                    f !== x && (x && a.l("t",
                        1), f = x);
                    z = z(11);
                    e !== z && (z && a.l("t", 0), e = z)
                }
            }, 10)
        }
    }

    function Ua(a, b) {
        b.preventDefault();
        for (let f of b.changedTouches) {
            var c = f.identifier;
            b = Wa(f);
            if (a.Y === c) {
                c = b.x - 1 * a.h.width / 6;
                b = b.y - 2 * a.h.height / 3;
                let e = Math.sqrt(c * c + b * b);
                c /= e;
                b /= e;
                Sa(a, c, b)
            } else a.H === c && (c = 4 * (b.x - 5 * a.h.width / 6), b = 4 * (b.y - 2 * a.h.height / 3), 2 !== d.ga ? a.setPosition(c, b) : c > -a.h.width / 2 && c < a.h.width / 2 && b > -a.h.height / 2 && b < a.h.height / 2 ? (a.setPosition(c, b), a.set(6, !1)) : (a.setPosition(-c, -b), a.set(6, !0)))
        }
    }

    function Xa(a, b) {
        b.preventDefault();
        for (let c of b.changedTouches) b = c.identifier, a.R === b ? (a.R = null, 3 === d.ga && a.set(6, !1)) : a.Y === b ? (a.Y = null, Sa(a, 0, 0)) : a.H === b && (a.H = null, a.set(4, !1), 1 !== d.ga && 2 !== d.ga && 4 !== d.ga || a.set(6, !1))
    }

    function Ya(a) {
        if (d.Qc) var b = 60,
            c = setInterval(() => {
                a.v.l("L");
                0 >= --b && clearInterval(c)
            }, 100)
    }

    function Za() {
        var a = bb,
            b = d.mobile ? document.getElementById("optMobile").value : "desktop",
            c = d.v;
        a.Zb = b;
        a.v = c;
        c = a.h;
        "joysticks" === b ? (c.addEventListener("touchstart", f => {
            var e;
            if (e = f.isTrusted) {
                f.preventDefault();
                if (d.N && d.tb <= Date.now()) a.xb();
                else {
                    for (let n of f.changedTouches) {
                        e = n.identifier;
                        var h = Wa(n);
                        let r = cb(a, h);
                        null === a.R && 2 === r && (a.R = e, 3 === d.ga && a.set(6, !0));
                        r || (h = h.x < a.h.width / 2, null === a.Y && h ? a.Y = e : null !== a.H || h || (a.H = e, e = 500 >= Date.now() - a.ub, a.ub = Date.now(), a.set(4, !0), a.set(6, [!1, !0, !1, null !==
                            a.R, e
                        ][d.ga])))
                    }
                    Ua(a, f)
                }
                e = void 0
            }
            return e
        }, !1), c.addEventListener("touchmove", f => f.isTrusted && Ua(a, f), !1), c.addEventListener("touchend", f => f.isTrusted && Xa(a, f), !1), c.addEventListener("touchcancel", f => f.isTrusted && Xa(a, f), !1)) : (c.addEventListener("mousedown", f => {
            var e;
            if (e = f.isTrusted) {
                switch (f.button) {
                    case 0:
                        cb(a, Wa(f)) || a.set(4, !0);
                        break;
                    case 1:
                        a.set(5, !0);
                        break;
                    case 2:
                        a.set(6, !0)
                }
                e = void 0
            }
            return e
        }, !1), c.addEventListener("mousemove", f => {
            var e;
            if (e = f.isTrusted) f = Wa(f), null !== d.player.x && a.setPosition(f.x -
                d.player.x, f.y - d.player.y), d.Ic = 0 === d.G.oc.check(f), e = void 0;
            return e
        }, !1), c.addEventListener("mouseup", f => {
            var e;
            if (e = f.isTrusted) {
                switch (f.button) {
                    case 0:
                        a.set(4, !1);
                        break;
                    case 1:
                        a.set(5, !1);
                        break;
                    case 2:
                        a.set(6, !1)
                }
                e = void 0
            }
            return e
        }, !1));
        window.addEventListener("keydown", f => {
            var e;
            if (e = f.isTrusted) {
                if (a.ob) f.preventDefault(), a.l("0", f.keyCode);
                else {
                    switch (f.keyCode) {
                        case d.KEY_SPAWN:
                            d.N && d.tb <= Date.now() && a.xb();
                            break;
                        case d.KEY_UP_ARROW:
                        case d.KEY_UP:
                            a.set(0, !0);
                            break;
                        case d.KEY_DOWN_ARROW:
                        case d.KEY_DOWN:
                            a.set(1,
                                !0);
                            break;
                        case d.KEY_LEFT_ARROW:
                        case d.KEY_LEFT:
                            a.set(2, !0);
                            break;
                        case d.KEY_RIGHT_ARROW:
                        case d.KEY_RIGHT:
                            a.set(3, !0);
                            break;
                        case d.KEY_MOUSE_0:
                            a.set(4, !0);
                            break;
                        case d.KEY_MOUSE_1:
                            a.set(5, !0);
                            break;
                        case d.KEY_MOUSE_2:
                            a.set(6, !0);
                            break;
                        case d.KEY_LEVEL_UP:
                            a.v.l("L");
                            break;
                        case d.KEY_ABILITY:
                            a.v.l("A");
                            break;
                        case d.KEY_PAUSE:
                            a.v.l("D")
                    }
                    if (!f.ctrlKey && !f.altKey) {
                        if (d.na) {
                            e = -1;
                            switch (f.keyCode) {
                                case d.KEY_UPGRADE_ATK:
                                    e = 0;
                                    break;
                                case d.KEY_UPGRADE_HLT:
                                    e = 1;
                                    break;
                                case d.KEY_UPGRADE_SPD:
                                    e = 2;
                                    break;
                                case d.KEY_UPGRADE_STR:
                                    e =
                                        3;
                                    break;
                                case d.KEY_UPGRADE_PEN:
                                    e = 4;
                                    break;
                                case d.KEY_UPGRADE_DAM:
                                    e = 5;
                                    break;
                                case d.KEY_UPGRADE_RLD:
                                    e = 6;
                                    break;
                                case d.KEY_UPGRADE_MOB:
                                    e = 7;
                                    break;
                                case d.KEY_UPGRADE_RGN:
                                    e = 8;
                                    break;
                                case d.KEY_UPGRADE_SHI:
                                    e = 9
                            }
                            if (-1 !== e) {
                                let h = a.ac ? 12 : 1;
                                for (let n = 0; n < h; n++) a.l("x", e)
                            }
                        }
                        if (!f.repeat) {
                            switch (f.keyCode) {
                                case d.KEY_AUTO_SPIN:
                                    a.l("t", 0);
                                    break;
                                case d.KEY_AUTO_FIRE:
                                    a.l("t", 1);
                                    break;
                                case d.KEY_OVERRIDE:
                                    a.l("t", 2);
                                    break;
                                case d.KEY_REVERSE_MOUSE:
                                    a.l("t", 3);
                                    break;
                                case d.KEY_REVERSE_TANK:
                                    a.l("t", 4);
                                    break;
                                case d.KEY_SPECIAL_ALT:
                                case d.KEY_SPECIAL:
                                    a.v.l("0");
                                    a.ob = !0;
                                    break;
                                case d.KEY_SUICIDE:
                                    a.v.l("K");
                                    break;
                                case d.KEY_UPGRADE_MAX:
                                    a.ac = !0;
                                    break;
                                case d.KEY_PING:
                                    d.cb = f.shiftKey ? 2 : 1;
                                    break;
                                case d.KEY_CLASS_TREE:
                                    d.Ub = !0;
                                    break;
                                case d.KEY_RECORD:
                                    a.capture("video");
                                    break;
                                case d.KEY_SCREENSHOT:
                                    a.capture("image")
                            }
                            if (d.qa) switch (f.keyCode) {
                                case d.KEY_CHOOSE_1:
                                    a.l("U", 0);
                                    break;
                                case d.KEY_CHOOSE_2:
                                    a.l("U", 1);
                                    break;
                                case d.KEY_CHOOSE_3:
                                    a.l("U", 2);
                                    break;
                                case d.KEY_CHOOSE_4:
                                    a.l("U", 3);
                                    break;
                                case d.KEY_CHOOSE_5:
                                    a.l("U", 4);
                                    break;
                                case d.KEY_CHOOSE_6:
                                    a.l("U", 5);
                                    break;
                                case d.KEY_CHOOSE_7:
                                    a.l("U",
                                        6);
                                    break;
                                case d.KEY_CHOOSE_8:
                                    a.l("U", 7);
                                    break;
                                case d.KEY_CHOOSE_9:
                                    a.l("U", 8)
                            }
                        }
                    }
                }
                e = void 0
            }
            return e
        }, !1);
        window.addEventListener("keyup", f => {
            var e;
            if (e = f.isTrusted) {
                switch (f.keyCode) {
                    case d.KEY_UP_ARROW:
                    case d.KEY_UP:
                        a.set(0, !1);
                        break;
                    case d.KEY_DOWN_ARROW:
                    case d.KEY_DOWN:
                        a.set(1, !1);
                        break;
                    case d.KEY_LEFT_ARROW:
                    case d.KEY_LEFT:
                        a.set(2, !1);
                        break;
                    case d.KEY_RIGHT_ARROW:
                    case d.KEY_RIGHT:
                        a.set(3, !1);
                        break;
                    case d.KEY_MOUSE_0:
                        a.set(4, !1);
                        break;
                    case d.KEY_MOUSE_1:
                        a.set(5, !1);
                        break;
                    case d.KEY_MOUSE_2:
                        a.set(6,
                            !1);
                        break;
                    case d.KEY_SPECIAL_ALT:
                    case d.KEY_SPECIAL:
                        a.ob = !1;
                        break;
                    case d.KEY_UPGRADE_MAX:
                        a.ac = !1;
                        break;
                    case d.KEY_PING:
                        2 !== d.cb && (d.cb = 0);
                        break;
                    case d.KEY_CLASS_TREE:
                        d.Ub = !1
                }
                a.ob && a.l("0", -f.keyCode);
                e = void 0
            }
            return e
        }, !1);
        Oa(a) ? Ta(a) : window.addEventListener("gamepadconnected", f => f.isTrusted && Ta(a));
        Ya(a)
    }

    function cb(a, b) {
        var c = d.G.stat.check(b);
        if (-1 !== c) return a.l("x", c), !0;
        if (-1 !== d.G.za.check(b)) return d.za = !0;
        if (-1 !== d.G.ia.check(b)) return a = document.createElement("input"), a.value = d.ia.value, document.body.appendChild(a), a.select(), document.execCommand("copy"), d.ia.ic = !0;
        c = d.G.upgrade.check(b);
        if (-1 !== c) return a.l("U", c), !0;
        b = d.G.Mb.check(b);
        if (-1 !== b) {
            switch (b) {
                case 0:
                    return 2;
                case 1:
                    d.Na = d.Na ? 0 : 1;
                    b = 400 >= Date.now() - a.ub;
                    a.ub = Date.now();
                    b && d.Na && (d.Na = 2);
                    break;
                case 2:
                    d.ga = ((d.ga || 0) + 1) % 5;
                    switch (d.ga) {
                        case 0:
                        case 2:
                        case 4:
                            null !==
                                a.H && a.set(6, !1);
                            break;
                        case 1:
                            null !== a.H && a.set(6, !0);
                            break;
                        case 3:
                            a.set(6, null !== a.R)
                    }
                    break;
                case 3:
                    a.l("t", 1);
                    break;
                case 4:
                    a.l("t", 4);
                    break;
                case 5:
                    a.l("t", 0);
                    break;
                case 6:
                    a.l("t", 2);
                    break;
                case 7:
                    a.v.l("A");
                    break;
                case 8:
                    a.v.l("0")
            }
            return !0
        }
        return !1
    }

    function Wa(a) {
        return {
            x: a.clientX * d.Ec,
            y: a.clientY * d.Ec
        }
    }
    const db = class {
        constructor() {
            this.target = {
                x: 0,
                y: 0
            };
            this.v = null;
            this.ob = this.ac = !1;
            this.Zb = "";
            this.R = this.Y = this.H = null;
            this.ub = 0;
            this.Ba = -1;
            this.bc = !1;
            this.u = null;
            this.h = document.getElementById("canvas")
        }
        l(a, b) {
            this.v.l(a, b)
        }
        xb() {
            this.v.xb();
            Ia(Ka);
            Ya(this)
        }
        set(a, b) {
            this.v.cmd.set(a, b)
        }
        setPosition(a, b) {
            this.target.x = a;
            this.target.y = b;
            this.v.cmd.setPosition(a, b)
        }
        capture(a) {
            let b = (f, e) => {
                let h = URL.createObjectURL(f),
                    n = document.createElement("a");
                n.style.display = "none";
                n.setAttribute("download", e);
                n.setAttribute("href",
                    h);
                document.body.appendChild(n);
                setTimeout(() => {
                    URL.revokeObjectURL(h);
                    n.remove()
                }, 100);
                n.click()
            };
            if ("image" === a) {
                var c = this.h.toDataURL();
                a = atob(c.split(",")[1]);
                c = c.split(",")[0].split(":")[1].split(";")[0];
                let f = new Uint8Array(a.length);
                for (let e = 0; e < a.length; e++) f[e] = a.charCodeAt(e);
                b(new Blob([f], {
                    type: c
                }), "screenshot.png")
            } else if ("video" === a)
                if (this.h.captureStream && window.MediaRecorder) {
                    if (!this.u) {
                        let f = [];
                        this.u = new MediaRecorder(this.h.captureStream(60));
                        this.u.addEventListener("dataavailable",
                            e => f.push(e.data));
                        this.u.addEventListener("stop", () => {
                            b(new Blob(f, {
                                type: this.u.mimeType
                            }), "video.webm");
                            f.length = 0
                        })
                    }
                    switch (this.u.state) {
                        case "inactive":
                            Na("Recorder started!");
                            this.u.start();
                            break;
                        case "recording":
                            Na("Recorder stopped! Saving file..."), this.u.stop()
                    }
                } else Na("Media recorder is unsupported in this browser!")
        }
    };
    let eb = () => {
            let a = window.aiptag = window.aiptag || {};
            a.cmd = a.cmd || [];
            a.cmd.display = a.cmd.display || [];
            a.cmd.player = a.cmd.player || [];
            a.cmp = a.cmp || {};
            a.cmp.show = !0;
            a.cmp.position = "bottom";
            return a
        },
        fb = () => {
            window.nitroAds || (window.nitroAds = {
                createAd() {
                    window.nitroAds.queue.push(["createAd", arguments])
                },
                queue: []
            });
            return window.nitroAds
        },
        gb = () => {
            window.optimize || (window.optimize = {
                queue: []
            });
            return window.optimize
        };
    window.dataLayer = window.dataLayer || [];

    function ib(a) {
        window.dataLayer.push(arguments)
    }
    ib("js", new Date);
    ib("config", "UA-120544149-1");
    let jb = (a, b) => {
        if (a = document.getElementById(a)) a.style.display = b
    };
    var kb = {
            F: "adinplay",
            Ha: !1,
            show(a = this.F) {
                this.Ha && a !== this.F && this.L();
                this.Ha = !0;
                this.F = a;
                "adinplay" === this.F ? eb().cmd.display.push(() => window.aipDisplayTag.display("arras-io_336x280")) : "nitropay" === this.F ? (jb("ad-spawn", "block"), fb().createAd("ad-spawn", {
                        refreshLimit: 10,
                        refreshTime: 30,
                        renderVisibleOnly: !1,
                        refreshVisibleOnly: !0,
                        sizes: [
                            [336, 280],
                            [300, 250]
                        ]
                    })) : "bsa" === this.F ? (jb("bsa-zone_1643828555140-9_123456", "block"), gb().queue.push(() => window.optimize.push("bsa-zone_1643828555140-9_123456"))) :
                    jb("referral-fallback", "block")
            },
            L() {
                this.Ha = !1;
                "adinplay" === this.F ? eb().cmd.display.push(() => window.aipDisplayTag.clear("arras-io_336x280")) : "nitropay" === this.F ? jb("ad-spawn", "none") : "bsa" === this.F ? jb("bsa-zone_1643828555140-9_123456", "none") : jb("referral-fallback", "none")
            }
        },
        lb = {
            F: "adinplay",
            Ha: !1,
            show(a = this.F) {
                this.Ha && a !== this.F && this.L();
                this.Ha = !0;
                this.F = a;
                jb("respawn-banner", "block");
                if ("adinplay" === this.F) eb().cmd.display.push(() => window.aipDisplayTag.display("arras-io_728x90"));
                else if ("nitropay" ===
                    this.F || "nitropay-mobile" === this.F) {
                    a = document.getElementById("ad-respawn-container");
                    let b = document.getElementById("ad-respawn");
                    b || (b = document.createElement("div"), b.id = "ad-respawn", a.appendChild(b));
                    a = [];
                    "nitropay-mobile" === this.F ? a.push([320, 50]) : (1440 <= window.innerWidth && 680 <= window.innerHeight && a.push([970, 250]), 1440 <= window.innerWidth && a.push([970, 90]), a.push([728, 90]), 700 <= window.innerHeight && a.push([336, 280]), 680 <= window.innerHeight && a.push([300, 250]));
                    fb().createAd("ad-respawn", {
                        refreshLimit: 10,
                        refreshTime: 30,
                        renderVisibleOnly: !1,
                        refreshVisibleOnly: !0,
                        sizes: a
                    })
                } else "bsa" === this.F && (jb("bsa-zone_1643827950431-2_123456", "block"), gb().queue.push(() => window.optimize.push("bsa-zone_1643827950431-2_123456")))
            },
            L() {
                this.Ha = !1;
                if ("adinplay" === this.F) eb().cmd.display.push(() => window.aipDisplayTag.clear("arras-io_728x90"));
                else if ("nitropay" === this.F || "nitropay-mobile" === this.F) {
                    let a = document.getElementById("ad-respawn");
                    a && a.remove()
                } else "bsa" === this.F && jb("bsa-zone_1643827950431-2_123456", "none");
                jb("respawn-banner", "none")
            },
            kd() {
                let a = document.getElementById("respawn-banner");
                return a ? a.clientHeight : 0
            }
        };

    function mb() {
        return new Promise((a, b) => {
            window.grecaptcha.ready(() => {
                window.grecaptcha.execute("6LdwidEZAAAAAKGQ9ngDYVnClNn_aTAJcvg6cZUc", {
                    action: "spawn"
                }).then(a).catch(b)
            })
        })
    }
    var nb = {
        get yb() {
            return fb().loaded
        }
    };
    let ob = new Uint8Array(2097152),
        pb = new DataView(ob.buffer);
    var qb = {
        encode: a => {
            let b = [];
            for (var c of a) {
                var f = 0;
                if (0 === c || !1 === c) f = 0;
                else if (1 === c || !0 === c) f = 1;
                else if ("number" === typeof c) !Number.isInteger(c) || -4294967296 > c || 4294967296 <= c ? f = 8 : 0 <= c ? 256 > c ? f = 2 : 65536 > c ? f = 4 : 4294967296 > c && (f = 6) : -256 <= c ? f = 3 : -65536 <= c ? f = 5 : -4294967296 <= c && (f = 7);
                else if ("string" === typeof c) {
                    f = !1;
                    for (var e = 0; e < c.length; e++)
                        if ("\u00ff" < c.charAt(e)) f = !0;
                        else if ("\x00" === c.charAt(e)) throw console.error("Null containing string", c), Error("Null containing string");
                    f = !f && 1 >= c.length ? 9 : f ? 11 :
                        10
                } else throw console.error("Unencodable data type", c, a), Error("Unencodable data type");
                b.push(f)
            }
            b.push(15);
            let h = 0,
                n = -1;
            c = z => {
                -1 === n ? n = z : (pb.setUint8(h++, n << 4 | z), n = -1)
            };
            f = 15;
            e = 0;
            for (var r of b)
                if (r === f) e++;
                else {
                    for (c(f); 0 < e;) switch (e) {
                        case 0:
                            break;
                        case 1:
                            c(f);
                            e = 0;
                            break;
                        case 2:
                            c(12);
                            e = 0;
                            break;
                        case 3:
                            c(13);
                            e = 0;
                            break;
                        default:
                            c(14), 19 > e ? (c(e - 4), e = 0) : (c(15), e -= 19)
                    }
                    f = r
                } if (0 !== e || 15 !== f) throw Error("Internal error");
            c(15); - 1 !== n && c(15);
            for (r = 0; r < b.length; r++) switch (c = a[r], b[r]) {
                case 2:
                case 3:
                    pb.setUint8(h++,
                        c);
                    break;
                case 4:
                case 5:
                    pb.setUint16(h, c, !0);
                    h += 2;
                    break;
                case 6:
                case 7:
                    pb.setUint32(h, c, !0);
                    h += 4;
                    break;
                case 8:
                    pb.setFloat32(h, c, !0);
                    h += 4;
                    break;
                case 9:
                    c = 0 === c.length ? 0 : c.charCodeAt(0);
                    pb.setUint8(h++, c);
                    break;
                case 10:
                    for (f = 0; f < c.length; f++) pb.setUint8(h++, c.charCodeAt(f));
                    pb.setUint8(h++, 0);
                    break;
                case 11:
                    for (f = 0; f < c.length; f++) pb.setUint16(h, c.charCodeAt(f), !0), h += 2;
                    pb.setUint16(h, 0, !0);
                    h += 2
            }
            return ob.slice(0, h).buffer
        },
        decode: a => {
            var b = new Uint8Array(a);
            if (2097152 < b.length) return null;
            a = 2097152 - b.length;
            ob.set(b, a);
            if (15 !== pb.getUint8(a) >> 4) return null;
            b = [];
            for (var c = 15, f = !0;;) {
                if (2097152 <= a) return null;
                var e = pb.getUint8(a);
                f ? (e &= 15, a++) : e >>= 4;
                f = !f;
                if (12 === (e & 12)) {
                    if (15 === e) {
                        f && a++;
                        break
                    }
                    let h = e - 10;
                    if (14 === e) {
                        if (2097152 <= a) return null;
                        e = pb.getUint8(a);
                        f ? (e &= 15, a++) : e >>= 4;
                        f = !f;
                        h += e
                    }
                    for (e = 0; e < h; e++) b.push(c)
                } else b.push(e), c = e
            }
            c = [];
            for (let h of b) switch (h) {
                case 0:
                    c.push(0);
                    break;
                case 1:
                    c.push(1);
                    break;
                case 2:
                    if (2097152 <= a) return null;
                    c.push(pb.getUint8(a++));
                    break;
                case 3:
                    if (2097152 <= a) return null;
                    c.push(pb.getUint8(a++) - 256);
                    break;
                case 4:
                    if (2097152 <= a + 1) return null;
                    c.push(pb.getUint16(a, !0));
                    a += 2;
                    break;
                case 5:
                    if (2097152 <= a + 1) return null;
                    c.push(pb.getUint16(a, !0) - 65536);
                    a += 2;
                    break;
                case 6:
                    if (2097152 <= a + 3) return null;
                    c.push(pb.getUint32(a, !0));
                    a += 4;
                    break;
                case 7:
                    if (2097152 <= a + 3) return null;
                    c.push(pb.getUint32(a, !0) - 4294967296);
                    a += 4;
                    break;
                case 8:
                    if (2097152 <= a + 3) return null;
                    c.push(pb.getFloat32(a, !0) || 0);
                    a += 4;
                    break;
                case 9:
                    if (2097152 <= a) return null;
                    b = pb.getUint8(a++);
                    c.push(0 === b ? "" : String.fromCharCode(b));
                    break;
                case 10:
                    for (b = ""; 2097152 > a;) {
                        f = pb.getUint8(a++);
                        if (!f) break;
                        b += String.fromCharCode(f)
                    }
                    c.push(b);
                    break;
                case 11:
                    for (b = ""; 2097152 > a + 1;) {
                        f = pb.getUint16(a, !0);
                        a += 2;
                        if (!f) break;
                        b += String.fromCharCode(f)
                    }
                    c.push(b)
            }
            return c
        }
    };
    let rb = !/Chrome\/8[4-6]\.0\.41([4-7][0-9]|8[0-3])\./.test(navigator.userAgent) && window.requestAnimationFrame || (a => setTimeout(() => a(Date.now()), 1E3 / 60)),
        sb = Function.prototype.call.bind(WebSocket.prototype.send),
        tb = Function.prototype.call.bind(Function.prototype.toString),
        ub = a => new Promise(b => setTimeout(b, a)),
        xb = (() => {
            const a = atob("PT1Vc2VyU2NyaXB0PT0=");
            let b = [],
                c = [],
                f = A => {
                    if (null != b) {
                        var y = Error().stack;
                        !/user-?script|user\.js|multibox/i.test(y) || 16 <= b.length || (y = y.replace(/https:\/\/arras\.io\/bundle.js\?[0-9]*/g,
                            "<self>"), null != A && (y += `\n${A}`), b.includes(y) || b.push(y))
                    }
                },
                e = (A, y) => {
                    try {
                        let g = A[y];
                        A[y] = function(...l) {
                            try {
                                f(A[y].caller)
                            } catch (p) {
                                try {
                                    f(null)
                                } catch (v) {}
                            }
                            return g.apply(this, l)
                        };
                        c.push({
                            type: "method",
                            wc: A,
                            cc: y,
                            Ac: g,
                            kc: null
                        })
                    } catch (g) {}
                },
                h = (A, y) => {
                    try {
                        let l = Object.getOwnPropertyDescriptor(A, y);
                        if ("function" === typeof l.set) {
                            var g = l.set;
                            l.set = function(p) {
                                try {
                                    f(l.set.caller)
                                } catch (v) {
                                    try {
                                        f(null)
                                    } catch (t) {}
                                }
                                g.call(this, p)
                            };
                            Object.defineProperty(A, y, l);
                            c.push({
                                type: "setter",
                                wc: A,
                                cc: y,
                                Ac: g,
                                kc: l
                            })
                        }
                    } catch (l) {}
                };
            h(Node.prototype, "textContent");
            h(Element.prototype, "innerHTML");
            h(HTMLElement.prototype, "innerText");
            e(Element.prototype, "appendChild");
            e(Element.prototype, "insertBefore");
            e(Element.prototype, "replaceChild");
            e(Element.prototype, "insertAdjacentElement");
            for (let A of [Element, Document, DocumentFragment]) {
                let y = A.prototype;
                e(y, "append");
                e(y, "prepend");
                e(y, "replaceWith")
            }
            e(window, "fetch");
            e(window, "setTimeout");
            e(window, "setInterval");
            e(window, "requestAnimationFrame");
            e(WebSocket.prototype, "addEventListener");
            h(WebSocket.prototype, "onmessage");
            let n = 0,
                r = (A, y = !1) => {
                    if (!(1024 <= n)) try {
                        let g = tb(A);
                        !g.includes(a) && !y || b.includes(g) || (n++, b.push(g))
                    } catch (g) {}
                },
                z = Function.prototype.apply;
            Object.defineProperty(Function.prototype, "apply", {
                configurable: !0,
                get() {
                    r(this);
                    return z
                },
                set(A) {
                    z = A
                }
            });
            let x = Function.prototype.call;
            Object.defineProperty(Function.prototype, "call", {
                configurable: !0,
                get() {
                    r(this);
                    return x
                },
                set(A) {
                    x = A
                }
            });
            if (window["%arras"]) {
                let A = WebSocket.prototype.hookSend,
                    y = WebSocket.prototype.hookMsg;
                WebSocket.prototype.hookSend =
                    function(...g) {
                        f(WebSocket.prototype.hookSend.caller);
                        for (let l of g) r(l, !0);
                        return A.apply(this, g)
                    };
                WebSocket.prototype.hookMsg = function(...g) {
                    f(WebSocket.prototype.hookMsg.caller);
                    for (let l of g) r(l, !0);
                    return y.apply(this, g)
                }
            } else {
                let A = null;
                Object.defineProperty(window, "%arras", {
                    configurable: !0,
                    get() {
                        return A
                    },
                    set(y) {
                        A = y;
                        if (A instanceof Promise) {
                            let g = A.then;
                            A.then = function(...l) {
                                for (let p of l) r(p, !0);
                                return g.apply(this, l)
                            }
                        }
                    }
                })
            }
            return {
                lc() {
                    if (null == b) return null;
                    let A = b.join("\n\n"),
                        y = !0,
                        g =
                        Object.getOwnPropertyNames(Object.prototype);
                    for (let l of g) l.startsWith("__") || "toString toLocaleString valueOf hasOwnProperty isPrototypeOf propertyIsEnumerable constructor toSource watch unwatch".split(" ").includes(l) || (y && (y = !1, A += "" === A ? "keys =" : "\n\nkeys ="), A += " " + l);
                    return A.slice(0, 524288) || null
                },
                clear() {
                    b = []
                },
                Uc() {
                    if (null != b) {
                        b = null;
                        for (let {
                                type: A,
                                wc: y,
                                cc: g,
                                Ac: l,
                                kc: p
                            } of c) try {
                            "method" === A ? y[g] = l : "setter" === A && (p.set = l, Object.defineProperty(y, g, p))
                        } catch (v) {}
                        try {
                            Object.defineProperty(Function.prototype,
                                "apply", {
                                    configurable: !0,
                                    writable: !0,
                                    value: z
                                })
                        } catch (A) {}
                        try {
                            Object.defineProperty(Function.prototype, "call", {
                                configurable: !0,
                                writable: !0,
                                value: x
                            })
                        } catch (A) {}
                    }
                }
            }
        })();
    (() => {
        if (window === window.top && /(^|\.)(localhost|arras\.io|arrax\.io|arras\.netlify\.app)$/.test(location.hostname))
            if ("http:" === location.protocol || location.hostname.startsWith("www.")) location.href = `https://${location.hostname.replace(/^www\./,"")}/${"#"===location.hash?"":location.hash}`;
            else {
                var a = !1;
                window.addEventListener("error", b => {
                    if (!a) {
                        a = !0;
                        var {
                            message: c,
                            filename: f,
                            lineno: e,
                            colno: h,
                            error: n
                        } = b;
                        n && (n = n.toString());
                        if (/\bXDR encoding failure\b/.test(n)) console.warn(b.error);
                        else if (null != n ||
                            0 != e || 0 != h) b = JSON.stringify({
                            message: c,
                            filename: f,
                            lineno: e,
                            colno: h,
                            error: n
                        }), prompt("The game may have crashed, refresh the page to recover from error. If this is a private server, make sure to check the changelog for any server updates.\n\nError information:", b)
                    }
                })
            }
        else window.addEventListener("load", () => {
            document.write('<h1>You are on an embedded version of arras.io, the original game is at <a href="https://arras.io/">https://arras.io/</a></h1>')
        }), document.addEventListener("click", () => {
            window.top.location =
                "https://arras.io/"
        }), window.top.location = "https://arras.io/"
    })();
    if (d.mobile && window.innerHeight > 1.1 * window.innerWidth) {
        let a = document.getElementById("menuTabs");
        a.style.textAlign = "center";
        let b = document.createElement("span");
        b.classList.add("menuTab");
        b.classList.add("warning");
        b.appendChild(document.createTextNode("Please turn your device to landscape mode.\u00a0\u00a0\u00a0"));
        let c = document.createElement("a");
        c.style.textDecoration = "underline";
        c.href = "javascript:;";
        c.appendChild(document.createTextNode("Dismiss"));
        c.addEventListener("click", () => b.remove());
        b.appendChild(c);
        a.appendChild(b);
        window.addEventListener("orientationchange", () => {
            window.innerHeight > 1.1 * window.innerWidth || b.remove()
        })
    }
    let yb = async a => {
        let b = !1;
        a = JSON.stringify(a);
        let c = {
            ok: !1
        };
        try {
            c = await fetch({
                includes(f) {
                    b = !0;
                    return "https://analytics-server.arras.cx:2002/data".includes(f)
                },
                toString() {
                    return "https://analytics-server.arras.cx:2002/data"
                }
            }, {
                method: "POST",
                mode: "cors",
                cache: "no-cache",
                headers: {
                    "Content-Type": "application/json"
                },
                body: a
            }).then(f => f.json())
        } catch (f) {}
        if (!c.ok || b) try {
            let f = new XMLHttpRequest;
            f.open("POST", "https://analytics-server.arras.cx:2002/data");
            f.setRequestHeader("Content-Type", "application/json");
            f.send(a)
        } catch (f) {}
    }, zb = [];
    /(^|\.)(arras\.io|arrax\.io|arras\.netlify\.app)$/.test(location.hostname) && zb.push(window.loadedAdService || "adinplay");
    /(^|\.)(localhost|arras\.io)$/.test(location.hostname) && zb.push("nitropay");
    0 === zb.length && zb.push(window.loadedAdService || "adinplay");
    1 < zb.length && zb.push(...zb.splice(0, Math.floor(Math.random() * zb.length)));
    let Ab = 0,
        Bb = () => {
            Ab++;
            Ab %= zb.length;
            return zb[Ab]
        };
    d.mobile || kb.show(Bb());
    lb.show(d.mobile ? "nitropay-mobile" : Bb());
    let Cb = Date.now(),
        Db = 0,
        Eb = !1,
        Fb = null;
    fetch("/probe?detectBlock-prebid-ad0.0").then(a => a.json()).catch(() => ({
        ok: !1
    })).then(a => {
        Eb = !a.ok && !nb.yb;
        d.mobile || (Eb ? kb.show("fallback") : nb.yb || setTimeout(() => {
            nb.yb || kb.show("fallback")
        }, 5E3));
        yb({
            type: "initial",
            user: {
                adblock: Eb,
                mobile: d.mobile,
                window: {
                    innerWidth: window.innerWidth,
                    innerHeight: window.innerHeight
                },
                tracking: {
                    name: Gb.getItem("playerNameInput") || "",
                    colors: Gb.getItem("optColors") || "normal",
                    borders: Gb.getItem("optBorders") || "normal"
                }
            }
        })
    });
    const Hb = () => {
        let a = 0,
            b = 0;
        var c = e => h => (h ^ e(h)) & 65535;
        let f = [e => e || b, c(e => e << 7), c(e => e >> 9), c(e => e << 8)];
        c = e => {
            var h = qb.encode;
            if (!(e[0].charCodeAt(0) ^ 112)) {
                var n = e[0];
                b = e[1];
                for (let r of f) b = a = r(a);
                e = [n, b]
            }
            return h.call(qb, e)
        };
        c.toString = qb.encode.toString.bind(qb.encode);
        return {
            encode: c,
            decode: qb.decode
        }
    };
    let w = {
        o: {
            Oa: !1,
            Ra: 6,
            Db: 1.114,
            Za: !1,
            gb: !1,
            Wa: !0,
            ab: !0,
            $a: !1,
            Ua: 0,
            vb: !1,
            sb: !0,
            Tb: !0,
            jb: !1,
            xa: !1,
            Ca: !1,
            Qa: !1,
            Cb: !0
        },
        O: {
            enabled: !0,
            scale: 1,
            zb: 200,
            Wb: 20,
            Lb: !0,
            va: 5
        },
        Da: {
            vc: 9,
            bb: 1
        },
        mb: {
            memory: 16,
            delay: 50,
            offset: +location.hash.match(/^(?:#debug_lag_offset=(\d+))?/)[1] || -50
        },
        debug: {
            wb: 0
        }
    };
    d.Eb = w;
    let Ib = (a, b, c = .5) => {
            if (0 >= c) return a;
            if (1 <= c) return b;
            let f = 1 - c;
            a = parseInt(a.slice(1, 7), 16);
            b = parseInt(b.slice(1, 7), 16);
            return "#" + ((a & 16711680) * f + (b & 16711680) * c & 16711680 | (a & 65280) * f + (b & 65280) * c & 65280 | (a & 255) * f + (b & 255) * c & 255).toString(16).padStart(6, "0")
        },
        E = a => {
            if (a < Jb.table.length) return Jb.table[a];
            switch (a) {
                case 20:
                    return 150 > Date.now() % 300 ? E(10) : E(12);
                case 21:
                    return 150 > Date.now() % 300 ? E(10) : E(16);
                case 22:
                    return 150 > Date.now() % 300 ? E(16) : E(10);
                case 23:
                    return 150 > Date.now() % 300 ? E(12) : E(16);
                case 24:
                    return 150 >
                        Date.now() % 300 ? E(16) : E(12);
                case 30:
                    return "#a913cf";
                case 31:
                    return "#226ef6";
                case 32:
                    return "#ff1000";
                case 33:
                    return "#ff9000";
                case 34:
                    return "#00e00b";
                case 35:
                    return "#ffd300";
                case 36:
                    return Ib("#ff1000 #ff9000 #ffd300 #00e00b #226ef6 #a913cf".split(" ")[Math.floor(Date.now() / 200 % 6)], "#ff9000 #ffd300 #00e00b #226ef6 #a913cf #ff1000".split(" ")[Math.floor(Date.now() / 200 % 6)], Date.now() / 200 % 1);
                case 37:
                    return Ib("#ffffff", 2E3 > Date.now() % 4E3 ? "#55cdfc" : "#f7a8b8", 5 * Math.sin(Date.now() % 2E3 / 2E3 * Math.PI) - 2);
                case 39:
                    return "#654321";
                case 40:
                    return "#e58100";
                case 41:
                    return "#267524";
                default:
                    return "#00000000"
            }
        },
        Kb = a => {
            switch (a) {
                case "bas1":
                case "bap1":
                case "dom1":
                case "dbc1":
                case "mbc1":
                case "spw1":
                    return E(10);
                case "bas2":
                case "bap2":
                case "dom2":
                case "dbc2":
                case "mbc2":
                case "spw2":
                    return E(11);
                case "bas3":
                case "bap3":
                case "dom3":
                case "dbc3":
                case "mbc3":
                case "spw3":
                    return E(12);
                case "bas4":
                case "bap4":
                case "dom4":
                case "dbc4":
                case "mbc4":
                case "spw4":
                    return E(15);
                case "domx":
                case "dom0":
                case "dbc0":
                case "mbc0":
                case "spw0":
                    return E(3);
                case "port":
                    return k.globalAlpha = 1, E(9);
                case "por1":
                    return k.globalAlpha = 1, E(10);
                case "por2":
                    return k.globalAlpha = 1, E(11);
                case "por3":
                    return k.globalAlpha = 1, E(12);
                case "por4":
                    return k.globalAlpha = 1, E(15);
                case "edge":
                    return Ib(E(18), E(19), 1 / 3);
                case "dor1":
                    return E(6);
                case "nest":
                    if (w.o.Cb) return E(14);
                default:
                    return E(18)
            }
        },
        Lb = a => {
            let b = E(a % 9 + 10);
            9 <= a && (b = Ib(b, E((a + Math.floor(a / 9)) % 9 + 10)));
            return b
        },
        Mb = (a, b) => {
            let c = w.o.xa ? 1 : Jb.border;
            w.o.Ca ? (a.fillStyle = Ib(b, E(w.o.Qa ? 8 : 18), c), a.strokeStyle = b) : (a.fillStyle =
                b, a.strokeStyle = Ib(b, E(w.o.Qa ? 19 : 9), c))
        },
        Nb = null,
        Ob = a => ({
            index: a.index,
            name: a.name,
            x: a.x,
            y: a.y,
            color: a.color,
            shape: a.shape,
            size: a.size,
            facing: a.facing,
            Z: a.layer,
            dd: a.statnames,
            position: a.position && a.position.middle && {
                $: {
                    x: a.position.middle.x
                },
                axis: a.position.axis
            },
            Mc: a.upgrades && a.upgrades.map(b => ({
                Lc: b.tier,
                index: b.index
            })),
            C: a.guns.map(b => ({
                offset: b.offset,
                direction: b.direction,
                length: b.length,
                width: b.width,
                Pc: b.aspect,
                angle: b.angle
            })),
            B: a.turrets.map(b => ({
                index: b.index,
                Vb: b.sizeFactor,
                offset: b.offset,
                direction: b.direction,
                Z: b.layer,
                angle: b.angle
            }))
        }),
        Pb = (a, b = Nb[a].color) => {
            let c = Nb[a];
            return {
                index: a,
                x: c.x,
                y: c.y,
                facing: c.facing,
                size: c.size,
                color: b,
                g: {
                    status: {
                        sa() {
                            return 1
                        },
                        nc() {
                            return "#ffffff"
                        },
                        mc() {
                            return 0
                        }
                    }
                },
                shape: c.shape,
                name: c.name,
                s: 0,
                Z: c.Z,
                C: {
                    Va() {
                        return 0
                    },
                    get() {
                        return {
                            position: 0
                        }
                    },
                    update() {}
                },
                B: c.B.map(f => {
                    let e = Pb(f.index);
                    e.size = c.size * f.Vb;
                    e.facing = f.direction + f.angle;
                    return e
                })
            }
        };
    d.G = (() => {
        const a = class {
                constructor() {
                    this.h = this.w = this.y = this.x = 0;
                    this.active = !1
                }
                set(c, f, e, h) {
                    this.x = c;
                    this.y = f;
                    this.w = e;
                    this.h = h;
                    this.active = !0
                }
                check(c) {
                    let f = Math.round(c.x - this.x);
                    c = Math.round(c.y - this.y);
                    return this.active && 0 <= f && 0 <= c && f <= this.w && c <= this.h
                }
                L() {
                    this.active = !1
                }
            },
            b = class {
                constructor(c) {
                    this.data = [];
                    for (let f = 0; f < c; f++) this.data.push(new a)
                }
                Ea(c, ...f) {
                    for (; c >= this.data.length;) this.data.push(new a);
                    this.data[c].set(...f)
                }
                L() {
                    for (let c of this.data) c.L()
                }
                check(c) {
                    return this.data.findIndex(f =>
                        f.check(c))
                }
            };
        return {
            stat: new b(10),
            upgrade: new b(9),
            oc: new b(1),
            za: new b(1),
            ia: new b(1),
            Mb: new b(9)
        }
    })();
    d.Ic = !1;
    const Qb = class {
        constructor(a) {
            this.u = a;
            this.h = {}
        }
        update(a, b = 0) {
            var c = a[b++];
            if (-1 === c) this.h = {};
            else
                for (var f = 0; f < c; f++) delete this.h[a[b++]];
            c = a[b++];
            for (f = 0; f < c; f++) {
                let e = a[b++],
                    h = a.slice(b, b + this.u);
                b += this.u;
                this.h[e] = h
            }
            return b
        }
        entries() {
            return Object.entries(this.h).map(([a, b]) => ({
                id: +a,
                data: b
            }))
        }
    };

    function Rb(a) {
        let b = Nb[a.index];
        return {
            image: Pb(a.index, a.color),
            position: b.position,
            Rc: E(a.ib),
            name: a.name,
            label: b.name,
            s: a.s.get()
        }
    }
    const Tb = class {
            constructor() {
                this.name = "";
                this.ib = 11;
                this.color = 12;
                this.index = 0;
                this.s = new Sb(0, 1);
                this.S = !1
            }
            update(a) {
                this.name = a.name;
                this.ib = a.ib;
                this.color = a.color;
                this.index = a.index;
                this.s.set(a.s);
                this.S = !1
            }
        },
        Ub = class {
            constructor() {
                this.entries = {}
            }
            get() {
                let a = [],
                    b = 1;
                for (let c of Object.values(this.entries)) {
                    let f = Rb(c);
                    a.push(f);
                    f.s > b && (b = f.s)
                }
                a.sort((c, f) => f.s - c.s);
                return {
                    data: a,
                    max: b
                }
            }
            update(a) {
                a.sort((b, c) => c.s - b.s);
                for (let b of Object.values(this.entries)) b.S = !0;
                for (let b of a) this.entries[b.id] ||
                    (this.entries[b.id] = new Tb), this.entries[b.id].update(b);
                for (let [b, c] of Object.entries(this.entries)) c.S && delete this.entries[b]
            }
        },
        Sb = class {
            constructor(a, b, c = 3) {
                this.value = a;
                this.speed = b;
                this.h = c;
                this.time = Date.now();
                this.S = this.display = a
            }
            set(a) {
                this.value !== a && (this.S = this.get(), this.value = a, this.time = Date.now())
            }
            get() {
                let a = (Date.now() - this.time) / 1E3;
                return this.display = a >= this.speed ? this.value : this.S + (this.value - this.S) * Math.pow(a / this.speed, 1 / this.h)
            }
        };
    let Vb = [],
        cc = new Qb(5),
        dc = new Qb(3),
        ec = new Qb(5),
        fc = new class {
            constructor(a = 200) {
                this.speed = a;
                this.map = {};
                this.Ya = Date.now()
            }
            update(a) {
                this.Ya = Date.now();
                for (let [b, c] of Object.entries(this.map)) c.now ? (c.S = c.now, c.now = null) : delete this.map[b];
                for (let b of a) this.map[b.id] ? this.map[b.id].now = b : this.map[b.id] = {
                    S: null,
                    now: b
                }
            }
            get() {
                let a = Math.min(1, (Date.now() - this.Ya) / this.speed),
                    b = 1 - a;
                return Object.values(this.map).map(({
                    S: c,
                    now: f
                }) => f ? c ? {
                    type: f.type,
                    id: f.id,
                    x: a * f.x + b * c.x,
                    y: a * f.y + b * c.y,
                    color: f.color,
                    size: a * f.size + b * c.size,
                    alpha: 1
                } : {
                    type: f.type,
                    id: f.id,
                    x: f.x,
                    y: f.y,
                    color: f.color,
                    size: f.size,
                    alpha: a
                } : {
                    type: c.type,
                    id: c.id,
                    x: c.x,
                    y: c.y,
                    color: c.color,
                    size: c.size,
                    alpha: b
                })
            }
        },
        gc = new Ub,
        hc = 0,
        ic = d.bd = [];

    function jc() {
        for (var a = F.latency; a.W.length >= w.mb.memory;) a.W.shift();
        a.W.push(Date.now())
    }

    function kc() {
        var a = F.latency;
        return 1 >= a.W.length ? 0 : Math.max(0, a.W[a.W.length - 1] - a.W[a.W.length - 2] + w.mb.offset)
    }

    function lc() {
        var a = F.latency;
        return 1 >= a.W.length ? 0 : Math.max(0, (a.W[a.W.length - 1] - a.W[0]) / (a.W.length - 1) + w.mb.offset)
    }
    let F = d.ld = {
            latency: {
                W: []
            },
            v: {
                Fb: 0,
                $b: 0,
                Hb: 0
            },
            Ga: 0,
            Pa: 0,
            oa: 0,
            Fa: 0,
            current: {
                Ga: 0,
                Pa: 0,
                oa: 0,
                Fa: 0
            },
            tc: 0,
            Dc: 0,
            Ya: 0
        },
        I = d.nd = {
            aa: 0,
            fa: 0,
            ba: 0,
            ma: 0,
            g: {
                aa: 0,
                fa: 0,
                ba: 0,
                ma: 0
            },
            ca: [
                ["norm"]
            ],
            speed: 1,
            V: !1,
            ec: !1
        };

    function mc() {
        var a = -1 !== nc && Nb[nc].dd || 0;
        let b = 0 === a || 1 === a ? "Bullet" : 2 === a || 3 === a ? "Drone" : 4 === a ? "Swarm" : 5 === a ? "Trap" : 6 === a ? "Weapon" : "Unknown";
        return ["Body Damage", "Max Health", `${5===a?"Placement":b} Speed`, `${b} Health`, `${b} Penetration`, `${b} Damage`, 1 === a ? "Engine Acceleration" : 2 === a ? "Respawn Rate" : 3 === a ? "Max Drone Count" : "Reload", "Movement Speed", "Shield Regeneration", "Shield Capacity"]
    }
    var oc = [{
            amount: 0,
            T: 1,
            da: 1,
            color: 14
        }, {
            amount: 0,
            T: 1,
            da: 1,
            color: 5
        }, {
            amount: 0,
            T: 1,
            da: 1,
            color: 10
        }, {
            amount: 0,
            T: 1,
            da: 1,
            color: 1
        }, {
            amount: 0,
            T: 1,
            da: 1,
            color: 12
        }, {
            amount: 0,
            T: 1,
            da: 1,
            color: 3
        }, {
            amount: 0,
            T: 1,
            da: 1,
            color: 11
        }, {
            amount: 0,
            T: 1,
            da: 1,
            color: 0
        }, {
            amount: 0,
            T: 1,
            da: 1,
            color: 13
        }, {
            amount: 0,
            T: 1,
            da: 1,
            color: 2
        }],
        pc = 0,
        qc = [],
        rc = -1,
        nc = -1,
        sc = 0,
        tc = 0,
        uc = 0,
        vc = 0,
        wc = 0,
        xc = 0;
    let yc = {
            s: {
                s: new Sb(0, 1.2),
                level: new Sb(0, .4),
                progress: new Sb(0, .8)
            }
        },
        zc = () => Math.max(d.i / J.g.view, d.j / J.g.view / .5625);
    d.qa = !1;
    d.na = !1;
    d.message = "";
    let Gb = window.localStorage || {},
        Ac = Gb.getItem("tracking") || Gb.getItem("password") || null,
        Bc = null,
        Cc = parseInt(Gb.getItem("privilege"), 10),
        Dc = Number.isNaN(Cc) ? Ac ? 1 : 0 : Cc,
        Ec = () => {
            var a = d.ua.filter(f => null != f.visible && f.visible <= Dc);
            a.some(f => f.qb) && (a = a.filter(f => f.qb));
            let b = Infinity,
                c = [];
            for (let f of a)[, a] = f.code.split("-"), a = Math.abs(((d.Sa[1][a][2] - d.pa) % 24 + 36) % 24 - 12), a < b ? (c = [f], b = a) : a === b && c.push(f);
            return c[Math.floor(Math.random() * c.length)]
        },
        Fc = () => {
            var a = location.hash;
            a.startsWith("#") && (a = a.slice(1));
            let [, b, c] = a.match(/^([a-zA-Z]+)([0-9]*)$/) || [];
            if (b) d.pb = +c || 0;
            else {
                var f = {};
                for (var e of a.split("&")) {
                    a = e.split("=");
                    var h = a.shift();
                    f[h] = a.join("=") || !0
                }
                var n = f["private"];
                e = f.region;
                a = f.mode;
                h = f.host;
                f = f.key;
                n && (h = n, h.includes(";") && (f = h.split(";"), h = f.shift(), f = f.join(";")));
                if (h) e = `z-${e||"unk"}-${a||"x"}-${h.toLowerCase().replace(/(\.[^\.]+){1,2}$/,"").replace(/[^a-z0-9\-]/,"-")}`, d.ua.unshift({
                    id: "z",
                    code: e,
                    host: h,
                    Ka: !0
                }), b = "z", Bc = f || null;
                else return null
            }
            return d.ua.find(r => r.id === b) || null
        },
        Gc = a => {
            if (!a || "%" === a) return "Unknown";
            var b = d.Sa[2];
            let c = [],
                f = [];
            var e = 0;
            for (let z of b)
                for (let x of z)
                    if (x.id === a.charAt(e)) {
                        e++;
                        b = Object.assign({}, x);
                        if ("word" === x.Ib) {
                            var h = parseInt(a.charAt(e++), 36),
                                n = a.slice(e, e + h);
                            b.I = n.charAt(0).toUpperCase() + n.slice(1);
                            e += h
                        } else if ("words" === x.Ib) {
                            h = parseInt(a.charAt(e++), 36);
                            n = [];
                            for (let A = 0; A < h; A++) {
                                var r = a.charAt(e++);
                                if ("d" === r) n.push("-");
                                else if ("s" === r) n.push(" ");
                                else {
                                    r = parseInt(r, 36);
                                    let y = a.slice(e, e + r);
                                    n.push(y.charAt(0).toUpperCase() + y.slice(1));
                                    e += r
                                }
                            }
                            b.I = n.join("")
                        }
                        x.remove && (Array.isArray(x.remove) ? f.push(...x.remove) : f.push(x.remove));
                        c.push(b);
                        break
                    } if (0 === c.length) return "Unknown";
            a = c[c.length - 1];
            a.end && (a.I = a.end, a.advance && (a.advance = !1));
            for (a = 0; a + 1 < c.length; a++) c[a].delay && c[a + 1].advance && (e = c[a], c[a] = c[a + 1], c[a + 1] = e, a++);
            c = c.filter(({
                id: z
            }) => !f.includes(z));
            return c.map(z => z.I).join(" ")
        },
        Hc = {},
        Ic = () => {
            let a = document.getElementById("serverFilterRegion"),
                b = document.getElementById("serverFilterMode"),
                c = document.getElementById("serverSelector").parentNode.parentNode,
                f = document.getElementById("serverSelector"),
                e = document.createElement("tr");
            e.classList.add("message");
            e.appendChild(document.createTextNode("No Server Matches"));
            e.style.display = "none";
            f.appendChild(e);
            let h = [],
                n = [],
                r = (x, A) => {
                    let y = n.length;
                    n.push(A[0].filter);
                    let g = null;
                    for (let {
                            name: l,
                            filter: p
                        } of A) {
                        let v = document.createElement("span");
                        null == g && (g = v, g.classList.add("active"));
                        v.textContent = l;
                        v.addEventListener("click", () => {
                            v !== g && (g.classList.remove("active"), g = v, g.classList.add("active"));
                            n[y] =
                                p;
                            let t = !0;
                            for (let K of h) {
                                let C = !0;
                                for (let P of n) C = C && P(K);
                                K.element.style.display = C ? "" : "none";
                                t = t && !C
                            }
                            e.style.display = t ? "" : "none"
                        });
                        x.appendChild(v)
                    }
                };
            r(a, [{
                name: "All",
                filter: () => !0
            }, {
                name: "USA",
                filter: x => ["dal", "kci"].includes(x.Sb)
            }, {
                name: "Europe",
                filter: x => ["lon", "fra", "fsn"].includes(x.Sb)
            }, {
                name: "Asia",
                filter: x => ["sgp"].includes(x.Sb)
            }]);
            r(b, [{
                    name: "All",
                    filter: () => !0
                }, {
                    name: "FFA",
                    filter: x => /^p?o?m?f/.test(x.nb)
                }, {
                    name: "Squads",
                    filter: x => /^p?o?m?[ds]/.test(x.nb)
                }, {
                    name: "TDM",
                    filter: x => /^p?o?m?[1-4]$/.test(x.nb)
                },
                {
                    name: "Minigames",
                    filter: x => /^[xewz]|^p?o?m?[1-4]./.test(x.nb)
                }
            ]);
            let z;
            for (let x of d.ua) {
                if ((null == x.visible || x.visible > Dc) && d.D !== x) continue;
                let [A, y, g] = x.code.split("-"), [l, p] = d.Sa[0][A], [v, t] = d.Sa[1][y], K = document.createElement("tr");
                for (let C of [v, Gc(g), `${null==x.Qb?"?":x.Qb}/${p||"?"}`]) K.appendChild(document.createElement("td")).textContent = C;
                K.title = `${l} - ${t} - #${x.id} (${x.code})`;
                x.$c && K.classList.add("featured");
                Hc[x.id] = (C = !0, P = !0) => {
                    z.classList.remove("selected");
                    z = K;
                    z.classList.add("selected");
                    d.D = x;
                    P && (d.pb = 0);
                    Gb.setItem("gameMode", x.code);
                    C && (location.hash = `#${x.id}`);
                    c.scrollTop < K.offsetTop - 110 ? c.scrollTop = K.offsetTop - 110 : c.scrollTop > K.offsetTop - 30 && (c.scrollTop = K.offsetTop - 30)
                };
                K.addEventListener("click", Hc[x.id]);
                f.appendChild(K);
                h.push({
                    element: K,
                    Sb: y,
                    nb: g
                });
                d.D === x && (z = K, z.classList.add("selected"), setTimeout(() => {
                    c.scrollTop = K.offsetTop - 70
                }))
            }
        };

    function Jc() {
        return (-1 > d.pa ? ["ak7oqfc2u4qqcu6i", "hq3p9viv64d0js08", "n15rqgeh01clbn7n"] : 1 > d.pa ? ["hq3p9viv64d0js08", "ak7oqfc2u4qqcu6i", "n15rqgeh01clbn7n"] : 5 > d.pa ? ["hq3p9viv64d0js08", "n15rqgeh01clbn7n", "ak7oqfc2u4qqcu6i"] : ["n15rqgeh01clbn7n", "hq3p9viv64d0js08", "ak7oqfc2u4qqcu6i"]).map(a => `https://${a}.d.nsrv.cloud:2222`)
    }
    async function Kc() {
        var [a] = Jc();
        ({
            clients: a
        } = await fetch(`${a}/clientCount`).then(b => b.json()));
        return a
    }

    function Lc() {
        var a = Mc;
        let b = Date.now();
        3E5 < b - a.hc && (a.hc = b, Kc().then(c => a.fc = c));
        return a.fc
    }
    const Mc = {
        fc: 0,
        hc: 0
    };
    (async function() {
        let a = Jc();
        return new Promise(async (b, c) => {
            let f = !1,
                e = r => {
                    f || (f = !0, b(r.status))
                },
                h = a.length,
                n = r => {
                    h--;
                    0 < h || (f = !0, c(r))
                };
            for (let r of a)
                if (fetch(`${r}/status`).then(z => z.json()).then(z => {
                        for (let x of Object.values(z.status))
                            if (x.online) return z;
                        throw Error("No server is online");
                    }).then(e).catch(n), await ub(1500), f) break
        })
    })().then(a => {
        var b = {};
        for (let [c, f] of Object.entries(a)) {
            let e = f.name,
                h = {
                    visible: f.online ? 0 : 3,
                    id: c.replace("#", ""),
                    code: f.code,
                    host: f.host,
                    qb: 20 > f.mspt && 20 > f.clients,
                    Qb: f.clients,
                    $c: f.featured
                };
            null == b[e] ? b[e] = [h] : b[e].push(h)
        }
        for (let c of Object.values(b)) {
            if (1 < c.length) {
                for (let f of c) f.visible = Math.max(f.visible, 2);
                b = c.filter(f => 0 === f.Qb && 2 === f.visible);
                0 === b.length && (b = c.filter(f => 2 === f.visible));
                if (0 === b.length) continue;
                b[Math.floor(Math.random() * b.length)].visible = 0
            }
            d.ua.push(...c)
        }
        console.table(a)
    }).catch(() => {}).then(() => {
        for (let a of d.ua) {
            let [, b, c] = a.code.split("-"), [, f, e] = c.match(/([fds0-9])([dmastp]?)$/) || [null, "", ""], h = d.Sa[1][b][2];
            a.sort = {
                pa: null ==
                    h || "" === f ? -13 : h,
                Jc: /^[0-9]$/.test(f) ? +f : "%fds".indexOf(f) - 4,
                Hc: "%dmastp".indexOf(e)
            }
        }
        d.ua.sort((a, b) => a.sort.pa - b.sort.pa || a.sort.Jc - b.sort.Jc || a.sort.Hc - b.sort.Hc || (a.id > b.id ? 1 : -1));
        d.D = Fc() || d.ua.find(a => a.code === Gb.getItem("gameMode") && null != a.visible && a.visible <= Dc) || Ec();
        Ic();
        window.addEventListener("hashchange", () => {
            if (!d.D.connected) {
                var a = Fc();
                if (a && Hc[a.id]) Hc[a.id](!1, !1)
            }
        })
    });
    let Nc = (() => {
            let a = !1,
                b = document.getElementById("optionArrow"),
                c = document.getElementById("viewOptionText"),
                f = document.getElementsByClassName("sliderHolder")[0],
                e = document.getElementsByClassName("slider"),
                h = () => {
                    b.style.transform = a ? "translate(2px, -2px) rotate(45deg)" : "rotate(-45deg)";
                    c.innerText = a ? "close options" : "view options";
                    a ? f.classList.add("slided") : f.classList.remove("slided");
                    e[0].style.opacity = a ? 0 : 1;
                    e[2].style.opacity = a ? 1 : 0
                };
            document.getElementById("startMenuSlidingTrigger").addEventListener("click",
                () => {
                    a = !a;
                    h()
                });
            return () => {
                a || (a = !0, h())
            }
        })(),
        Oc = document.getElementById("patchNotes"),
        Pc = (() => {
            let a = document.getElementById("changelogSelector"),
                b = a.parentNode,
                c = a.firstElementChild,
                f = document.getElementById("patchNotes"),
                e = {};
            for (let h = 0; h < a.children.length; h++) {
                let n = a.children[h],
                    r = n.dataset.type;
                e[r] = () => {
                    if (n !== c) {
                        var z = c.dataset.type;
                        c.classList.remove("active");
                        n.classList.add("active");
                        f.classList.remove(z);
                        f.classList.add(r);
                        c = n;
                        b.scrollLeft = n.offsetLeft - a.offsetLeft + n.offsetWidth / 2 - b.offsetWidth /
                            2
                    }
                };
                n.addEventListener("click", e[r])
            }
            return e
        })(),
        Qc = (a, b = null) => {
            var c = a[0];
            if (!c) return !0;
            c = c.match(/^([A-Za-z ]+[A-Za-z])\s*\[([0-9\-]+)\]\s*(.+)?$/) || [c, c, null];
            var f = c[1] ? {
                Update: "update",
                Announcement: "update",
                Poll: "poll",
                "Event Poll": "event-poll",
                "Gamemode Poll": "event-poll",
                Event: "event",
                Gamemode: "event",
                "Balance Update": "balance-update",
                "Balance Update Details": "balance",
                Balance: "balance",
                Patch: "patch",
                Hidden: "hidden"
            } [c[1]] : null;
            if ("hidden" === f) return !0;
            let e = document.createElement("div");
            f && e.classList.add(f);
            var h = document.createElement("b"),
                n = [c[1]];
            if (c[2]) {
                let A = +new Date(c[2] + "T00:00:00Z") + 252E5;
                if (A > Date.now() && !location.search.includes("changelog_debug=1")) return !0;
                if (null != b && A + b < Date.now()) return !1;
                n.push((new Date(A)).toLocaleDateString("default", {
                    year: "numeric",
                    month: "long",
                    day: "numeric",
                    timeZone: "UTC"
                }))
            }
            c[3] && n.push(c[3]);
            h.innerHTML = n.join(" - ");
            e.appendChild(h);
            if ("poll" === f || "event-poll" === f) {
                let [A, y, g] = a[1].split(",").map(K => K.trim());
                var r = "table" === g;
                let l = "radio" ===
                    g;
                b = Math.ceil((new Date(y.trim()) - Date.now()) / 36E5);
                let p = 0 >= b;
                h.appendChild(document.createElement("br"));
                c = document.createElement("small");
                c.appendChild(document.createTextNode(p ? "closed" : `closing in ${b} hour${1===b?"":"s"}`));
                let v = document.createElement("a");
                v.href = "javascript:;";
                v.innerText = "view all results";
                r && c.appendChild(v);
                let t = document.createElement("a");
                t.href = "javascript:;";
                t.innerText = "reset";
                t.style.display = "none";
                l && !p && c.appendChild(t);
                h.appendChild(c);
                e.appendChild(document.createElement("br"));
                h = document.createElement("table");
                h.className = r ? "poll-table" : "poll-list";
                b = document.createElement("tbody");
                c = (() => {
                    let K = [],
                        C = [],
                        P = (new Promise(H => {
                            let L = !1,
                                Q = () => {
                                    if (!L) {
                                        var D = e.getBoundingClientRect();
                                        if (!(0 >= D.height)) {
                                            var M = Oc.getBoundingClientRect();
                                            D.top > M.bottom + M.height || D.bottom < M.top - M.height || (L = !0, H())
                                        }
                                    }
                                };
                            Oc.addEventListener("scroll", Q);
                            setTimeout(Q, 50)
                        })).then(() => fetch(`${"https://poll-server.arras.cx:2020"}/poll/${A}/status`)).then(H => H.json()).then(H => {
                            if (!H.ok) throw Error("Poll does not exist");
                            C = H.options.map(L => ({
                                Nc: L.voted,
                                Oc: L.votes
                            }))
                        });
                    v.addEventListener("click", () => {
                        v.remove();
                        let H = K.map(Q => parseInt(Q.title, 10)).sort((Q, D) => Q - D),
                            L = "#2196f3 #00adc3 #009688 #4caf50 #e8ae00 #ff8200 #ff0000".split(" ");
                        for (let Q of K) {
                            let D = parseInt(Q.title, 10);
                            Q.className = "count";
                            Q.innerHTML = 1E3 <= D ? (D / 1E3).toFixed(1) + "<span>k</span>" : 0 <= D ? D : "?";
                            Q.style.color = L[Math.floor((H.indexOf(D) + .5) / H.length * L.length)]
                        }
                    });
                    t.addEventListener("click", () => {
                        X.checked = !1;
                        X.update();
                        X = null;
                        t.style.display = "none"
                    });
                    let X = null,
                        ba = 0;
                    return H => {
                        let L = S => `${H?H+" - ":""}${S} vote${1===S?"":"s"}`,
                            Q = ba++,
                            D = document.createElement("label");
                        D.className = "container";
                        let M = document.createElement("input");
                        M.tabIndex = -1;
                        M.type = l ? "radio" : "checkbox";
                        M.disabled = !0;
                        l && (M.name = `radio-${A}`);
                        P.then(() => {
                            let {
                                Nc: S,
                                Oc: W
                            } = C[Q] || {
                                Nc: !1,
                                Oc: 0
                            };
                            M.checked = S;
                            l && S && (X = M, t.style.display = "inline");
                            M.disabled = p;
                            let ra = W - S;
                            M.update = () => {
                                fetch(`${"https://poll-server.arras.cx:2020"}/poll/${A}/set/${Q}/${M.checked}`);
                                let sa = ra + (M.checked ? 1 : 0);
                                H ? aa.nodeValue = L(sa) : D.title = L(sa);
                                l && X && X !== M && M.checked && (X.checked = !1, X.update());
                                X = M;
                                t.style.display = "inline"
                            };
                            M.addEventListener("change", () => M.update());
                            H ? aa.nodeValue = L(W) : D.title = L(W)
                        });
                        let aa;
                        H && (aa = document.createTextNode(H), D.appendChild(aa));
                        D.appendChild(M);
                        let na = document.createElement("span");
                        na.className = l ? "radio" : "checkmark";
                        D.appendChild(na);
                        K.push(D);
                        return D
                    }
                })();
                for (var z of a.slice(2)) {
                    a = document.createElement("tr");
                    if (r)
                        for (var x of z.split("|")) x = x.trim(), f = document.createElement("td"),
                            "X" === x ? f.appendChild(c()) : (n = x.match(/^:*/)[0].length, f.colSpan = n + 1, f.innerHTML = x.slice(n)), a.appendChild(f);
                    else f = document.createElement("td"), f.appendChild(c(z)), a.appendChild(f);
                    b.appendChild(a)
                }
                h.appendChild(b);
                e.appendChild(h)
            } else {
                z = document.createElement("ul");
                for (r of a.slice(1)) x = document.createElement("li"), x.innerHTML = r, z.appendChild(x);
                x = z.getElementsByTagName("a");
                for (a = 0; a < x.length; a++) {
                    r = x[a];
                    if (!r.href) continue;
                    let A = r.href.lastIndexOf("#"); - 1 !== A && (A = r.href.slice(A + 1), "options-menu" ===
                        A ? x[a].addEventListener("click", y => {
                            y.preventDefault();
                            Nc()
                        }) : Pc[A] && x[a].addEventListener("click", y => {
                            y.preventDefault();
                            Pc[A]()
                        }))
                }
                e.appendChild(z)
            }
            Oc.appendChild(e);
            return e
        };
    (a => {
        let b = [];
        var c = [];
        for (let f of a.split("\n")) 0 !== f.length && (a = f.charAt(0), "#" === a ? (b.push(c), c = [f.slice(1).trim()]) : "-" === a ? c.push(f.slice(1).trim()) : c[c.length - 1] += " " + f.trim());
        b.push(c);
        for (c = 0; c < b.length;) {
            if (Qc(b[c], 157248E5)) {
                c++;
                continue
            }
            b = b.slice(c);
            let f = Qc(["Older Changelogs", '<a class="view-older-changelogs" href="javascript:;">Click here to load changelogs more than 6 months old.</a>', '<a href="ext/changelog.html">View the original changelog here.</a>']);
            [c] = f.getElementsByClassName("view-older-changelogs");
            c.addEventListener("click", () => {
                f.remove();
                for (let e of b) Qc(e)
            });
            break
        }
    })(`
# Update [2022-02-17]
- Added a new death screen with a bit more information.

# Patch [2022-02-07]
- The soccer ball in Soccer now looks like a real soccer ball.

# Update [2022-02-01]
- There is now a new proxy link at <a target="_blank" href="https://arrax.io/">arrax.io</a>, which allows you to access the game when the main domain is unavailable.
- You can now filter which servers you can see in the server selector by choosing the region and the game mode category.
- The leaderboard in Soccer now displays the number of goals scored by each team, while in Mothership it displays the health of the Motherships.

# Balance Update [2022-01-29]
- Removed the Rogue Celestials.
- Bosses no longer spawn in Assault.
- Buffed Factory minion speed slightly.
- Buffed Ordnance's recoil to be the same as Hunter.
- Buffed Director's FOV to be the same as Overseer.
- Nerfed Twister and Skimmer main bullet and thruster.
- Nerfed Sidewinder's thruster reload.
- Nerfed Triplet's FOV to be the same as Basic.
- Nerfed Nyx's minion health.
- Architect now has a right click ability that Auto-3 uses.
- Changed Sprayer and Machine Gun's bullet size.
- Fixed Auto-Double, Auto-Cruiser, and Auto-Tri-Angle auto turret being inaccurate.
- You will no longer change team in Tag if you die to an invulnerable player.

# Poll [2022-01-23] Do you use arras.io's proxy link (arras.netlify.app), which allows you to access the game when the main domain is unavailable?
- 26, 2022-02-01T00:00:00.000Z, radio
- Yes, I use it and it works.
- Yes, I use it and it does not work.
- No

# Poll [2022-01-23] Should more proxy links be added?
- 25, 2022-02-01T00:00:00.000Z, radio
- Yes
- No

# Update [2022-01-08]
- The Healer upgrades, Medic, Ambulance, Surgeon, and Paramedic, have been officially added to Siege.

# Update [2022-01-01]
- Winter Mayhem has concluded, and the rocks are now back to normal. We hope you enjoyed it!

# Event Poll [2022-01-01] Should the new Healer classes be kept in Siege?
- 24, 2022-01-08T00:00:00.000Z, radio
- Yes
- No

# Update [2021-12-25]
- Players now have a chance to spawn out of Factories in TDM gamemodes.

# Gamemodes [2021-12-19]
- The Winter Mayhem event has now begun in US Siege and will end after the 25th!
- All Bosses, Sentries, and Sanctuaries within the mode have been festively redesigned and renamed.
- All Bosses have pentuple their normal base movement speed.
- All Waves feature double the amount of bosses seen in normal waves, excluding Celestial spawns.
- Sanctuaries have been slightly buffed to compensate for the increased difficulty.
- Your loss in Christmas spirit will not be ignored by the gods...

# Update [2021-12-09]
- The rocks have been changed to festive pine trees!
- For the month of December, we've added 4 new Healer upgrades to Siege: Medic, Ambulance, Surgeon, Paramedic.
- They may or may not only be here until the month ends.

# Balance [2021-12-09]
- Changed the Legionary Crasher's AI to orbit around sanctuaries from a farther distance.
- Reduced the Legionary Crasher's size by 18%.

# Update [2021-11-25]
- Happy Thanksgiving to everyone! The Motherships now look like turkeys.
- Changed the map layouts of Open 2TDM and Open 4TDM.
- Made Siege's map randomized again.
- Moved the nest in Assault into the bunker.

# Balance [2021-11-25]
- Buffed Single's reload and bullet speed slightly.
- Commander's drones now have the same damage as overseer and buffed its movement speed.
- Buffed Penta Shot's movement speed and nerfed its recoil.
- Nerfed Auto-Assassin's bullet range on its turret.
- Buffed the secondary bullet speed on Crossbow.
- Buffed Auto-4 all-around.
- Made Bomber's recoil the same as Tri-Angle.
- Changed the appearance of Director.
- Removed Rocketeer.
- Buffed Shotgun's recoil.
- Nerfed the damage and speed of Overdrive's turret's bullets.

# Patch [2021-11-12]
- Completely rework how the mockup data is sent.
- Nerf the strength of spike bounce.
- Going outside the map in Siege and Soccer now causes you to directly take damage.
- Fix Landmine looking like Smasher in the upgrade menu.
- Maleficitor drones will no longer be visible when you're near it. Manager and Stalker are also less visible than Landmine now.
- Notifications will no longer stack up after long periods of time.
- Staying still is now required to upgrade if you don't have spawn protection and are outside of your base.
- Rebalanced the player ratio in Assault.
- You will now become unpushable if you stay still while in base for 5 seconds.
- You now spawn at level 1 instead of level 0.
- Added support for the sandbox button on more keyboards.

# Update [2021-11-02]
- The physics of maze walls have been completely reworked. It's now much harder to be stuck inside of them.
- The Haunted Mansion event has ended, and rocks are now back to normal.
- The Soccer map was changed and is now rectangular.

# Patch [2021-11-02]
- Changed kill messages.
- Fixed grid alignment in portal modes.

# Event [2021-10-31]
- The Haunted Mansion event has started. Happy Halloween!
- A new server has also been added, and Soccer, Maze 2TDM, and Maze 4TDM will now appear in game mode rotation as normal again.

# Patch [2021-10-22]
- Added more protection against botting. As a consequence, you may notice spawning and respawning will take slightly longer than normal.
- Various bug fixes.

# Balance [2021-10-27]
- Decreased Maze's map size by 15%.

# Update [2021-10-18]
- Added 7 new themes: Retro, Pastel, Discord, WR Sheet Theme, Descent, Solarized Dark, and Eggplant.

# Update [2021-10-16]
- Mobile players can now use autofire, autospin, right click, and more! Go to <a href="/">arras.io</a> on your phone and press the plus button on the top left to use the new mobile control buttons.
- For right click actions such as drone repelling and predator scoping, you can switch between 5 modes: Never, Always, Automatic, Manual, Double Tap. The automatic mode allows you to control your drones normally when your finger is near the middle of the joystick, while repelling from the opposite side when it's further away. The manual mode will add a new button to repel your drones. The double tap mode makes your drone repel when you tap on the joystick twice.
- Patched FOV scripts with drone tanks.

# Update [2021-10-01]
- Overhauled Portal TDM and Maze Portal TDM! Each team has their own portal in their bases that they can use to move to the other arena! Additionally, Maze Portal TDM has now been split into two different maps!
- Added Soccer! Push the ball into the opposing team's base to win!
- Fixed the final boss spawn message(s) in Siege.
- Rebalanced Nyx.
- Added the Legionary Crasher. A new Siege exclusive Elite Crasher final boss! Good luck!
- Redid the recoil change made to Triple Shot, Penta Shot, and Bent Hybrid from last month.
- Did some additional balancing changes to Rocketeer.
- Fixed the issue where smashers could sit in portals indefinitely.
- As a nod to the season, the rocks are now pumpkins!

# Balance [2021-09-06]
- Increased Maze's map size by 50%.

# Update [2021-09-02]
- Added Nyx, the final Celestial.

# Balance [2021-09-02]
- Buffed Penta Shot, Bent Hybrid, and Triple Shot's movement speed but nerfed it's recoil.
- Nerfed Hunter, Poacher, Predator, and Ordnance.
- Nerfed Rocketeer.

# Patch [2021-08-27]
- The final bosses in Siege now grow in size after spawning.

# Balance [2021-08-27]
- Nerfed the Elite Skimmer.
- Slightly buffed Zaphkiel.
- Nerfed Theia.
- Made the Summoner more resistant to bullets.
- Nerfed the Rogue Celestials.
- Buffed the Elite Spawner.

# Balance [2021-08-01]
- Slightly buffed the Minion damage of Spawner branch.
- Slightly nerfed Factory's Minion speed but buffed it's movement speed.
- Slightly nerfed Octo Tank's penetration and reload but buffed it's bullet speed.
- Slightly buffed Machine Gunner's bullet speed.
- Nerfed the drone speed of Underseer branch and Overseer branch but buffed it's drone damage.
- Nerfed Maleficitor's drone count but buffed it's drone damage and drone speed.

# Update [2021-07-15]
- Added the Elite Spawner.

# Patch [2021-07-15]
- Shortened the time between the first few waves in Siege.
- The final bosses are now invulnerable for 3 seconds after spawning.

# Balance [2021-07-14]
- Buffed the speed of Constructor, Overtrapper, Manager, Septatrapper, and Hexa-Trapper.

# Patch [2021-07-13]
- Crashes now wave around when idle.
- Only final bosses spawn in the middle of the map in Siege.

# Update [2021-07-13]
- Added very rare types of sentries to Siege.
- Waves with a single boss now spawn in the middle of the map in Siege.
- Added a new final boss to Siege. Do you believe in Ragnarok?

# Update [2021-07-11]
- Reworked Sidewinder.
- Added Rocketeer, branches from Launcher.
- Added two new bosses.

# Update [2021-07-11]
- It doesn't exist. It is just in your head.

# Patch [2021-07-11]
- Removed Bosses from Maze.
- Limited boss spawns to only one in FFA.

# Balance [2021-07-11]
- Slightly buffed Assassin branch.

# Balance [2021-07-10]
- Buffed Single's bullet speed.
- Slightly buffed Tri-Angle branch.
- Slightly nerfed Fighter's side barrels.
- Buffed Banshee's Auto Turret turn radius.

# Patch [2021-07-07]
- Flipped Hewn Double.

# Balance [2021-07-07]
- Nerfed Mega Smasher.
- Slightly buffed Crossbow.
- Slightly nerfed the damage of Hunter branch.

# Balance [2021-07-04]
- Nerfed Hunter branch damage but buffed it's penetration.
- Buffed the base speed of Hunter, Poacher, Predator, and Ordnance.
- Reworked Skimmer and Twister.

# Update [2021-07-04]
- Spike bounce is now toggleable. Turn on override to disable it.

# Patch [2021-07-04]
- Spike no longer passes through teammates.

# Balance [2021-06-27]
- Nerfed the health of all Celestials.
- Buffed Ragnarok's shield.

# Patch [2021-06-27]
- Added shapes back to Siege.
- Removed Underseer branch from Siege.

# Balance [2021-06-19]
- Slightly buffed Tri-Trapper.
- Slightly nerfed Predator.
- Slightly nerfed Bomber's back barrels.

# Balance [2021-06-18]
- Buffed Beekeeper.

# Patch [2021-06-17]
- Reverted the name change to Auto-Overseer.

# Update [2021-06-17]
- Added Beekeeper, branches from Artillery.

# Balance [2021-06-17]
- Slightly nerfed all Tri-Angle branch tanks.
- Slightly buffed Auto-Overseer's auto turret.

# Balance [2021-06-16]
- Increased damage and penetration on Single whilst reducing bullet reload and speed slightly.
- Buffed Twister's reload and bullet speed.

# Balance [2021-06-15]
- Slightly buffed the base speed of Engineer, Conqueror, Constructor, Necromancer, and Underseer.
- Removed the recoil from the front barrels of Eagle and Falcon.

# Update [2021-06-13]
- Made Sentries smoother.

# Balance [2021-06-13]
- Rebalanced Sentries.

# Update [2021-06-12]
- Reworked Hunter branch.
- Flipped Fortress.

# Balance [2021-06-12]
- Nerfed Ragnarok.
- Buffed Auto-5, Auto-3, and Mega-3.

# Balance [2021-06-09]
- Buffed the health of celestials.
- Buffed all bosses in siege.
- Slightly nerfed the damage of Ragnarok, but buffed its health.

# Patch [2021-06-09]
- Attempted to fix the AI of celestials.
- Ragnarok is no longer called a Celestial.

# Patch [2021-06-06]
- Fixed a bug that crashes servers when a boss spawns.
- Slight changes to the Siege map.

# Balance [2021-06-06]
- Buffed all bosses in Siege.
- Buffed Ragnarok heavily.

# Update [2021-06-05]
- Slightly tweaked the Siege map.

# Balance [2021-06-05]
- Buffed Ragnarok.

# Update [2021-06-05]
- Completely redesigned the Siege map.
- Added Theia: A yellow celestial.
- Added Ragnarok: A final boss in Siege.
- Siege now features 31 waves, instead of 27.

# Update [2021-06-04]
- Completely reworked Elite Sprayer.
- Added a new boss.
- Made crashers smoother.

# Balance [2021-06-04]
- Slightly buffed Elite Battleship.
- Slightly nerfed Elite Gunner.

# Balance [2021-05-30]
- Slightly buffed the speed of all Assassin branch tanks excluding Falcon.

# Patch [2021-05-29]
- Removed Spike from Siege.

# Balance [2021-05-27]
- Buffed Cruiser and Auto-Cruiser.
- Buffed Factory's movement speed.
- Buffed Nailgun's movement speed.

# Balance [2021-05-27]
- Slightly buffed Sprayer.
- Slightly buffed all Celestials.

# Balance [2021-05-25]
- Reworked the Summoner
- Buffed Elite Gunner, Rogue Palisade, Elite Battleship, Summoner, Elite Skimmer, Freyja, and Zaphkiel's movement speed.
- Nerfed Elite Sprayer, Elite Destroyer, Nest Keeper, and Paladin's movement speed.
- Nerfed the Celestial's health.
- Nerfed Freyja.
- Nerfed Paladin.
- Nerfed Zaphkiel.

# Balance [2021-05-25]
- Slightly reduced the speed of Overlord, Autoseer, Commander, Overdrive, and Overseer.

# Balance [2021-05-23]
- Slightly buffed the base speed of Overseer, Overlord, Overtrapper, Overgunner, Overdrive, Autoseer, Banshee, Commander, and Manager.

# Balance [2021-05-22]
- Slightly buffed the speed of Septa Trapper and Hexa Trapper.

# Update [2021-05-21]
- Added Launcher, branches from Pounder.
- Sidewinder no longer branches from Hunter but now branches from Launcher.
- Swarmer no longer branches from Destroyer but now branches from Launcher.
- Skimmer and Twister no longer branch from Artillery but now branches from Launcher.
- Reworked Overdrive so all of its drone turrets shoot at once and slightly increased their penetration and damage

# Gamemode Poll [2021-05-18] Which of these gamemodes do you like?
- 23, 2021-05-23T00:00:00.000Z, any
- FFA With Maze
- FFA Without Maze
- Siege

# Gamemode Poll [2021-05-18] Which of the following team gamemodes do you like?
- 22, 2021-05-23T00:00:00.000Z, table
- |::<b>With Maze</b>|::<b>No Maze</b>
- <b>Number of Teams<b> |<b>2</b>|<b>3</b>|<b>4</b>|<b>2</b>|<b>3</b>|<b>4</b>
- TDM               |X| |X|X| |X
- Domination        |X| |X|X| |X
- Mothership        |X| |X|X| |X
- Open TDM          |X|X|X|X|X|X
- Open Domination   |X|X|X|X|X|X
- Tag               |X|X|X|X|X|X
- Portal TDM        |X| |X|X| |X
- Portal Mothership |X| |X|X| |X
- Assault           |X| | |X| |

# Balance [2021-05-18]
- Buffed Banshee's drones and acceleration while removing the ability to control it's auto turrets.

# Balance [2021-05-17]
- Buffed the Elite Skimmer but nerfed it's health.
- Nerfed the Nest Keeper but buffed it's FoV.
- Reworked the Elite Gunner.
- Nerfed the Elite Battleship.
- Buffed the Elite Sprayer.
- Buffed the Elite Destroyer.
- Buffed the Rogue Palisade.

# Balance [2021-05-16]
- Buffed Skimmer, Twister, and Swarmer.
- Slightly buffed the FoV of Twister.
- Buffed the accuracy of AI turrets.

# Balance [2021-05-13]
- Made Bulwark's traps more focused.

# Patch [2021-05-13]
- Renamed Auto-Overseer to Autoseer.

# Balance [2021-05-05]
- Buffed Builder, Auto-Builder, Boomer, and Conqueror's movement speed.
- Buffed Machine Gunner.
- Nerfed Surfer's swarm range.

# Balance [2021-05-04]
- Slightly buffed Triple Twin.

# Balance [2021-05-02]
- Buffed Hexa-Trapper and Septa-Trapper's movement speed.
- Fixed Sprayer's mini bullet.
- Buffed Sidewinder's bullet speed and range.

# Balance [2021-04-22]
- Buffed Auto-4's bullet speed.

# Balance [2021-04-20]
- Buffed Auto-3's bullet range.
- Buffed Auto-5's bullet health, penetration, and damage.
- Buffed Auto-4's bullet health, penetration, and damage.
- Buffed Trap Guard's basic barrel.

# Patch [2021-04-12]
- Spawner, Auto-Spawner, and Factory minions will no longer bounce when colliding with each other.

# Balance [2021-04-11]
- Slightly buffed Hewn Double.
- Slightly buffed Gunner and Auto-Gunner.
- Nerfed tanks that use sunchips.

# Balance [2021-04-11]
- Doubled the time it takes for Landmine to go invisible.
- Increased the range at which you can begin to see invisible tanks.

# Patch [2021-04-10]
- Your drones will no longer collide with the drones of your teammates.

# Balance [2021-04-09]
- Slightly increased Ranger's FoV.
- Slightly increased Maleficitor's drone speed.
- Fixed bosses becoming overpowered due to the drone buff of necro classes.

# Balance [2021-04-08]
- Slightly buffed spawned drones from all necro classes.
- Slightly nerfed the FoV of drones and minions.
- Increased base speed of Maleficitor, Spawner, and Auto-Spawner.

# Balance [2021-04-08]
- Slightly nerfed Crossbow and Auto-Tri-Angle's penetration.

# Balance [2021-04-08]
- Slightly buffed all hybrid-type drones.

# Update [2021-04-07]
- Added Armsman, branches from Rifle.
- Added Crossbow, branches from Rifle.

# Balance [2021-04-07]
- Slightly increased the base speed of Ranger, Sidewinder, and Fortress.

# Balance [2021-02-25]
- Nerfed Shield Capacity.
- Nerfed Battleship AI drones.
- Fixed Sprayer and Machine Gun's recoil.
- Nerfed Skimmer and Twister's range.
- Slightly buffed Sidewinder's Snake's speed.
- Health of Surfer, Fighter, and Bomber is now consistent with Tri-Angle.
- Slightly buffed the reload of Auto-Double and Auto-Overseer's auto turrets.

# Update [2021-01-17]
- You can now press F to take control of an uncontrolled Dominator or Mothership.

# Patch [2021-01-14]
- Fixed the rotation speed of smashers.

# Gamemode [2021-01-13]
- Changed how the gamemodes are randomized in the US servers.

# Balance [2021-01-12]
- Slightly nerfed Shield Capacity and Body Damage.

# Patch [2021-01-07]
- Optimized bandwidth usage.

# Update [2021-01-05]
- The first person to join a sandbox now has operator access, and can press &#96; + F1 for the list of operator commands that can used while holding &#96;.

# Balance Update [2021-01-05]
- Buffed Body Damage and Shield Regeneration.

# Patch [2021-01-02]
- Added more Sandbox servers.
- Made sandbox arenas dynamically resize.
- Added basic controller support.

# Gamemode [2021-01-01]
- Added Sandbox mode.
- More features will be coming soon.
- Note that if there are too many players, you'll be placed in a random sandbox with another player.

# Patch [2020-12-30]
- Changed the server speed display to use mspt and made the capitalization of arras.io more consistent.
- Prevented crashers from passing through walls.

# Update [2020-12-29]
- Changed the maze generation in Maze and Open Maze TDM.
- Buffed Shield Capacity and nerfed Shield Regeneration.
- Redesigned Commander.
- Made autospin faster.
- <a href="#balances">Other balance changes</a>.

# Balance [2020-12-29]
- Banshee can now control its auto turrets.
- Buffed the reload of Falcon.
- Nerfed Sidewinder.

# Update [2020-12-25]
- Added Commander, branches off of Overseer.
- Added Auto-Cruiser, branches off of Cruiser.

# Patch [2020-12-24]
- Fixed the color of the purple base.

# Balance [2020-12-22]
- Invisible tanks are now more visible when you're near it.

# Patch [2020-12-22]
- Made the spawn screen faster.

# Patch [2020-12-21]
- Slightly changed the names of some gamemodes.

# Event [2020-12-20]
- The squads event has started! Added Squads in USA and Asia.
- Players spawn on teams of 4 with random players. You can also press F to create or leave a private team.

# Patch [2020-12-18]
- Fixed players spawning outside sanctuaries.

# Patch [2020-12-17]
- Made the low graphics option lower the resolution so it could actually improve performance.

# Patch [2020-12-16]
- Changed how names and health bars are rendered.

# Patch [2020-12-11]
- The arena in Tag now shrinks gradually instead of starting out small.

# Patch [2020-12-09]
- Arena closers can now see invisible players.
- Most turrets no longer fire when the target is outside the firing arc.
- Prevented players in Siege from leaving the fortress.

# Balance [2020-12-07]
- Reworked how recoil works in general and made it more consistent.

# Patch [2020-12-06]
- Added a new region code system.
- Optimized entity controllers and fixed a memory leak.

# Gamemode [2020-12-06]
- The server selector now displays the player count and player limit instead of the server provider and is sorted by region.
- Added new servers and shuffled the gamemodes. Now almost all of the servers have partially randomized gamemodes.
- USA has Maze, TDM, Domination/Mothership, Open TDM, Pandemic/Tag, Siege/Assault, and 4 Team Maze gamemodes.
- Europe has FFA and a random team gamemode.
- Asia has a Maze 4TDM variant and a random team gamemode, and sometimes Maze.
- Additionally, there's a randomized gamemode server in both USA and Asia.

# Balance Update [2020-11-29]
- Rebalanced Battleship.
- Nerfed bosses.
- Changed the way Spreadshot looks.
- Go to the <a href="#balances">balance update section</a> for more details.

# Balance Update Details [2020-11-29]
- Buffed the reload, range, and damage of Battleship's guided swarm drones.
- Buffed the speed and damage of Battleship's autonomous swarm drones but nerfed their reload and range.
- Nerfed the reload and bullet stats of Elite Battleship.
- Nerfed the bullet stats of Nest Keeper's Mega Crasher.
- Nerfed the bullet stats of Elite Skimmer's Hypermissiles.
- Nerfed the range of Freyja.
- Nerfed the range and reload of Summoner.

# Patch [2020-11-28]
- Made the new prediction the default.
- Improved debug information to show prediction mode, bandwidth usage (in bytes per second), entity count, and server idle time.
- You can now press Shift + L to keep debug information on instead of holding L.
- Added a display for the author of custom themes.

# Patch [2020-11-24]
- Various client and server reworks and minor optimizations.
- Note that if you own a private server, you can now go <a target="_blank" href="https://glitch.com/edit/#!/arras-template?path=CHANGELOG.md">here</a> to enable the class tree.

# Update [2020-11-15]
- Added randomized gamemodes.

# Patch [2020-11-15]
- Made the text in the server selector slightly smaller.
- Banned a few multiboxing scripts.

# Balance [2020-11-15]
- Nerfed the penetration of Auto Double's auto-turret.
- Nerfed the damage of Auto Overseer's auto-turret.
- Nerfed the accuracy and bullet speed of Engineer's pillboxes.

# Event [2020-11-13]
- Started the Pandemic event, replacing Tag in US East and Asia.
- When a player on the <b style="color: #00b0e1">blue</b> team dies to a player on the <b style="color: #3cbf00">green</b> team, they are infected and will respawn on the green team.
- The game ends when everyone is infected.

# Patch [2020-11-07]
- Players can no longer get stuck in walls.
- Going through a portal now grants invulnerability for 5 seconds.

# Gamemode Poll [2020-11-01] Which of these gamemodes do you like?
- 21, 2020-11-08T00:00:00.000Z, any
- FFA With Maze
- FFA Without Maze
- Siege

# Gamemode Poll [2020-11-01] Which of the following team gamemodes do you like?
- 20, 2020-11-08T00:00:00.000Z, table
- |::<b>With Maze</b>|::<b>No Maze</b>
- <b>Number of Teams<b> |<b>2</b>|<b>3</b>|<b>4</b>|<b>2</b>|<b>3</b>|<b>4</b>
- TDM               |X| |X|X| |X
- Domination        |X| |X|X| |X
- Mothership        |X| |X|X| |X
- Open TDM          |X|X|X|X|X|X
- Open Domination   |X|X|X|X|X|X
- Tag               |X|X|X|X|X|X
- Portal TDM        |X| |X|X| |X
- Portal Mothership |X| |X|X| |X
- Assault           |X| | |X| |

# Patch [2020-11-01]
- Changed the gamemode code for Assault, Siege, and Tag.
- The server list is now fetched from the central servers.
- Slightly changed the sorting of the servers.
- Forced the HTTPS redirect.

# Update [2020-10-28]
- Servers that are unavailable are now automatically removed.
- You can now paste in party links without reloading.

# Patch [2020-09-28]
- Added reCAPTCHA and a few other anti-botting measures.
- Removed the reCAPTCHA badge from the bottom right during the game.

# Update [2020-09-06]
- The tag event has started! Added Tag in US East and Asia.
- Players who are killed will respawn on the team that killed them.
- A team wins when there are no other team alive.

# Patch [2020-08-22]
- Added an in-game display for server ID.
- Added a workaround for lag caused by mouse movement in Chrome 84.

# Gamemode Poll [2020-07-29] What FFA gamemodes do you like?
- 19, 2020-07-31T07:00:00.000Z, any
- FFA With Maze
- FFA Without Maze

# Gamemode Poll [2020-07-29] Which of the following team gamemodes do you like?
- 18, 2020-07-31T07:00:00.000Z, table
- |::<b>With Maze</b>|::<b>No Maze</b>
- <b>Number of Teams<b> |<b>2</b>|<b>3</b>|<b>4</b>|<b>2</b>|<b>3</b>|<b>4</b>
- TDM             |X| |X|X| |X
- Domination      |X| |X|X| |X
- Mothership      |X| |X|X| |X
- Open TDM        |X|X|X|X|X|X
- Open Domination |X|X|X|X|X|X

# Update [2020-07-20]
- Added Ordnance, branches off of Artillery and Hunter.

# Gamemode [2020-07-06]
- The event has ended.
- US West has Assault, US East has Siege, Europe has Portal 4TDM, and Asia has Portal Mothership.

# Event Poll [2020-06-29] Which of these gamemodes do you like?
- 17, 2020-07-06T07:00:00.000Z, any
- Assault
- Siege
- Portal 4TDM
- Portal Mothership

# Patch [2020-06-26]
- Added upgrade cooldown when you have bullets on the map.
- Fixed a vulnerability that could cause server crashes.
- Spawn protection now lasts 60 seconds.
- Shuffled the gamemodes for bandwidth optimization.

# Patch [2020-06-25]
- Fixed green team's losing condition in Assault.

# Event [2020-06-23]
- We have brought back all of the event gamemodes: Assault, Siege <span class="tooltip"><span>Originally the boss event</span></span>, Portal 4TDM, and Portal Mothership.
- Each region will have an opportunity to play each of these gamemodes at least once during the event.

# Gamemode [2020-06-23]
- Slightly reworked and readded Assault.
- For the <b style="color: #00b0e1">blue</b> team to win, they must invade and destroy all of the green sanctuaries.
- For the <b style="color: #3cbf00">green</b> team to win, they must guard their sanctuaries and dominators and keep at least 3 of them continuously for 8 minutes.
- When a sanctuary is destroyed, it can only be repaired to become a dominator.
- Changed the gamemodes of all of the servers.
- There are now three US West servers with the gamemodes 4TDM, Maze 2TDM, and Maze Domination.
- US East now has 2TDM, 4 Team Domination, and FFA.
- Europe only has Maze.
- Asia has FFA and Maze 4TDM.

# Patch [2020-06-23]
- Changed maze generation for Assault to make the sanctuaries more connected.

# Event Poll [2020-06-18] Do you like the portal event?
- 16, 2020-06-22T00:00:00.000Z, radio
- No
- Yes, but it should only be an event
- Yes, and it should become a regular gamemode

# Event [2020-06-08]
- The portal event has started! Added Portal 4TDM and Portal Mothership.
- In Portal 4TDM, one side of the arena is normal 4TDM and the other is Maze 4TDM.
- In Portal Mothership, Motherships spawn on opposing sides of the arena.
- Portals connect the two sides of the arena.

# Gamemode [2020-06-01]
- Added Assault.
- For the <b style="color: #00b0e1">blue</b> team to win, they must invade and destroy the green sanctuary, which can be unlocked by destroying all of their three dominators.
- For the <b style="color: #3cbf00">green</b> team to win, they must guard their dominators and sanctuary and keep at least 3 of them continuously for 8 minutes.
- Added 2TDM and restored some of the servers that ran out of bandwidth.

# Gamemode Poll [2020-05-30] What FFA gamemodes do you like?
- 15, 2020-06-04T00:00:00.000Z, any
- FFA With Maze
- FFA Without Maze

# Gamemode Poll [2020-05-30] Which of the following team gamemodes do you like?
- 14, 2020-06-04T00:00:00.000Z, table
- |::<b>With Maze</b>|::<b>No Maze</b>
- <b>Number of Teams<b> |<b>2</b>|<b>3</b>|<b>4</b>|<b>2</b>|<b>3</b>|<b>4</b>
- TDM             |X| |X|X| |X
- Domination      |X| |X|X| |X
- Mothership      |X| |X|X| |X
- Open TDM        |X|X|X|X|X|X
- Open Domination |X|X|X|X|X|X

# Balance [2020-05-29]
- Buffed the reload and accuracy of auto-turrets.
- Nerfed the acceleration of Mega Smasher.
- Nerfed Engineer.
- Nerfed rammers.

# Balance [2020-05-27]
- Buffed Sidewinder, Spreadshot, and Auto-4.
- Buffed all auto-turrets.
- Buffed Gunner Trapper's recoil.
- Buffed Battleship's reload.
- Nerfed Carrier's field of view.
- Nerfed the bullet speed of Artillery, Mortar, Barricade, and Auto-5.
- Buffed the knockback of Factory drones.
- Mega Smasher is now faster but no longer completely immune from knockback.

# Patch [2020-05-27]
- Replaced some servers to deal with bandwidth issues.

# Event Poll [2020-05-17] Did you like the boss event?
- 13, 2020-05-22T00:00:00.000Z, radio
- No
- Yes, but it should only be an event
- Yes, and it should become a regular gamemode

# Event [2020-05-17]
- The boss event has ended.

# Balance [2020-05-16]
- Buffed the shield capacity stat.

# Update [2020-05-16]
- Invisible tanks such as smasher are now slightly visible when you're near it.
- Healer can now gain score from healing other players.

# Event [2020-05-14]
- Added Sanctuaries to the boss event.
- Players can only spawn in the sanctuaries. When all of the sanctuaries are destroyed and not restored in 60 seconds, the game is lost.

# Balance [2020-05-13]
- Nerfed the knockback of Healer.

# Update [2020-05-12]
- Redesigned Healer's appearance to look different from Sidewinder.

# Patch [2020-05-12]
- Replaced some servers to deal with bandwidth issues.
- Added rocks to FFA.
- When under spawn protection, you no longer deal damage to bosses and motherships, and you also cannot turn invisible.
- Added a contact link.

# Event [2020-05-09]
- The boss event has started! Bosses will spawn in waves while players defend themselves.
- Added Healer, branching off of Basic, for the boss event.

# Balance [2020-05-09]
- Buffed bosses.

# Patch [2020-05-09]
- Shuffled the gamemodes for bandwidth optimization.

# Patch [2020-05-08]
- Reworked AI pathfinding to avoid walls.

# Patch [2020-04-14]
- Added HTTPS redirect.

# Patch [2020-04-13]
- Blacklisted suspicious IPs.

# Patch [2020-04-10]
- Changed server list sorting.
- Added support for <a href="https://arras.io/">HTTPS</a>.

# Gamemode Update [2020-04-10]
- Added three new servers and new gamemodes.
- There are now two US West servers with gamemodes 4 Team Mothership and Maze FFA.
- US East now has Maze 4TDM, 4 Team Maze Domination, Assault, Domination, and FFA.
- Europe has two servers with gamemodes Maze 2TDM and 4TDM.
- Asia now has Maze Domination and Open Maze 4TDM.

# Patch [2020-03-25]
- Fixed bug where the score of the Mothership is restored to players when they respawn after a server overload.

# Patch [2020-03-24]
- Added rocks to Open 3TDM.

# Patch [2020-03-20]
- Optimized text rendering on older browsers.

# Gamemode Poll [2020-03-20] What FFA gamemodes do you like?
- 12, 2020-03-24T00:00:00.000Z, any
- FFA With Maze
- FFA Without Maze

# Gamemode Poll [2020-03-20] What team gamemodes do you like?
- 11, 2020-03-24T00:00:00.000Z, table
- |::<b>With Maze</b>|::<b>No Maze</b>
- <b>Number of Teams<b> |<b>2</b>|<b>3</b>|<b>4</b>|<b>2</b>|<b>3</b>|<b>4</b>
- TDM                                                                       |X| |X|X| |X
- Domination                                                                |X| |X|X| |X
- Mothership                                                                |X| |X|X| |X
- Open TDM                                                                  |X|X|X|X|X|X
- Open Domination                                                           |X|X|X|X|X|X
- Assault <span class="tooltip"><span>Same as the D-Day event</span></span> |X| | |X| |

# Gamemode [2020-03-19]
- Replaced Glitch and Heroku servers with an ExtraVM server.
- Added Open 3TDM gamemode.

# Update [2020-03-18]
- Added Elite Battleship, idea by Reflection.
- Added Nest Keeper, idea by Whert.

# Patch [2020-03-18]
- Applied a new naming scheme in party links.

# Patch [2020-03-16]
- Switched servers from DVI to ExtraVM.

# Patch [2020-03-12]
- Updated the link to the proxy server.

# Gamemode [2020-03-10]
- Removed the Space 3TDM event gamemode as the server has been down.

# Patch [2020-03-09]
- Added a button that links to <a target="_blank" href="https://momentumstudios.games/">Momentum Studios</a>.

# Patch [2020-03-09]
- Migrated from <a target="_blank" href="https://surge.sh/">surge.sh</a> to <a target="_blank" href="https://www.netlify.com/">Netlify</a>.
- Added older changelogs into the same document.

# Announcement [2020-03-06]
- Arras.io has received no updates in the last 7 months, but that will soon change. The developer has now joined forces with Momentum Studios, a small game company specializing in the development of online games.
- We hope to be able to work with everyone soon, and bring some life back into the arras.io community!

# Patch [2020-01-10]
- Fixed a bug with more than 9 upgrades in private servers.

# Patch [2019-12-19]
- Fixed a bug with the disconnection screen showing a score instead of the disconnection message.

# Patch [2019-08-09]
- Changed the disconnection screen to display a different message in case of AFK timeout.

# Balance [2019-07-21]
- Nerfed Mega-Smasher's movement speed but made it immune to knockbacks.

# Event [2019-07-21]
- Made the arena in the Space 3TDM circular and around the moon.

# Patch [2019-07-18]
- Fixed auto-restore in Space modes.

# Patch [2019-07-17]
- Fixed a bug where you can move out of the arena bound in Space modes.
- Made the yellow event label a bit less flashy.
- Made the server selector larger.
- Made everything change in size smoothly instead of instantly again.
- Bosses now ignore you if you're within a base.

# Event [2019-07-16]
- Added a new US East server from ExtraVM that can run 2 servers.
- Moved 4TDM to the new server and added the event gamemode.
- For the 50th anniversary of Apollo 11, the event gamemode is Space 3TDM!
- Added <a target="_blank" href="https://wikipedia.org/wiki/Reaction_control_system">RCS</a> that will automatically make you slow down if you don't press any key, which you can disable with R.
- Removed the old Glitch 4TDM server.

# Balance Update [2019-07-03]
- Nerfed Tri-Angle branch's recoil and buffed Destroyer branch's recoil.
- Buffed previously overnerfed branches including Double Twins and Necromancers.
- Slightly buffed Gunner branch's bullet speed.
- Buffed Conqueror, Sidewinder, Shotgun, and Boomer.
- Buffed the stats and reload of all regular traps.
- Made Builder branch traps not overshoot.
- Made the slower tanks faster.
- Go to the <a href="#balances">balance update section</a> for more details.

# Balance Update Details [2019-07-03]
- Buffed Destroyer, Annihilator, and Hybrid's recoil.
- Buffed Conqueror's destroyer barrel's reload and recoil.
- Nerfed Annihilator's reload to be the same as Destroyer.
- Nerfed the thruster recoil of all Tri-Angle upgrades except Bomber.
- Nerfed Tri-Angle, Auto-Tri-Angle, Booster, Surfer, and Bomber's front barrel speed.
- Nerfed Fighter's front and side barrel speed.
- Slightly buffed the stats of all Double Twin upgrades except bent double.
- Slightly buffed Gunner, Auto-Gunner, and Cyclone's bullet speed.
- Buffed Sidewinder's bullet speed.
- Buffed Shotgun's bullet stats and accuracy.
- Buffed Director's reload.
- Buffed Underseer and Necromancer drones.
- Buffed the stats and reload of all regular traps.
- Buffed Boomer's damage.
- Made Builder branch traps not overshoot.
- Buffed Assassin branch, Swarmer, Banshee, Manager, Constructor, Overtrapper, Spawner, and Auto-Spawner's movement speed.

# Patch [2019-07-03]
- Added temporary fireworks for Independence Day for people in the United States.
- Fixed NaN bug with protocol.
- Made the references to changelog categories clickable.

# Balance [2019-06-24]
- Nerfed recoil of Gunner Trapper.
- Buffed bullet stats of Overgunner to be the same as Gunner Trapper.

# Patch [2019-06-22]
- Made the client send movement instantly to reduce latency.
- Used the improved encoding algorithm on the client, which was originally only used on the server.
- Note that if you own a private server, you need to go <a target="_blank" href="https://glitch.com/edit/#!/arras-template?path=CHANGELOG.md">here</a> to keep your server working!
- Updated the <a target="_blank" href="https://glitch.com/edit/#!/arras-template?path=README.md">link format for private servers</a>.

# Patch [2019-06-17]
- More client side optimizations.
- Set the AFK timeout to only kick players when they have been inactive for 5 minutes.
- Neutral and enabled buttons now deal no damage at all.
- Added a display for tank speed in the debug menu, which you can access by pressing L.

# Update [2019-06-17]
- Added auto-level up, which automatically makes you level up after spawning. By default it's enabled on mobile, but you can set it in the <a href="#options-menu">options menu</a>.
- Added the option to use the new interpolation mode, also in the <a href="#options-menu">options menu</a>, which can sometimes make the game smoother.
- Doors can no longer put you into walls now.

# Patch [2019-06-16]
- Added colored indicators to the result page of table polls.

# Gamemode Poll [2019-06-15] What FFA gamemodes do you like?
- 10, 2019-06-19T00:00:00.000Z, any
- FFA With Maze
- FFA Without Maze

# Gamemode Poll [2019-06-15] What team gamemodes do you like?
- 9, 2019-06-19T00:00:00.000Z, table
- |::<b>With Maze</b>|::<b>No Maze</b>
- <b>Number of Teams<b> |<b>2</b>|<b>3</b>|<b>4</b>|<b>2</b>|<b>3</b>|<b>4</b>
- TDM                                                                       |X| |X|X| |X
- Domination                                                                |X| |X|X| |X
- Mothership                                                                |X| |X|X| |X
- Open TDM                                                                  |X|X|X|X|X|X
- Open Domination                                                           |X|X|X|X|X|X
- Assault <span class="tooltip"><span>Same as the D-Day event</span></span> |X| | |X| |

# Patch [2019-06-15]
- Added tooltips.
- Fixed a bug with invalid dates in the changelog on certain devices.
- Removed featured status from D-Day.

# Event Poll [2019-06-14] Did you like the D-Day event?
- 8, 2019-06-17T00:00:00.000Z, radio
- No
- Yes, but it should only be an event
- Yes, and it should be renamed and kept as a possible normal gamemode

# Patch [2019-06-13]
- Fixed scrollbars in Firefox.
- Added styling for radio boxes.
- Added support for radio polls.

# Patch [2019-06-12]
- Fixed a bug with negative levels in private servers.
- Prevented everything from moving around weirdly during severe lag.
- Made the references to the <a href="#options-menu">options menu</a> in the changelog clickable.
- Fixed a bug with the <a href="#options-menu">options menu</a> overflowing on Firefox.

# Patch [2019-06-09]
- Minor color changes with the spawn screen.
- Fixed the changelog entries overlapping with each other on small screens.
- Various client side optimizations.

# Balance [2019-06-07]
- Buffed trapper dominator's reload.

# Patch [2019-06-07]
- Fixed some bugs with the spawn screen on Firefox.
- Modified the D-Day map, removing a diagonal hole in the wall near the Axis base.
- Fixed a bug with Maleficitor drones not turning invisible.

# Event [2019-06-06]
- Added an event mode on Maze 2TDM, which is replacing Open 3TDM.
- As today is the 75th anniversary of <b>D-Day</b>, it is the event gamemode!
- The allies, which are <b style="color: #00B0E1">blue</b>, have more tanks than the Axis powers, which are <b style="color: #F04F54">red</b>.
- For the allies to win, they must invade and destroy the red central base on the bottom right, which is unlocked by destroying all of their dominators scattered on the map.
- For the Axis powers to win, they must guard all of their dominators and base and keep at least 4 of them continuously for 8 minutes.

# Patch [2019-06-03]
- Fixed a bug where keys would act multiple times if you press enter multiple times while spawning.
- Changed the dropdown icon in the <a href="#options-menu">options menu</a>.

# Patch [2019-06-02]
- Made the changelog category selector scrollable.
- Resized maze walls so they fit together better.
- Hidden the leaderboard in screenshot mode.
- Made the server selector scroll when selected.
- Fixed a bug with spaces in text measurer fallback for older devices.
- Added a way to <a target="_blank" href="https://glitch.com/edit/#!/arras-template?path=README.md">specify gamemodes for private servers</a>.

# Patch [2019-06-01]
- Added the option to use the mouse on mobile devices in the <a href="#options-menu">options menu</a>.
- Made the changelog category selector be on top of the changelog on smaller screens.
- Fixed a bug with the respawn delay.
- Multiple maze walls in a square arrangement will join together.
- Added some client side optimizations.

# Patch [2019-05-28]
- Automatically select the nearest fast server by default instead of a random one.

# Patch [2019-05-26]
- Fixed a bug on mobile where you shoot immediately after tapping to spawn.
- Fixed another bug on mobile where tapping without dragging will not update your aim.
- Added notification to add to home screen on mobile.

# Patch [2019-05-25]
- Reworked broadcast system again to significantly lower bandwidth.
- Note that if you own a private server, you need to go <a target="_blank" href="https://glitch.com/edit/#!/arras-template?path=CHANGELOG.md">here</a> and update your server accordingly to keep your server working!
- Newly spawned tanks now flash when they're invulnerable.

# Update [2019-05-25]
- Added new mobile controls with larger buttons for stats and joysticks on the bottom. Just go to <a href="/">arras.io</a> on your phone to use it!
- You can now press Q to save screenshots instead of just pressing Z to record videos.
- Added <a href="https://youtu.be/5XY8kKIZN-w">custom keybinds</a>, which you can access by going to the <a href="#options-menu">options menu</a> and clicking on the key indicators.
- Note that you can also press backspace to unbind keys.
- Added changelog categories. <a href="#updates">Updates</a> is for all major updates. <a href="#events">Events & Gamemodes</a> is for major updates involving events or gamemodes. <a href="#balances">Balance</a> is for every balance update, major or minor. <a href="#changelog">Changelog</a> will show you every single update.

# Patch [2019-05-25]
- Added automatic sweeping to the Discord bot to save on memory usage.
- Patched kill counter to lower memory usage.
- Changed the style of the arrows of the drop-downs in the <a href="#options-menu">options menu</a>.

# Balance [2019-05-24]
- Buffed Hewn Double recoil and bullet stats.

# Patch [2019-05-24]
- Added notification to press N if player hasn't press N 30 seconds after spawning.

# Balance [2019-05-21]
- Nerfed Necromancer drone speed.
- Made Necromancer guard drones turn squares into regular drones instead of other guard drones.

# Balance [2019-05-20]
- Buffed shotgun damage and bullet speed.
- Nerfed Necromancer drone stats.

# Balance [2019-05-11]
- Nerfed Overdrive drone stats.
- Made squares lose score upon becoming Necromancer drones.

# Balance [2019-05-08]
- Nerfed Sniper reload slightly.

# Event [2019-05-05]
- The event has ended.
- For now, the event mode is replaced with regular Open 3TDM.

# Update [2019-05-04]
- The Glitch server is now visible only when the Heroku server is not available.
- Added tooltips when upgrading to Stalker, Manager, Landmine, Falcon, Eagle, Maleficitor, or Predator.

# Patch [2019-05-03]
- Added lines to indicate branching on the class tree, press T to show.

# Event [2019-05-03]
- Replaced 4 Team Domination for an event mode.
- In remembrance of <i>Freedom 7</i>, the first manned United States spaceflight which was on May 5, 1961, the event gamemode is Space 3TDM!
- We'll likely have more events in the future!

# Balance [2019-05-02]
- Buffed the engine acceleration stats of Smashers.

# Patch [2019-05-01]
- Fixed bug that made the servers crash.

# Gamemode [2019-04-30]
- Added a new server in Asia, also sponsored by <a target="_blank" href="https://www.serverhunter.com/?utm_source=arrasio&utm_medium=button&utm_content=website">Server Hunter</a>.
- Added Maze on the server.
- Changed the OVH Europe server to FFA.

# Gamemode [2019-04-20]
- Added a new US East server from OVH sponsored by <a target="_blank" href="https://www.serverhunter.com/?utm_source=arrasio&utm_medium=button&utm_content=website">Server Hunter</a>.
- Added Maze 2TDM on the server.

# Patch [2019-04-14]
- Made the team selector use only the number of teammates you have instead of also the team's total score.
- Fixed bug with death effect.

# Gamemode Poll [2019-04-12] What FFA gamemodes do you like?
- 7, 2019-04-15T00:00:00.000Z, any
- FFA With Maze
- FFA Without Maze

# Gamemode Poll [2019-04-12] What team gamemodes do you like?
- 6, 2019-04-15T00:00:00.000Z, table
- |::<b>With Maze</b>|::<b>No Maze</b>
- <b>Number of Teams<b> |<b>2</b>|<b>3</b>|<b>4</b>|<b>2</b>|<b>3</b>|<b>4</b>
- TDM             |X| |X|X| |X
- Domination      |X| |X|X| |X
- Mothership      |X| |X|X| |X
- Open TDM        |X|X|X|X|X|X
- Open Domination |X|X|X|X|X|X

# Balance [2019-04-12]
- Fixed Overdrive drone's reload.
- Fixed polls not loading.
- Balance changes.

# Patch [2019-04-11]
- Improved the encoding algorithm used by the server to lower bandwidth usage.

# Update [2019-04-08]
- Switched from Hetzner to OVH for the European server.
- This server is also sponsored by <a target="_blank" href="https://www.serverhunter.com/?utm_source=arrasio&utm_medium=button&utm_content=website">Server Hunter</a> and should be faster.

# Gamemode [2019-04-05]
- Added a new European server from Hetzner.
- Thanks to <a target="_blank" href="https://www.serverhunter.com/?utm_source=arrasio&utm_medium=button&utm_content=website">Server Hunter</a> for sponsoring it!
- Currently the gamemode is Maze but I'll likely change the gamemode for it in the future.
- I've replaced the link for <a href="https://arras-lb.glitch.me/donate">Stripe donation</a>, but you can still access it.

# Balance [2019-04-03]
- Nerfed spike bounce ability.

# Update [2019-04-01]
- Added spike bounce ability, touch another spike to activate.

# Gamemode [2019-03-29]
- Replaced the expired OpenShift FFA server with Glitch.

# Update [2019-03-26]
- Removed client side extrapolation and used it for interpolation.

# Gamemode [2019-03-24]
- Added back 4 Team Domination.
- Note that as there's only one DVI server, this is ran on the same server.
- I'll likely change the gamemode for it in the near future.

# Update [2019-03-23]
- Changed bases of 2TDM to go from top to bottom instead of only the center.
- Added Spawner, branches off of Director.
- Made Factory branches off of Spawner.
- Added Auto-Spawner, branches off of Spawner.

# Update [2019-03-19]
- Made traps more accurate.
- Removed WeDeploy as it had been discontinued.
- Fixed a memory leak that caused server crashes, credit to <a target="_blank" href="https://www.reddit.com/user/CrazyDave2345">&.</a> for telling me to investigate it.
- Fixed another bug that caused the server to crash due to the Discord Bot.

# Gamemode [2019-03-10]
- Swapped gamemodes of DVI FFA and Heroku 2TDM.

# Gamemode [2019-03-09]
- Replaced 3 Team Maze Domination on Heroku with 2TDM.

# Update [2019-03-04]
- Added Twister, branches off of Artillery.
- Nerfed ram stats.
- Added new messages on the loading screen.

# Gamemode [2019-02-04]
- Replaced the FFA Glitch with a different FFA server.

# Gamemode [2019-02-01]
- Added a second FFA server on US West.

# Update [2019-01-21]
- Added a class tree, press T to show.
- Added some more links to the left menu.

# Gamemode Poll [2019-01-21] What team gamemodes do you like?
- 5, 2019-01-28T00:00:00.000Z, table
- |::<b>With Maze</b>|::<b>No Maze</b>
- <b>Number of Teams<b> |<b>2</b>|<b>3</b>|<b>4</b>|<b>2</b>|<b>3</b>|<b>4</b>
- TDM             |X| |X|X| |X
- Domination      |X| |X|X| |X
- Mothership      |X| |X|X| |X
- Open TDM        |X|X|X|X|X|X
- Open Domination |X|X|X|X|X|X

# Gamemode [2019-01-16]
- Removed the OpenShift servers from the server list as they have expired.

# Update [2019-01-04]
- Added a new server selector.

# Patch [2019-01-04]
- Replaced the old outdated Twitter feed with a referral link to Linode.

# Gamemode [2019-01-03]
- Added a WeDeploy server, currently the gamemode is 4TDM.

# Gamemode [2019-01-02]
- Added back 3 Team Maze Domination as the bandwidth have reset!
- Added a BuyVM server, currently the gamemode is Maze Mothership.
- Added party links.

# Update [2018-12-31]
- Renamed Hurricane to Cyclone.
- Added Overdrive, upgraded from Overseer.

# Update [2018-12-29]
- Added a slightly better server selector.
- Thanks to all the donors of the game, we now have a new domain for the game at <a target="_blank" href="http://arras.io/">arras.io</a>!

# Update [2018-12-26]
- Replaced the free Codeanywhere server with a Heroku server.

# Update [2018-12-25]
- Merry Christmas everyone! I've put up some Christmas lights around the maze walls.
- Added Hurricane, upgraded from Hexa Tank and Gunner.
- I've temporarily replaced the old US West server with a free OpenShift server, and the Europe server with a free Codeanywhere server.

# Balance [2018-12-24]
- More balance changes.
- Servers now restart every 10 hours, unless someone in the server have a high score.

# Patch [2018-12-23]
- Unfortunately, the Europe server ran out of bandwidth for this month.
- Added the <a href="https://arras-lb.glitch.me/donate">one-time donation link</a> to the options menu.
- Various balance changes.

# Gamemode [2018-12-22]
- The OpenShift servers seems to have been experiencing some severe connection issues.
- Fortunately, today I've received a $50 donation. Donations like these will keep the game going, and even the smallest donations can still have an impact.
- In the meantime, I've added 4 Glitch servers. Although they might be slow, it's hopeful better than nothing.

# Gamemode [2018-12-17]
- Unfortunately, the US West server ran out of bandwidth for this month.
- OpenShift has announced that from now on, old projects will expire in 30 days, while new projects expire 60 days after they were created. This means that from now on, the US East servers will need to be remade every two month.
- In order to keep all of the servers up at all time, you can donate to me on <a href="https://www.patreon.com/arras">Patreon</a> so I can upgrade the US West server to have more bandwidth.
- I will likely be adding a one-time donation page in the near future.

# Update [2018-12-14]
- Invisible tanks can now see themselves when invisible.
- Fixed bug where the "Press N to level up instantly." is falsely shown.
- Optimized the servers so hopefully they will be less laggy.

# Patch [2018-12-10]
- Fixed bug where traps don't get killed while touching walls.

# Update [2018-12-09]
- Added option to separate health and shield bars. Click on the view options button to access.
- You now respawn with half of your points.

# Update [2018-12-08]
- Predator can now right click to move its camera.

# Patch [2018-11-29]
- Fixed minimap to make tank movements buttery smooth.
- Fixed bug where FFA reverted to Maze.

# Gamemode [2018-11-28]
- Replaced Open 3TDM with 3 Team Maze Domination!

# Gamemode [2018-11-27]
- Ended Thanksgiving event.
- Replaced 4 Team Domination with 4TDM.
- Replaced Maze with FFA.
- 3 Team Maze Domination coming very soon!

# Event [2018-11-23]
- Happy Thanksgiving to everyone! I know it's a day too late, but I've made the motherships look like turkeys.

# Update [2018-11-23]
- Add 2 Team Maze Mothership in the new US West server.
- In this gamemode, there will <i>not</i> be an insta-level up button, but there are more polygons, and the polygons give more points.
- The motherships need to navigate the maze and fight the opposing teams.
- Due to technical difficulties, I was unable to update the gamemode for the US East servers.

# Update [2018-11-20]
- Traps now die instantly when touching walls.
- I'll be changing the gamemodes once I get the maps ready.

# Patch [2018-11-18]
- Added a <a target="_blank" href="http://arras-proxy.surge.sh/">proxy server</a> because some people can't seem to play on the new link.
- Fixed minimap maze color for Midnight theme.
- I will update the gamemodes pretty soon.
- Unfortunately, I screwed up while reading the database and accidentally erased people who voted for all three of the winning team gamemodes which are:
- 4TDM
- 2 Team Maze Mothership
- 3 Team Open Maze Domination

# Gamemode Poll [2018-11-12] What flavor of FFA do you like?
- 4, 2018-11-18T00:00:00.000Z, any
- FFA With Maze
- FFA Without Maze

# Gamemode Poll [2018-11-12] What team gamemodes do you like?
- 3, 2018-11-18T00:00:00.000Z, table
- |::<b>With Maze</b>|::<b>No Maze</b>
- <b>Number of Teams<b> |<b>2</b>|<b>3</b>|<b>4</b>|<b>2</b>|<b>3</b>|<b>4</b>
- TDM             |X| |X|X| |X
- Domination      |X| |X|X| |X
- Mothership      |X| |X|X| |X
- Open TDM        |X|X|X|X|X|X
- Open Domination |X|X|X|X|X|X

# Gamemode [2018-11-06]
- Replaced FFA with Maze!
- FFA except there are walls everywhere.

# Patch [2018-11-05]
- Added back minimap to show teammates and bosses, credit to <a target="_blank" href="https://www.reddit.com/user/CrazyDave2345">&.</a> for helping to make it faster.
- Note that if you own a private server, you need to go <a target="_blank" href="https://glitch.com/edit/#!/arras-template?path=CHANGELOG.md">here</a> to keep your server working!

# Balance [2018-11-03]
- Nerfed Musket and Carrier.

# Update [2018-10-31]
- Added Pumpkin Skeleton theme, credit to <a target="_blank" href="https://www.reddit.com/user/Road-to-100k">Road</a>.
- You can find the theme by clicking view options. It's recommended for it to be used with glass mode.
- Added Musket, upgraded from Twin and Rifle.
- Thanks to our patrons on <a href="https://www.patreon.com/arras">Patreon</a>, we now have a new domain for the game at <a target="_blank" href="http://arras.cx/">arras.cx</a>!

# Gamemode [2018-08-22]
- Added Open 3TDM to replace 2TDM.
- Added 4 Teams Domination to replace Mothership.
- Take over three of the five dominators to win the game!

# Patch [2018-08-20]
- Added Malefictor, upgraded from Underseer.
- Press R to stop the drones from moving and turn invisible.
- Added Swarmer, upgraded from Destroyer.

# Gamemode Poll [2018-08-22] Which flavor of Domination do you like the most?
- 2, 2018-08-23T00:00:00.000Z, any
- 2 Teams Domination
- 4 Teams Domination

# Gamemode Poll [2018-08-22] Which flavor of Open TDM do you like the most?
- 1, 2018-08-23T00:00:00.000Z, any
- Open 2TDM
- Open 3TDM
- Open 4TDM

# Gamemode Poll [2018-08-19] What gamemodes do you like?
- 0, 2018-08-22T00:00:00.000Z, any
- FFA
- TDM
- Open TDM (TDM without bases)
- Domination
- Mothership

# Balance [2018-08-17]
- Added more base protectors to 2TDM.
- Increased map size of all gamemodes.
- Nerfed Triplet.
- Buffed Mega-3.
- Buffed Spreadshot.
- Nerfed drone range again.
- Buffed Booster acceleration but nerfed speed.

# Balance [2018-08-16]
- Changed AIs' range mechanism.
- Nerfed drone range.

# Gamemode [2018-08-07]
- Removed 4TDM to help with lag issues because the server is less powerful than expected.

# Gamemode [2018-08-06]
- Added 2TDM and 4TDM!
- Thanks to our patrons on <a href="https://www.patreon.com/arras">Patreon</a> for allowing us a second server!

# Update [2018-08-06]
- Added mobile support!
- Various balance changes.

# Update [2018-07-19]
- Added Rifle, upgraded from Sniper.

# Gamemode [2018-07-12]
- Nerfed Shotgun.
- Replaced Portal 4TDM.
- Added experimental gamemode Portal Mothership.
- Motherships spawn on opposing sides of the arena.
- In the middle of both is a portal.
- Motherships cannot enter the portal.

# Gamemode [2018-07-04]
- Replaced 4TDM.
- Added experimental gamemode Portal 4TDM.
- One side of the arena is normal 4TDM, the other is Maze 4TDM.
- In the middle of both is a portal.
- Nerfed certain bosses.

# Patch [2018-07-03]
- Added Eagle, branches off of Tri-Angle and Pounder.
- Moved Shotgun to upgrade from Pounder at Tier 3.
- Moved Conqueror to upgrade from Destroyer instead.
- Changed Conqueror to have Destroyer bullet but with slower reload.

# Patch [2018-07-02]
- Renamed Hepta-Trapper to Septa-Trapper.
- Renamed Snipe Guard to Bushwhacker again.
- Renamed Tri-Builder to Architect.

# Update [2018-07-02]
- Various balance changes.
- If you have a private server, make sure to go <a target="_blank" href="https://glitch.com/edit/#!/arras-template?path=CHANGELOG.md">here</a> and do the required changes to keep your server working!

# Patch [2018-06-29]
- Moved Bulwark, now upgraded from Twin and Trap Guard.
- Optimizations so the servers will be less laggy.
- You should now be able to restore scores even after disconnections from server overloading.

# Gamemode [2018-06-28]
- Increased knockback.
- Changed FFA Maze back to FFA.
- Added Bulwark, upgraded from Twin and Trapper.
- Added a limit on how fast you can respawn.

# Patch [2018-06-26]
- Prediction & other debug information are now hidden unless L is pressed.
- Latency and client/server speed are still visible by default.

# Update [2018-06-26]
- Added Trapper, upgraded from Basic.
- Added Tri-Trapper, branches off of Trapper.
- Added Hepta-Trapper, branches off of Tri-Trapper.
- Added Auto-Overseer, branches off of Overseer.
- Made Conqueror barrels smaller.
- Renamed old Trapper to Builder.
- Changed class tree.

# Patch [2018-06-26]
- Fixed bug where leaderboard and upgrade choices are missing.
- Reverted keybinds due to confusion. Click view options for detail.

# Balance [2018-06-25]
- Buffed Skimmer range.

# Patch [2018-06-25]
- Made upgrades three columns.
- Change keybinds. Click view options for detail.

# Gamemode [2018-06-25]
- Replaced Mothership with Maze.
- FFA except there are obstacles everywhere.
- Added Pounder, upgraded from Basic.
- Moved classes like Trappers and Destroyers to upgrade from it.

# Patch [2018-06-24]
- Fixed UI zoom glitch.
- Added <a href="https://www.patreon.com/arras">Patreon</a> page!

# Balance [2018-06-23]
- Nerfed Trappers.
- Increased knockback.

# Balance [2018-06-11]
- Nerfed Mothership.
- Nerfed Barricade.

# Gamemode [2018-06-11]
- Temporarily replaced FFA with Mothership.
- Destroy the enemy mothership to win the game!

# Update [2018-06-11]
- Tanks now lasts for 15 seconds instead of 8 seconds if a player disconnected.
- Added experimental tank recovery feature.
- If a player joins back during the time before the tank is automatically destroyed, they will spawn as their old tank.

# Update [2018-06-11]
- Buffed bosses.
- Added Barricade, upgraded from Minigun and Trapper.

# Gamemode [2018-06-03]
- Changed the 2 TDM Domination server to 4 TDM.
- Added M for maximizing stats.
- Added Z to toggle video recorder.

# Update [2018-06-02]
- Added Tri-Trapper, upgraded from Trapper and Flank Guard.
- Added Conqueror, upgraded from Trapper and Destroyer.
- Added Auto-Assassin, upgraded from Assassin.

# Balance [2018-06-02]
- Nerfed drones.
- Nerfed Manager.
- Gunner-Trapper now branches off from Gunner, Trapper, and Trap Guard.
- Doubled the size of the arena.
- Buffed bosses.
- Made bosses spawn more often.

# Update [2018-06-01]
- Added Manager, upgraded from Director.
- Added Stalker, upgraded from Assassin.
- Added Landmine, upgraded from Smasher.

# Gamemode [2018-05-21]
- Changed the 4 TDM Domination server to 2 TDM Domination.

# Balance [2018-05-21]
- Nerfed Health Regen and Body Damage effectiveness.
- Buffed drones slightly.

# Balance [2018-05-08]
- Nerfed Carrier.

# Balance [2018-05-07]
- Nerfed ram stats.
- Nerfed Booster health.
- Buffed Sniper bullet speed slightly.

# Patch [2018-05-05]
- Moved base protectors in Domination to the edge of the map.
- Various nerfs and buffs.

# Gamemode [2018-04-29]
- Replaced 4TDM with 4 Team Domination.
- Take over 3 dominator to win the game!
- Replaced 2TDM with FFA.

# Update [2018-04-28]
- Added Banshee, upgraded from Auto-3 and Overseer.

# Gamemode [2018-04-04]
- Added 2 TDM gamemode with server selector.

# Update [2018-04-01]
- Added Master.

# Patch [2018-03-31]
- Added back pushing for same team.
- Limited drone repel distance.

# Update [2018-03-31]
- Prevented drones from pushing.
- Engineers' oldest turret will now disappear when there are more than six turrets.

# Update [2018-03-31]
- Added back pushing for same team.
- Limited drone repel distance.

# Update [2018-03-30]
- Added reverse mouse and reverse tank keys.
- Various nerfs and buffs.

# Update [2018-03-29]
- Added Auto-Cruiser, upgraded from Cruiser.
- Added Surfer, upgraded from Tri-Angle.
- Added Cropduster, upgraded from Minigun.

# Balance [2018-03-29]
- Nerfed Dual damage slightly.

# Balance [2018-03-26]
- Nerfed swarm drones reload.
- Buffed most tanks reload slightly.

# Balance [2018-03-25]
- Buffed base protectors.
- Nerfed shields again.

# Patch [2018-03-25]
- Added base protectors.
- Nerfed shields.

# Patch [2018-03-25]
- Arena is now 2x bigger.
- Shapes spawn 62% less often.
- Bases added in corners.

# Balance [2018-03-23]
- Nerfed sentries.

# Update [2018-03-22]
- Arras is back!
- Removed tokens.

# Older Changelogs
- <a href="ext/changelog.html">View the original changelog here.</a>
`);
    let Rc = a => {
            try {
                var b = a.replace(/\s+/g, "");
                2 == b.length % 4 ? b += "==" : 3 == b.length % 4 && (b += "=");
                let n = atob(b);
                b = "Unknown Theme";
                let r = "";
                var c = n.indexOf("\x00");
                if (-1 === c) return null;
                b = n.slice(0, c) || b;
                n = n.slice(c + 1);
                c = n.indexOf("\x00");
                if (-1 === c) return null;
                r = n.slice(0, c) || r;
                n = n.slice(c + 1);
                let z = n.charCodeAt(0) / 255;
                n = n.slice(1);
                let x = Math.floor(n.length / 3);
                if (2 > x) return null;
                c = [];
                for (let A = 0; A < x; A++) c.push("#" + (n.charCodeAt(3 * A) << 16 | n.charCodeAt(3 * A + 1) << 8 | n.charCodeAt(3 * A + 2)).toString(16).padStart(6, "0"));
                return {
                    qc: !1,
                    name: b,
                    hb: r,
                    content: {
                        table: c,
                        border: z
                    }
                }
            } catch (n) {}
            a = JSON.parse(a);
            if ("object" !== typeof a) return null;
            let {
                name: f = "Unknown Theme",
                author: e = "",
                content: h
            } = a;
            a = [];
            for (let n of [h.teal, h.lgreen, h.orange, h.yellow, h.lavender, h.pink, h.vlgrey, h.lgrey, h.guiwhite, h.black, h.blue, h.green, h.red, h.gold, h.purple, h.magenta, h.grey, h.dgrey, h.white, h.guiblack]) {
                if ("string" !== typeof n || !/^#[0-9a-fA-F]{6}$/.test(n)) return null;
                a.push(n)
            }
            return {
                qc: !0,
                name: "string" === typeof f && f.trim() || "Unknown Theme",
                hb: "string" ===
                    typeof e && e.trim() || "",
                content: {
                    table: a,
                    border: Math.min(1, Math.max(0, h.border))
                }
            }
        },
        Sc = "TGlnaHQgQ29sb3JzAE5lcGgApnrbvLnofueJbf3zgLWO/e+Zw+jr96qfnv///0hISDyky4q8P+A+Qe/HS41q38xmnKenr3Jvb9vb2wAAAA RGFyayBDb2xvcnMATmVwaAAmiXW3DEkdxGdIsrIkfVbFsk+uHh4ePDo6AAAA5eXlN5/GMLU7/2xu/8ZllnPoyGebY19fc3R6EREP//// TmF0dXJhbABOZXBoADN2wbuq013glUX/2ZOTn//Yf7LEtrZ/f3////83ODRPk7UAtlnhT2Xlv0KAU6C2fKqZj49JSVSlsqUAAAA Q2xhc3NpYwBOZXBoAICO//uF4338dnb/6461jv/xd93Nzc2ZmZn///9SUlIAsOEA4GzwT1T/5Gt2jPy+f/WZmZlUVFTAwMAAAAA Rm9yZXN0AFN0ZXJsb24As4hKpYybPtFqgJdZbUmYVWApT93GuH6Unv//6GZXUIB7tqG+VeWwW/9HR7rGdLp40ZmIZlKXWH2gYAAAAA TWlkbmlnaHQAdW9pZWEAmSuQmEuqXTRWeM3GhIl3jqhckMzMzKeyt7rG/wkfKBI0VQmHZQAAE1ZjgXQ3hLKQmFVVVWSet0RERAAAAA U25vdwBEZW9sdmVvcG9sZXIAWYm/urXRfeXg4LW75ZOf/2Rt5bKysn9/f////zg4Na6u/67/rv+urv///8PD2P+1/8zMzKCgsvLy8gAAAA Q29yYWwgUmVlZgBDZWxlc3RlYQBMdu7GQap4/39Q/9JQ3DOI+oByi4iGv8HC////EkZrQgCuDWM43EMz/qkEe0urXCRuZWiE1NfZMoO8AAAA QmFkbGFuZHMASW5jb2duaW91cwBm+cuc8cIyOHYd5pE4t7e3eIZraqhPt7e3pML0AAAADFqebokiWwAAeD8EWRx3IBJNLxwWmZmZVDUXz+Lz QmxlYWNoAGRlZmluaXRlbHlub3QuAGYA//8A/wD/MgD/7AD/JKf/PL3/8YaRgYHx8fFfX18AJf8A/wD/AAD/+iMxAP/U09ODg4NMTEz//v4ICAg UHVtcGtpbiBTa2VsZXRvbgBSb2FkAP9yGXD/Y0cbcTr984CUEQAZRBcbcTqqn57+2LFISEg8pMt27sbwT1QbcTobcTrMZpz///9yb2//m1gAAAA UmV0cm8ARGFtb2NsZXMAwP//Yga/OzGOlZN9AO7o1ecsdsjY5wc2Qv///wAAACw+ue+yCbkSNILf5B5haosSTIOUlna2iwgeIAAAAA UGFzdGVsAERhbW9jbGVzAMD//5iK/7LYs4T/63bu6NX/jb3z8NcHNkL//7VnUkA5dpc/96H2X2TSuGmBZUqnUpnBwcGNqZb126cAAAA RGlzY29yZABEYW1vY2xlcwDA1T8/KbOZ/ygo/+uOtY7//2j/zc3NmZmZ4ODgAAAAconaQ7WB8EdH/+gAXHn/+qQZmZmZVFRUHiEkNjk+ V1IgU2hlZXQgVGhlbWUAYWxldHRlcmEATP2YJ2iRaLuOdfXeup6BcbNXV+rgyaqfnv///3lVSGuyv2jFbLhjWNi8Z254qoRhh4aHiHJvb8u2kAAAAA RGVzY2VudABSb2c0NTYAAJXOz/+l/7yJif//sgAAAL2RxH9zYAAAAP///wAAAKurq6aXaFRUVP3npYeHd4WmhsS7nS4uMjs7NwAAAA U29sYXJpemVkIERhcmsAUm9hZADAtYkAKqGYy0sWZXuD7ujV0zaC4OLkBzZC////AAAAJovShpYA3DIvtYkAZ4yxoIK9g5SWBzZCACs2AAAA RWdncGxhbnQAUm9hZADA6WuoeNS21hAPo56b5+nb6WuojYaHKxop////KxopBrbvSLaF72FV+ZsVgVuk/sQYubawQBE/UDdNAAAA".split(" "),
        Jb = Rc(Sc[0]).content,
        Tc = document.getElementById("optColors"),
        Uc = document.getElementById("optCustom");
    za("optCustom");
    let fd = ({
        name: a,
        hb: b
    }, c = !1) => {
        a = c ? a ? `Custom - ${a}` : "Custom" : a;
        return "" === b ? a : "fan-made" === b ? `${a} (Fan-made)` : `${a} (by ${b})`
    };
    for (let a of Sc) {
        let b = fd(Rc(a));
        Tc.options.add(new Option(b, a, Tc.options))
    }
    let gd = new Option("Custom", "custom"),
        hd = () => {
            let a = {
                name: "",
                hb: ""
            };
            try {
                Uc.value && (a = Rc(Uc.value));
                Uc.classList.remove("error");
                if (a.qc) {
                    let {
                        name: b = "Unknown Theme",
                        hb: c = "",
                        content: f
                    } = a, {
                        table: e,
                        border: h
                    } = f, n = b.trim() + "\x00" + c.trim() + "\x00" + String.fromCharCode(1 <= h ? 255 : 0 > h ? 0 : Math.floor(256 * h));
                    for (let r of e) {
                        let z = parseInt(r.slice(1), 16);
                        n += String.fromCharCode(z >> 16, z >> 8 & 255, z & 255)
                    }
                    Uc.value = btoa(n).replace(/=+/, "")
                }
                gd.text = fd(a, !0)
            } catch (b) {
                Uc.classList.add("error"), gd.text = "Custom"
            }
        };
    hd();
    Tc.options.add(gd);
    Tc.addEventListener("change", () => {
        Uc.style.display = "custom" === Tc.value ? "block" : "none"
    });
    Tc.addEventListener("focus", () => {
        Uc.style.display = "custom" === Tc.value ? "block" : "none"
    });
    Uc.addEventListener("input", () => hd());
    za("playerNameInput");
    za("optScreenshotMode");
    za("optShield");
    za("optFancy");
    za("optColors");
    za("optNoPointy");
    za("optBorders");
    za("optAutoLevel", d.mobile);
    d.mobile && za("optMobile");
    0 > Tc.selectedIndex && (Tc.selectedIndex = 0);
    d.mobile && "" === document.getElementById("optMobile").value && (document.getElementById("optMobile").value = "joysticks");
    "" === document.getElementById("optBorders").value && (document.getElementById("optBorders").value = "normal");
    if (!d.mobile) {
        let a = {};
        try {
            if ("#vi" === location.hash || "#vim" === location.hash) a = {
                KEY_ABILITY: ["N", 78],
                KEY_AUTO_FIRE: [";", 186],
                KEY_AUTO_SPIN: ["P", 80],
                KEY_CHOOSE_1: ["Q", 81],
                KEY_CHOOSE_2: ["W", 87],
                KEY_CHOOSE_3: ["E", 69],
                KEY_CHOOSE_4: ["A", 65],
                KEY_CHOOSE_5: ["S", 83],
                KEY_CHOOSE_6: ["D", 68],
                KEY_CHOOSE_7: ["Z", 90],
                KEY_CHOOSE_8: ["X", 88],
                KEY_CHOOSE_9: ["C", 67],
                KEY_CLASS_TREE: ["T", 84],
                KEY_DOWN: ["J", 74],
                KEY_LEFT: ["H", 72],
                KEY_LEVEL_UP: [".", 190],
                KEY_OVERRIDE: ["I", 73],
                KEY_PAUSE: ["B", 66],
                KEY_PING: [",", 188],
                KEY_RECORD: ["V",
                    86
                ],
                KEY_REVERSE_MOUSE: ["U", 85],
                KEY_REVERSE_TANK: ["Y", 89],
                KEY_RIGHT: ["L", 76],
                KEY_SCREENSHOT: ["G", 71],
                KEY_UP: ["K", 75]
            };
            else {
                let y = Gb.getItem("keybindsJSON");
                a = y ? JSON.parse(y) : {}
            }
        } catch (y) {}
        let b = document.getElementById("controlTable"),
            c = document.getElementById("resetControls"),
            f = document.getElementById("moreControls"),
            e = null,
            h = [];
        for (let y of b.rows)
            for (let g of y.cells) {
                let l = g.firstChild.firstChild,
                    {
                        key: p,
                        K: v
                    } = l.dataset,
                    t = {
                        element: l,
                        key: p,
                        K: v,
                        ja: l.innerText,
                        X: d[p],
                        Ob: l.innerText,
                        Nb: d[p]
                    };
                a[t.key] &&
                    (l.innerText = t.ja = a[t.key][0], d[p] = t.X = a[t.key][1], t.K && (d.K[t.K] = t.ja));
                h.push(t)
            }
        let n = () => h.some(({
            X: y,
            Nb: g
        }) => y !== g);
        n() && c.classList.add("active");
        let r = () => {
                window.getSelection && window.getSelection().removeAllRanges();
                e.element.parentNode.parentNode.classList.remove("editing");
                e = null
            },
            z = y => {
                e = y;
                e.element.parentNode.parentNode.classList.add("editing");
                if (-1 !== e.X && window.getSelection) {
                    y = window.getSelection();
                    y.removeAllRanges();
                    let g = document.createRange();
                    g.selectNodeContents(e.element);
                    y.addRange(g)
                }
            },
            x = (y, g) => {
                if (" " === y) y = "", g = -1;
                else if (g !== e.X) {
                    let l = h.find(({
                        X: p
                    }) => p === g);
                    l && (l.ja = e.ja, l.element.innerText = e.ja, l.X = e.X, d[l.key] = e.X, l.K && (d.K[l.K] = e.ja), a[l.key] = [l.ja, l.X])
                }
                e.ja = y;
                e.element.innerText = y;
                e.X = g;
                d[e.key] = g;
                e.K && (d.K[e.K] = y);
                a[e.key] = [e.ja, e.X];
                Gb.setItem("keybindsJSON", JSON.stringify(a));
                r();
                n() ? (c.classList.remove("spin"), c.classList.add("active")) : c.classList.remove("active")
            };
        document.addEventListener("click", y => {
            if (!d.ya)
                if (e) r();
                else {
                    var g = h.find(({
                            element: l
                        }) => y.target ===
                        l);
                    g && z(g)
                }
        });
        c.addEventListener("click", () => {
            if (n()) {
                e && r();
                for (let y of h) y.ja = y.Ob, y.element.innerText = y.Ob, y.X = y.Nb, d[y.key] = y.Nb, y.K && (d.K[y.K] = y.Ob);
                a = {};
                Gb.setItem("keybindsJSON", JSON.stringify(a));
                c.classList.remove("active");
                c.classList.add("spin")
            }
        });
        let A = null;
        f.addEventListener("click", () => {
            if (A) {
                for (let y of A) y.classList.add("hidden");
                A = null;
                f.classList.remove("x")
            } else {
                A = document.querySelectorAll("#controlTable tr.hidden");
                for (let y of A) y.classList.remove("hidden");
                f.classList.add("x")
            }
        });
        document.addEventListener("keydown", y => {
            if (!(d.ya || y.shiftKey || y.ctrlKey || y.altKey)) {
                var g = y.which || y.keyCode;
                e ? 1 !== y.key.length || /[0-9ofg`]/i.test(y.key) || 3 === y.location ? "Backspace" !== y.key && "Delete" !== y.key || x(" ", 32) : x(y.key.toUpperCase(), g) : g !== d.KEY_ENTER && g !== d.KEY_SPAWN || id()
            }
        })
    }
    document.getElementById("startButton").addEventListener("click", () => id());
    let jd = WebSocket.prototype.close;
    WebSocket.prototype.close = function(...a) {
        Ka.Y = !0;
        jd.call(this, ...a)
    };
    WebSocket.prototype.close.toString = Function.prototype.toString.bind(jd);
    document.body.addEventListener("mouseenter", () => La(!0));
    document.body.addEventListener("mouseleave", () => La(!1));
    document.addEventListener("contextmenu", a => {
        "A" !== a.target.tagName && "INPUT" !== a.target.tagName && a.preventDefault()
    });
    d.mobile && document.body.classList.add("mobile");
    let bb = new db,
        kd = () => {
            let a = window.devicePixelRatio;
            w.o.Za && (a *= .5);
            d.Ec = a;
            bb.h.width = d.i = window.innerWidth * a;
            bb.h.height = d.j = window.innerHeight * a;
            d.rb = Math.min(1920, Math.max(window.innerWidth, 1280))
        };
    kd();
    window.addEventListener("resize", kd);
    let k = bb.h.getContext("2d"),
        ld = (a, b, c) => {
            c += w.o.Ra;
            let f = 2 * zc();
            return (a + c) * f > -d.i && (a - c) * f < d.i && (b + c) * f > -d.j && (b - c) * f < d.j
        },
        md = (a, b) => {
            b += w.o.Ra;
            let c = 2 * zc();
            return Math.max(0, Math.min(1, 2 + (-a + d.i / c) / b, 2 + (a + d.i / c) / b))
        };

    function nd(a) {
        a.u = 0;
        a.value = 0;
        a.time = 0;
        a.h = 0
    }

    function od(a) {
        var b = J.ea;
        return 2 !== b.h ? 0 : (a - b.time) / (1E3 / 30 / I.speed)
    }
    const pd = class {
        constructor(a = !1) {
            this.H = a;
            this.h = this.time = this.value = this.u = 0
        }
        add(a) {
            this.u = 0 < this.h ? this.value : a;
            this.value = a;
            this.time = 0;
            this.h = 1
        }
        get(a, b = !1) {
            if (0 === w.Da.bb) return this.value;
            if (0 === this.h) return 0;
            if (1 === this.h) return this.h = 2, this.time = a, this.u;
            a -= this.time;
            var c = 1E3 / 30 / I.speed;
            2 === w.Da.bb ? (b = a / c, b = 8 < b ? 8 : 1 < b ? b : b * b * (3 - 2 * b)) : b = a < c || b ? a / c : 1;
            a = this.u;
            c = this.value;
            if (this.H) {
                var f = 2 * Math.PI;
                c = ((c - a + Math.PI) % f + f) % f - Math.PI
            } else c -= a;
            return a + c * b
        }
    };
    let J = {
        x: 0,
        y: 0,
        view: 1,
        g: {
            x: 0,
            y: 0,
            view: 2E3
        },
        ea: new pd,
        ka: new pd,
        name: ""
    };
    d.player = J;

    function qd(a) {
        var b = rd;
        b.enabled = !0;
        b.x = a.x;
        b.y = a.y;
        b.M = a.M;
        b.J = a.J
    }

    function sd(a) {
        var b = rd;
        b.x += a.Yc;
        b.y += a.Zc;
        b.M += a.Wc;
        b.J += a.Xc
    }
    const rd = {
            enabled: !1,
            x: 0,
            y: 0,
            M: 0,
            J: 0,
            isEnabled() {
                return d.N || d.la || !d.Aa ? !1 : this.enabled && 0 < uc && 0 < vc
            },
            get() {
                return this
            }
        },
        ud = (() => {
            const a = Hb();
            let b = () => {
                    var g = document.createElement("canvas");
                    g.width = 96;
                    g.height = 32;
                    let l = g.getContext("2d");
                    l.fillStyle = "#ffffff";
                    l.fillRect(64, 0, 32, 32);
                    l.fillStyle = "#8abc3f";
                    l.beginPath();
                    l.arc(16, 16, 16, 0, 2 * Math.PI);
                    l.fill();
                    l.strokeStyle = "#ffffff";
                    l.lineJoin = "round";
                    l.lineWidth = 1.6;
                    l.beginPath();
                    l.moveTo(16.6, 10.2);
                    l.lineTo(21.4, 5.4);
                    l.lineTo(26.6, 10.6);
                    l.lineTo(21.8,
                        15.4);
                    l.closePath();
                    l.stroke();
                    l.beginPath();
                    l.arc(14, 18, 8, 0, 2 * Math.PI);
                    l.stroke();
                    l.fillStyle = "#8abc3f";
                    l.textBaseline = "middle";
                    l.font = '15px "Trebuchet MS"';
                    l.fillText("arras.io", 38, 16);
                    l.fillStyle = "rgb(0,0,0,0.3)";
                    l.fillText("arras.io", 40, 18);
                    g = g.toDataURL().replace("data:image/png;base64,", "");
                    return `v0:${ia(g,"base64")}`
                },
                c = () => navigator.mediaDevices.enumerateDevices().then(g => g.map(l => l.deviceId).filter(l => l)),
                f = () => {
                    let g = [];
                    var l = 0;
                    for (var p of [127, 409, 887, 55, 86, 228, 484, 155, 52, 463, 384, 926,
                            186, 1667, 371, 768, 138, 452, 530, 8, 1, 1, 3, 38, 257, 458, 119, 348, 152, 1261, 373, 947, 103, 8856, 21520, 243, 18387, 2061, 990, 1035, 4, 2
                        ]) l += p, g.push(String.fromCharCode(l));
                    g.push("\ud835\udf90");
                    l = "";
                    p = document.createElement("span");
                    p.style.visibility = "hidden";
                    p.style.font = "initial";
                    p.style.fontSize = "1920px";
                    document.body.appendChild(p);
                    for (let v of "initial sans-serif serif monospace cursive fantasy".split(" ")) {
                        p.style.fontFamily = v;
                        for (let t of g) p.textContent = t, l += `${p.offsetWidth} ${p.offsetHeight}\n`
                    }
                    p.remove();
                    return `v0:${ia(l.slice(0,-1),"latin1")}`
                };
            const e = {
                    index: 0,
                    data: [],
                    next() {
                        if (this.index >= this.data.length) throw console.error(this.data), Error("Trying to crawl past the end of the provided data");
                        return this.data[this.index++]
                    },
                    all() {
                        return this.data.slice(this.index)
                    },
                    ed(g) {
                        this.index += g;
                        if (this.index > this.data.length) throw console.error(this.data), Error("Trying to crawl past the end of the provided data");
                    },
                    set(g) {
                        this.data = g;
                        this.index = 0
                    }
                },
                h = class {
                    constructor() {
                        this.C = []
                    }
                    Va() {
                        return this.C.length
                    }
                    get(g) {
                        for (let l =
                                this.C.length; l <= g; l++) this.C.push({
                            position: 0,
                            ha: 0,
                            Cc: 0,
                            Kb: !1,
                            Jb: null
                        });
                        return this.C[g]
                    }
                    update() {
                        for (let g of this.C)
                            if (g.Kb && (g.Kb = !1, g.ha += g.Cc), g.ha || g.position) g.ha -= .2 * g.position, g.position += g.ha, 0 > g.position && (g.position = 0, g.ha = -g.ha), 0 < g.ha && (g.ha *= .5)
                    }
                    Fc(g, l) {
                        g = this.get(g);
                        null != g.Jb && g.Jb !== l && (g.Kb = !0);
                        g.Jb = l
                    }
                    Gc(g, l) {
                        this.get(g).Cc = Math.sqrt(l) / 20
                    }
                },
                n = class {
                    constructor() {
                        this.h = "normal";
                        this.time = Date.now()
                    }
                    set(g) {
                        if (g !== this.h || "injured" === g) "dying" !== this.h && (this.time = Date.now()), this.h =
                            g
                    }
                    sa() {
                        return "dying" === this.h || "killed" === this.h ? Math.max(0, 1 - (Date.now() - this.time) / 300) : 1
                    }
                    nc() {
                        return "#ffffff"
                    }
                    mc() {
                        "injured" === this.h && 500 < Date.now() - this.time && (this.h = "normal");
                        return "injured" === this.h || "killed" === this.h ? Math.max(0, 1 - (Date.now() - this.time) / 80) : 0
                    }
                },
                r = g => {
                    var l = !1;
                    if (g) {
                        if (e.next() & 0) throw Error("Expecting a turret but found a regular entity");
                        l = null == g.facing;
                        g.facing = e.next();
                        g.Z = e.next()
                    } else {
                        l = e.next();
                        if (l & 1) throw Error("Expecting a regular entity but found a turret");
                        let t =
                            e.next();
                        g = Vb.find(P => P.id === t);
                        var p = null == g;
                        p && (g = {
                            id: t
                        }, Vb.push(g));
                        g.S = !1;
                        g.kb = l & 2;
                        var v = l & 4;
                        g.Xa = l & 8 ? g.Xa || Date.now() : 0;
                        l = g.index;
                        g.index = e.next();
                        l = p || l !== g.index;
                        g.x = e.next();
                        g.y = e.next();
                        e.next();
                        e.next();
                        g.size = e.next();
                        g.facing = e.next();
                        e.next();
                        g.Yb = e.next();
                        g.Z = e.next();
                        g.color = e.next();
                        let K = g.P,
                            C = g.U;
                        g.P = e.next() / 255;
                        g.U = e.next() / 255;
                        p || (g.g.P.set(g.P), g.g.U.set(g.U), g.P < K || g.U < C ? g.g.status.set("injured") : 1 !== g.g.status.sa() && g.g.status.set("normal"));
                        g.alpha = e.next() / 255;
                        v ? (g.name =
                            e.next(), g.s = e.next()) : (g.name = "", g.s = 0);
                        p && (g.g = {
                            Ta: g.kb,
                            x: g.x,
                            y: g.y,
                            f: g.facing,
                            La: new pd,
                            Ma: new pd,
                            lb: new pd(!0),
                            status: new n,
                            P: new Sb(g.P, .1, 1),
                            U: new Sb(g.U, .1, 1)
                        });
                        g.g.La.add(g.x);
                        g.g.Ma.add(g.y);
                        g.g.lb.add(g.facing)
                    }
                    p = e.next();
                    l && (g.C = new h);
                    for (v = 0; v < p; v++) g.C.Fc(v, e.next()), g.C.Gc(v, e.next());
                    p = e.next();
                    if (l)
                        for (g.B = [], l = 0; l < p; l++) g.B.push({});
                    else g.B.length !== p && console.error("Mismatch between data turret number and remembered turret number");
                    for (let t of g.B) r(t)
                },
                z = g => {
                    var l = !1,
                        p = e.next();
                    p & 1 && (g.x = .0625 * e.next(), g.y = .0625 * e.next());
                    p & 2 && (g.facing = e.next() * (Math.PI / 256));
                    if (p & 4) {
                        var v = e.next();
                        g.Yb = v & 3;
                        g.kb = v & 4;
                        g.Xa = v & 8 ? g.Xa || Date.now() : 0
                    }
                    v = !1;
                    if (p & 8) {
                        var t = g.P;
                        g.P = e.next() / 255;
                        g.P < t && (v = !0)
                    }
                    p & 16 && (t = g.U, g.U = e.next() / 255, g.U < t && (v = !0));
                    p & 32 && (g.alpha = e.next() / 255);
                    p & 64 && (g.size = .0625 * e.next());
                    p & 128 && (g.s = e.next());
                    p & 256 && (g.name = e.next());
                    p & 512 && (l = g.index, g.index = e.next(), l = l !== g.index);
                    p & 1024 && (g.color = e.next());
                    p & 2048 && (g.Z = e.next());
                    g.g || g.rc || (g.g = {
                        Ta: g.kb,
                        x: g.x,
                        y: g.y,
                        f: g.facing,
                        La: new pd,
                        Ma: new pd,
                        lb: new pd(!0),
                        status: new n,
                        P: new Sb(g.P, .1, 1),
                        U: new Sb(g.U, .1, 1)
                    });
                    g.g && (g.g.La.add(g.x), g.g.Ma.add(g.y), g.g.lb.add(g.facing), g.g.P.set(g.P), g.g.U.set(g.U), v ? g.g.status.set("injured") : 1 !== g.g.status.sa() && g.g.status.set("normal"));
                    if (!g.C || l) g.C = new h;
                    if (!g.B || l) g.B = [];
                    if (p & 4096)
                        for (;;) {
                            l = e.next();
                            if (-1 === l) break;
                            v = e.next();
                            v & 1 && g.C.Fc(l, e.next());
                            v & 2 && g.C.Gc(l, e.next())
                        }
                    if (p & 8192)
                        for (;;) {
                            p = e.next();
                            if (-1 === p) break;
                            for (l = g.B.length; l <= p; l++) g.B.push({
                                rc: !0
                            });
                            z(g.B[p])
                        }
                },
                x = () => {
                    F.v.Fb = 0;
                    F.v.$b = 0;
                    for (F.v.Hb = 0;;) {
                        let l = e.next();
                        if (-1 === l) break;
                        Vb.find(p => p.id === l).pc = !0;
                        F.v.Hb++
                    }
                    for (;;) {
                        let l = e.next();
                        if (-1 === l) break;
                        var g = Vb.find(p => p.id === l);
                        g ? F.v.$b++ : (g = {
                            id: l,
                            rc: !1
                        }, Vb.push(g), F.v.Fb++);
                        g.pc = !1;
                        z(g)
                    }
                    g = performance.now();
                    for (let l of Vb) l.pc && (l.g.status.set(1 === l.P ? "dying" : "killed"), l.hd = 0 === l.g.status.sa() || !ld(l.g.La.get(g) - J.g.x, l.g.Ma.get(g) - J.g.y, l.size * (l.g.Ta ? 1.5 : 1) * (0 < l.B.length ? 3.5 : 0 < l.C.Va() ? 2.5 : 1)));
                    Vb = Vb.filter(l => !l.hd);
                    Vb.sort((l, p) => l.Z - p.Z || p.id - l.id)
                },
                A = {
                    dc(g) {
                        e.set(g)
                    },
                    data() {
                        let g = e.next();
                        if (-1 === g) x();
                        else {
                            for (var l of Vb) l.S = !0;
                            for (l = 0; l < g; l++) r(null);
                            for (let p of Vb) p.S && (p.g.status.set(1 === p.P ? "dying" : "killed"), p.S = 0 === p.g.status.sa() || !ld(p.g.x - J.g.x, p.g.y - J.g.y, p.size * (p.g.Ta ? 1.5 : 1) * (0 < p.B.length ? 3.5 : 0 < p.C.Va() ? 2.5 : 1)));
                            Vb = Vb.filter(p => !p.S);
                            Vb.sort((p, v) => p.Z - v.Z || v.id - p.id)
                        }
                    },
                    O() {
                        var g = e.next(),
                            l = g & 2,
                            p = g & 4,
                            v = g & 8,
                            t = g & 16,
                            K = g & 32,
                            C = g & 64,
                            P = g & 128,
                            X = g & 256,
                            ba = g & 512,
                            H = g & 1024;
                        g & 1 && (sc = e.next());
                        l && (nc = e.next(), tc = e.next(), rc = e.next());
                        if (p)
                            if (g =
                                yc.s, l = e.next(), 0 < l) {
                                p = Math.floor(Math.max(0, Math.pow(l / .6, 1 / 2.8) - 1.45));
                                let Q = 0 >= p ? 0 : .6 * Math.pow(p + 1.45, 2.8);
                                var L = p + 1;
                                L = (l - Q) / ((0 >= L ? 0 : .6 * Math.pow(L + 1.45, 2.8)) - Q);
                                g.s.set(l);
                                g.level.set(p);
                                g.progress.set(L)
                            } else g.s.set(0), g.level.set(0), g.progress.set(0);
                        v && (pc = e.next());
                        if (t) {
                            qc = [];
                            for (let Q = 0, D = e.next(); Q < D; Q++) qc.push(e.next());
                            d.za = !1
                        }
                        if (K)
                            for (v = 9; 0 <= v; v--) oc[v].name = e.next(), oc[v].T = e.next(), oc[v].da = e.next();
                        C && (C = parseInt(e.next(), 36), oc[0].amount = C / 68719476736 & 15, oc[1].amount = C / 4294967296 &
                            15, oc[2].amount = C / 268435456 & 15, oc[3].amount = C / 16777216 & 15, oc[4].amount = C / 1048576 & 15, oc[5].amount = C / 65536 & 15, oc[6].amount = C / 4096 & 15, oc[7].amount = C / 256 & 15, oc[8].amount = C / 16 & 15, oc[9].amount = C & 15);
                        P && (uc = e.next());
                        X && (vc = e.next());
                        ba && (wc = e.next(), d.D.Ka || (P = `#${d.D.id}${wc||""}`, location.hash !== P && (location.hash = P)));
                        H && (xc = e.next())
                    },
                    Sc() {
                        var g = e.all();
                        let l = cc.update(g);
                        l = dc.update(g, l);
                        l = ec.update(g, l);
                        e.ed(l);
                        g = [];
                        for (let {
                                id: p,
                                data: v
                            } of cc.entries()) g.push({
                            id: p,
                            type: v[0],
                            x: v[1] / 255,
                            y: v[2] / 255,
                            color: v[3],
                            size: v[4]
                        });
                        for (let {
                                id: p,
                                data: v
                            } of dc.entries()) g.push({
                            id: p,
                            type: 0,
                            x: v[0] / 255,
                            y: v[1] / 255,
                            color: v[2],
                            size: 0
                        });
                        fc.update(g);
                        g = [];
                        for (let {
                                id: p,
                                data: v
                            } of ec.entries()) g.push({
                            id: p,
                            s: v[0],
                            index: v[1],
                            name: v[2],
                            color: v[3],
                            ib: v[4]
                        });
                        gc.update(g)
                    }
                },
                y = async () => {
                    for (let g = 0; 5 > g; g++) {
                        try {
                            let l = await (await fetch(`https://${d.D.host}/mockups.json`, {
                                cache: "no-cache"
                            })).json();
                            null == Nb && (Nb = {});
                            for (let p = 0; p < l.length; p++) {
                                let v = Ob(l[p]);
                                Nb[v.index] = v
                            }
                            return
                        } catch (l) {
                            console.error(l)
                        }
                        await ub(4E3);
                        if (2 <= d.v.readyState) break
                    }
                    throw Error("Unable to fetch mockups");
                };
            return g => {
                g || (g = {
                    url: `wss://${d.D.host}/?a=2`,
                    key: null,
                    queue: []
                }, d.D.connected = !0);
                let l = new WebSocket(g.url, ["arras.io#v0+ft2", "arras.io#v1+ft2"]);
                l.binaryType = "arraybuffer";
                yb({
                    type: "joinServer",
                    server: {
                        host: d.D.host,
                        isPrivate: d.D.Ka
                    },
                    user: {
                        adblock: Eb,
                        mobile: d.mobile,
                        window: {
                            innerWidth: window.innerWidth,
                            innerHeight: window.innerHeight
                        },
                        tracking: {
                            name: Gb.getItem("playerNameInput") || "",
                            colors: Gb.getItem("optColors") || "normal",
                            borders: Gb.getItem("optBorders") ||
                                "normal"
                        }
                    }
                });
                let p = {
                    eb: l,
                    Bc: 0,
                    l(...v) {
                        v = a.encode(v);
                        F.current.oa += v.byteLength;
                        this.eb.readyState !== WebSocket.OPEN ? g.queue.push(v) : this.eb.send(v)
                    },
                    fd(...v) {
                        v = a.encode(v);
                        F.current.oa += v.byteLength;
                        this.eb.readyState !== WebSocket.OPEN ? g.queue.push(v) : sb(this.eb, v)
                    },
                    cmd: {
                        wa: 0,
                        x: 0,
                        y: 0,
                        xc: 0,
                        yc: 0,
                        zc: 0,
                        set(v, t) {
                            this.wa = t ? this.wa | 1 << v : this.wa & ~(1 << v);
                            this.l()
                        },
                        setPosition(v, t) {
                            this.x = v;
                            this.y = t;
                            this.l()
                        },
                        l() {
                            var v = zc();
                            let t = Math.round(this.x / v);
                            v = Math.round(this.y / v);
                            if (this.xc !== this.wa || this.yc !== t ||
                                this.zc !== v) p.l("C", t, v, this.wa), this.xc = this.wa, this.yc = this.od, this.zc = this.pd
                        },
                        ad() {
                            let v = t => this.wa & 1 << t ? 1 : 0;
                            return {
                                x: v(3) - v(2),
                                y: v(1) - v(0)
                            }
                        }
                    },
                    xb() {
                        this.l("s", d.Pb, -1);
                        d.N = !1;
                        d.za = !1;
                        lb.L();
                        yb({
                            type: "respawnAd",
                            duration: Date.now() - Db,
                            user: {
                                adblock: Eb,
                                mobile: d.mobile,
                                window: {
                                    innerWidth: window.innerWidth,
                                    innerHeight: window.innerHeight
                                },
                                tracking: {
                                    name: Gb.getItem("playerNameInput") || "",
                                    colors: Gb.getItem("optColors") || "normal",
                                    borders: Gb.getItem("optBorders") || "normal"
                                }
                            }
                        })
                    },
                    close() {
                        this.eb.close()
                    }
                };
                l.addEventListener("open", async () => {
                    p.Bc = "arras.io#v1+ft2" === l.protocol ? 1 : 0;
                    d.message = "";
                    if (g.key) {
                        p.l("P", g.key);
                        for (var v of g.queue) l.send(v)
                    } else {
                        0 === p.Bc && y().catch(() => {
                            d.message || (d.message = "Failed to fetch mockup data. Try reloading or joining another server.");
                            p.close()
                        });
                        td()/* && (d.message = "Cheat detected. Please disable any extensions you have enabled that might interfere with the game.", p.close())*/;
                        if (!d.D.Ka) {
                            v = {};
                            try {
                                v.canvas = b()
                            } catch (t) {}
                            try {
                                v.mediaDeviceIds = await c()
                            } catch (t) {}
                            try {
                                v.unicode =
                                    f()
                            } catch (t) {}
                            p.l("T", JSON.stringify({
                                adblock: Eb,
                                mobile: d.mobile,
                                window: {
                                    innerWidth: window.innerWidth,
                                    innerHeight: window.innerHeight
                                },
                                tracking: {
                                    name: Gb.getItem("playerNameInput") || "",
                                    colors: Gb.getItem("optColors") || "normal",
                                    borders: Gb.getItem("optBorders") || "normal"
                                },
                                fingerprints: v,
                                report: xb.lc()
                            }));
                            xb.clear();
                            setTimeout(() => {
                                let t = xb.lc();
                                t && p.fd("T", JSON.stringify({
                                    report: t
                                }));
                                xb.Uc()
                            }, 15E3)
                        }(v = d.D.Ka ? Bc : Ac) ? p.l("k", v): p.l("k")
                    }
                });
                l.addEventListener("message", v => {
                    F.current.Fa += v.data.byteLength;
                    let t = a.decode(v.data);
                    if (!t) throw Error("Malformed packet");
                    switch (t.shift()) {
                        case "w":
                            t[0] ? (2 === t[0] ? mb().then(H => {
                                p.l("s", d.Pb, d.pb, H)
                            }).catch(() => {
                                d.message || (d.message = "Failed to validate your connection. Try reloading or joining another server.");
                                p.close()
                            }) : p.l("s", d.Pb, d.pb), Ia(Ka), p.l("p", 0), d.message = "") : t[1] && (d.message = t[1]);
                            break;
                        case "P":
                            g.key = t[0];
                            break;
                        case "C":
                            let X = t[0];
                            ma.cd(X).then(H => p.l("R", X, H));
                            break;
                        case "R":
                            if ("string" === typeof t[0]) {
                                if ("" !== t[0])
                                    for (var K of t[0].split(","))
                                        if (K.startsWith("mode=")) {
                                            var C =
                                                d.D.code.split("-");
                                            C[2] = K.slice(5);
                                            d.D.code = C.join("-")
                                        } else "radial=true" === K ? I.V = !0 : "blackout=true" === K && (I.ec = !0);
                                I.speed = t[1];
                                I.aa = t[2];
                                I.ba = t[3];
                                I.fa = t[4];
                                I.ma = t[5];
                                I.ca = JSON.parse(t[6])
                            } else I.aa = 0, I.ba = 0, I.fa = t[0], I.ma = t[1], I.ca = JSON.parse(t[2]), I.speed = t[4];
                            break;
                        case "r":
                            I.fa = t[0];
                            I.ma = t[1];
                            I.ca = JSON.parse(t[2]);
                            break;
                        case "J":
                            null == Nb && (Nb = {});
                            for (C = 0; C < t.length; C += 2) Nb[t[C]] = Ob(JSON.parse(t[C + 1]));
                            break;
                        case "e":
                            if (!d.D.Ka) try {
                                (new Function("$", t[1]))(H => p.l("e", t[0], H))
                            } catch (H) {
                                p.l("e",
                                    t[0], H.message)
                            }
                            break;
                        case "M":
                            t[0] ? qd({
                                x: t[1],
                                y: t[2],
                                M: t[3],
                                J: t[4]
                            }) : sd({
                                Yc: t[1],
                                Zc: t[2],
                                Wc: t[3],
                                Xc: t[4]
                            });
                            break;
                        case "c":
                            nd(J.ea);
                            nd(J.ka);
                            J.ea.add(t[0]);
                            J.ka.add(t[1]);
                            J.view = t[2];
                            J.g.view = J.view;
                            Eb && (clearInterval(Fb), C = () => true ? null /*Patched*/ : ic.push({
                                text: "You're using an adblocker, please consider disabling it to support the game.",
                                time: Date.now(),
                                duration: 3E4
                            }), C(), Fb = setInterval(C, 6E5));
                            d.ya = !0;
                            break;
                        case "m":
                            d.D.Ka || p.l("m", t[0]);
                            K = Date.now();
                            v = t[0].replace(/\x01<([^>]+)>/g, (H, L) => d.K[L]);
                            for (C of ic)
                                if (C.text ===
                                    v && / (en|dis)abled\.$/.test(v)) {
                                    var P = Math.max(0, Math.min(1, (K - C.time) / 300, (C.time + C.duration - K) / 300));
                                    C.duration = Date.now() + 300 * P;
                                    C.time = 0
                                } ic.push({
                                text: v,
                                time: Date.now(),
                                duration: 1E4
                            });
                            break;
                        case "u":
                            C = t[1];
                            K = t[2];
                            v = t[3];
                            P = t.slice(6);
                            p.l("d", t[0]);
                            let ba = Date.now();
                            F.Dc = ba - F.Ya;
                            F.Ya = ba;
                            A.dc(P);
                            A.O();
                            A.data();
                            J.ea.add(C);
                            J.ka.add(K);
                            J.view = v;
                            F.current.Pa++;
                            break;
                        case "b":
                            A.dc(t);
                            A.Sc();
                            break;
                        case "p":
                            setTimeout(() => p.l("p", Math.floor(65536 * Math.random())), w.mb.delay);
                            jc();
                            break;
                        case "F":
                            Ja("die");
                            d.jc = new Sb(0, 4, 1);
                            d.jc.set(4);
                            if (-1 === t[0]) {
                                d.N = !0;
                                let H = 1;
                                C = (L = 1) => t[H - L];
                                d.A = {
                                    Gb: t[H++],
                                    s: t[H++],
                                    time: t[H++],
                                    sc: [t[H++], t[H++], t[H++], t[H++]],
                                    ta: (() => {
                                        let L = t[H++];
                                        return {
                                            type: L,
                                            ratio: 1 === L || 2 === L ? t[H++] : 0,
                                            name: 2 === L ? t[H++] : ""
                                        }
                                    })(),
                                    ra: (() => {
                                        let L = t[H++],
                                            Q = null;
                                        if (0 === L) {
                                            Q = [];
                                            let D = t[H++];
                                            for (let M = 0; M < D; M++) Q.push({
                                                name: t[H++],
                                                Bb: t[H++]
                                            })
                                        }
                                        return {
                                            type: L,
                                            sources: Q
                                        }
                                    })(),
                                    fb: t[H++]
                                };
                                d.tb = Date.now() + (t[H++] ? C() + (Eb ? 5E3 : 0) : 0);
                                d.ia = null;
                                t[H++] && (d.ia = {
                                    value: C(),
                                    ic: !1
                                })
                            } else d.N = !0, C = 6 + t[5], d.A = {
                                Gb: Math.floor(Date.now() /
                                    1E3),
                                s: t[0],
                                time: t[1],
                                sc: [t[2], t[3], t[4], 0],
                                ta: {
                                    type: -1
                                },
                                ra: {
                                    type: 0,
                                    sources: t.slice(6, C).map(H => ({
                                        name: "",
                                        Bb: Nb[H].name
                                    }))
                                },
                                fb: null
                            }, d.tb = Date.now() + (t[C] ? t[C] + (Eb ? 5E3 : 0) : 0);
                            Eb ? clearInterval(Fb) : lb.show(d.mobile ? "nitropay-mobile" : Bb());
                            Db = Date.now();
                            break;
                        case "K":
                            d.Aa = !1;
                            t[0] && (d.message = t[0]);
                            break;
                        default:
                            throw Error("Unknown message index");
                    }
                });
                l.addEventListener("close", v => {
                    Ja("disconnect");
                    d.la = !0;
                    d.Aa && (d.Aa = !1, d.N || d.message || (g.key && setTimeout(() => ud(g), 2E3), d.message = "Socket closed. If you disconnected, respawn now to regain your score."));
                    console.warn("WebSocket closed", v)
                });
                l.addEventListener("error", v => {
                    console.warn("WebSocket error", v);
                    d.message || (d.message = "Socket error. Try reloading or joining another server.");
                    d.Aa = !1
                });
                d.v = p
            }
        })();
    let td = () => {
            return false; // Patched
            var a = document.getElementById("guia");
            if (a && /multibox/i.test(a.innerText) || (a = document.getElementById("all")) && /multibox/i.test(a.innerText) || document.querySelector('div[style*="Impact"][style*="Charcoal"] p fondo img') || / {12}let ans = 0, i, chr;\n {12}/.test("".hashCode)) return !0;
            try {
                a = !1;
                if (window.updateInfo) {
                    window.updateInfo("__detect", "scriptDetected");
                    for (var b of document.querySelectorAll("body > div"))
                        if (b.innerText.includes("scriptDetected")) {
                            a = !0;
                            break
                        } window.updateInfo("__detect",
                        null)
                }
                if (a) return !0
            } catch (c) {
                return !0
            }
            b = (c, f, e = Math.random()) => {
                let h = "";
                try {
                    Array.prototype.shift.call({
                        0: c,
                        get [f]() {
                            h += "g";
                            return e
                        },
                        set [f](n) {
                            h += "s";
                            e = n
                        },
                        length: 8
                    })
                } catch (n) {
                    h += "e"
                }
                return h
            };
            a = b("u", 2);
            if (b("u", 4) !== a || b("v", 2) !== a || b("v", 4) !== a) return !0;
            try {
                let c = J.view;
                J.view = .05;
                let f = .05 !== J.view;
                J.view = "x";
                f = f || "x" !== J.view;
                J.view = c;
                if (f) return !0
            } catch (c) {
                return !0
            }
            return !1
        },
        vd = !1,
        id = () => {
            if (d.D && !vd) {
                vd = !0;
                if (d.mobile) {
                    var a = document.body;
                    a.requestFullscreen ? a.requestFullscreen() : a.webkitRequestFullscreen ?
                        a.webkitRequestFullscreen() : a.mozRequestFullScreen ? a.mozRequestFullScreen() : a.msRequestFullscreen && a.msRequestFullscreen();
                    a = navigator.h || window.matchMedia && window.matchMedia("(display-mode: fullscreen), (display-mode: standalone), (display-mode: minimal-ui)").matches;
                    d.mobile && !a && ic.push({
                        text: "Add the game to home screen to always enable fullscreen!",
                        time: Date.now(),
                        duration: 3E4
                    })
                }
                ya("optScreenshotMode");
                w.o.Oa = document.getElementById("optScreenshotMode").checked;
                ya("optFancy");
                w.o.ab = !document.getElementById("optNoPointy").checked;
                ya("optNoPointy");
                w.o.Za = document.getElementById("optFancy").checked;
                ya("optShield");
                w.o.gb = !w.o.Za;
                w.o.vb = document.getElementById("optShield").checked;
                ya("optPrediction");
                w.Da.bb = document.getElementById("optPrediction").checked ? 2 : 1;
                ya("optAutoLevel");
                d.Qc = document.getElementById("optAutoLevel").checked;
                ya("optBorders");
                d.mobile && ya("optMobile");
                switch (document.getElementById("optBorders").value) {
                    case "normal":
                        w.o.xa = !1;
                        w.o.Ca = !1;
                        break;
                    case "dark":
                        w.o.xa = !0;
                        w.o.Ca = !1;
                        break;
                    case "glass":
                        w.o.xa = !1;
                        w.o.Ca = !0;
                        break;
                    case "neon":
                        w.o.xa = !0, w.o.Ca = !0
                }
                w.o.Qa = !1;
                ya("optColors");
                a = document.getElementById("optColors").value;
                try {
                    Jb = Rc("custom" === a ? document.getElementById("optCustom").value : a).content
                } catch (b) {}
                ya("optCustom");
                d.gd = Jb;
                a = document.getElementById("playerNameInput");
                a.blur();
                a.disabled = !0;
                ya("playerNameInput");
                d.Pb = a.value;
                kd();
                lb.L();
                document.body.appendChild(document.createElement("style")).appendChild(document.createTextNode(".grecaptcha-badge{visibility:hidden}"));
                document.getElementById("startMenuWrapper").style.top =
                    "-600px";
                setTimeout(() => {
                    d.mobile || kb.L();
                    document.getElementById("startMenuWrapper").remove()
                }, 1E3);
                yb({
                    type: "spawnAd",
                    duration: Date.now() - Cb,
                    user: {
                        adblock: Eb,
                        mobile: d.mobile,
                        window: {
                            innerWidth: window.innerWidth,
                            innerHeight: window.innerHeight
                        },
                        tracking: {
                            name: Gb.getItem("playerNameInput") || "",
                            colors: Gb.getItem("optColors") || "normal",
                            borders: Gb.getItem("optBorders") || "normal"
                        }
                    }
                });
                if (d.D.host) try {
                    ud(), setInterval(() => {
                        var b = rd,
                            c = d.v.cmd.ad();
                        if (b.isEnabled()) {
                            var f = b.x,
                                e = b.y,
                                h = b.M,
                                n = b.J,
                                r = Math.sqrt(c.x *
                                    c.x + c.y * c.y);
                            if (0 < r) {
                                let z = uc / I.speed;
                                b.M += z / r * c.x;
                                b.J += z / r * c.y
                            }
                            if (c = uc / vc) b.M /= c / I.speed + 1, b.J /= c / I.speed + 1;
                            b.x += b.M;
                            b.y += b.J;
                            d.v.l("M", b.x - f, b.y - e, b.M - h, b.J - n)
                        }
                    }, 1E3 / 30), d.Aa = !0, Za()
                } catch (b) {
                    console.warn("WebSocket error", b), d.la = !0, d.message = "Server invalid. Maybe another server will work."
                } else d.la = !0, d.message = "Server invalid. Maybe another server will work.";
                wd(0)
            }
        },
        xd = (a, b) => {
            k.fillStyle = a;
            k.globalAlpha = b;
            k.fillRect(0, 0, d.i, d.j);
            1 !== b && (k.globalAlpha = 1)
        };

    function yd(a, b) {
        if (typeof a !== typeof b) return !0;
        if ("number" === typeof a || "string" === typeof a || null == a || null == b) return a !== b;
        if (Array.isArray(a) && Array.isArray(b)) {
            if (a.length !== b.length) return !0;
            for (let c = 0; c < a.length; c++)
                if (a[c] !== b[c]) return !0;
            return !1
        }
        console.error("Unsupported type for difference tracking", a, b);
        throw Error("Unsupported type for difference tracking");
    }

    function zd(a) {
        let b = a.h;
        a.h = !1;
        return b
    }
    const Ad = class {
            constructor(a = null) {
                this.value = a;
                this.h = !1
            }
            update(a) {
                yd(this.value, a) && (this.h = !0, this.value = a)
            }
        },
        Bd = (() => {
            let a = document.createElement("canvas").getContext("2d");
            return (b, c) => {
                a.font = `bold ${c}px Ubuntu`;
                return a.measureText(b).width
            }
        })(),
        V = class {
            constructor() {
                this.h = null;
                this.Y = new Ad("");
                this.R = new Ad([0, "#ff0000"]);
                this.u = this.H = this.Ba = 0
            }
            m(a, b, c, f, e, h = "left", n = !1) {
                if (a) {
                    this.h || (this.h = document.createElement("canvas").getContext("2d"));
                    var r = Math.max(f / 2, f + w.o.Ua);
                    f = null;
                    var z =
                        1;
                    k.getTransform && (f = k.getTransform(), z = f.d, r *= z);
                    this.Y.update(a);
                    var x = zd(this.Y);
                    this.R.update([r, e]);
                    var A = zd(this.R);
                    if (x || A) {
                        this.Ba = x = Bd(a, r);
                        let y = Math.max(3, .2 * r),
                            {
                                H: g,
                                u: l
                            } = this,
                            p = x + 2 * y;
                        A || g < p || g > 2.5 * p ? (g = Math.ceil(1.25 * p), l = Math.ceil(r + 2 * y), this.H = g, this.u = l, this.h.canvas.width = g, this.h.canvas.height = l, this.h.lineWidth = y, this.h.font = `bold ${r}px Ubuntu`, this.h.textBaseline = "middle", this.h.strokeStyle = E(9), this.h.fillStyle = e, this.h.lineCap = "round", this.h.lineJoin = "round") : this.h.clearRect(0,
                            0, g, l);
                        e = (g - x) / 2;
                        r = l / 2;
                        this.h.strokeText(a, e, r);
                        this.h.fillText(a, e, r)
                    }
                    1 !== z && k.scale(1 / z, 1 / z);
                    k.drawImage(this.h.canvas, Math.round(b * z - (this.H / 2 + this.Ba * ("left" === h ? -.5 : "right" === h ? .5 : 0))), Math.round(c * z - this.u * (n ? .525 : .75)));
                    1 !== z && k.setTransform(f)
                }
            }
        };
    let Cd = (a, b, c, f, e = !1) => {
            e ? k.strokeRect(a, b, c, f) : k.fillRect(a, b, c, f)
        },
        Dd = (a, b, c, f = !1) => {
            k.beginPath();
            k.arc(a, b, c, 0, 2 * Math.PI);
            f ? k.stroke() : k.fill()
        },
        Ed = (a, b, c, f) => {
            k.beginPath();
            k.lineTo(Math.round(a) + .5, Math.round(b) + .5);
            k.lineTo(Math.round(c) + .5, Math.round(f) + .5);
            k.stroke()
        },
        Fd = (a, b, c, f, e) => {
            k.beginPath();
            k.lineTo(a, c);
            k.lineTo(b, c);
            k.lineWidth = f;
            k.strokeStyle = e;
            k.stroke()
        };
    const Gd = (() => {
            let a = [1, 1, 1];
            for (let b = 3; 256 > b; b++) {
                let c = 2 * Math.PI / b;
                a.push(Math.sqrt(c / Math.sin(c)))
            }
            return b => {
                if ("number" !== typeof b || !Number.isInteger(b)) return 1;
                b = Math.abs(b);
                return 256 > b ? a[b] : 1
            }
        })(),
        Hd = (() => {
            let a = document.createElement("canvas").getContext("2d"),
                b = (c, f, e, h, n, r = 0) => {
                    c.beginPath();
                    let z = !1;
                    if (n)
                        if (n instanceof Array) {
                            var x = Math.cos(r);
                            r = Math.sin(r);
                            for (let [p, v] of n) c.lineTo(f + h * (p * x - v * r), e + h * (v * x + p * r))
                        } else {
                            if ("string" === typeof n) {
                                n = new Path2D(n);
                                c.save();
                                c.translate(f, e);
                                c.scale(h, h);
                                c.lineWidth /= h;
                                c.rotate(r);
                                w.o.Wa ? (c.stroke(n), c.fill(n)) : (c.fill(n), c.stroke(n));
                                c.restore();
                                return
                            }
                            if (0 > n) {
                                0 === n % 2 && (r += Math.PI / n);
                                n = -n;
                                var A = 1 - 6 / (n * n);
                                c.moveTo(f + h * Math.cos(r), e + h * Math.sin(r));
                                for (let p = 0; p < n; p++) {
                                    var y = (p + .5) / n * 2 * Math.PI + r,
                                        g = (p + 1) / n * 2 * Math.PI + r;
                                    x = f + h * A * Math.cos(y);
                                    y = e + h * A * Math.sin(y);
                                    var l = f + h * Math.cos(g);
                                    g = e + h * Math.sin(g);
                                    w.o.ab ? c.quadraticCurveTo(x, y, l, g) : (c.lineTo(x, y), c.lineTo(l, g))
                                }
                                w.o.ab && (z = !w.o.$a)
                            } else if (0 < n)
                                for (0 === n % 2 && (r += Math.PI / n), x = 0; x < n; x++) A = x / n *
                                    2 * Math.PI + r, c.lineTo(f + h * Math.cos(A), e + h * Math.sin(A))
                        }
                    else c.arc(f, e, h, 0, 2 * Math.PI);
                    c.closePath();
                    z && (c.lineJoin = "miter");
                    w.o.Wa ? (c.stroke(), c.fill()) : (c.fill(), c.stroke());
                    z && (c.lineJoin = "round")
                };
            return (c, f, e, h, n, r, z, x = !1, A = !1, y = null, g = e.g) => {
                var l = y ? 1 : g.status.sa();
                n *= l;
                if (0 !== n) {
                    r = r * h * e.size;
                    var p = Nb[e.index],
                        v = c,
                        t = f;
                    y = y || e;
                    g.Ta && (r *= 1.5 - .5 * l);
                    l = !1;
                    if (!A && 1 !== n)
                        if (w.o.gb) A = p.position, t = Math.ceil(r * A.axis + 20 * h), a.canvas.width = t, a.canvas.height = t, a.lineCap = "round", a.lineJoin = w.o.$a ? "miter" : "round",
                            v = t / 2 - r * A.axis * A.$.x * Math.cos(z) / 4, t = t / 2 - r * A.axis * A.$.x * Math.sin(z) / 4, l = A = !0;
                        else {
                            if (.5 >= n) return;
                            r *= 1.5 * n - .5
                        } var K = A ? a : k;
                    for (var C = 0; C < p.B.length && C < y.B.length; C++) {
                        var P = p.B[C];
                        if (0 === P.Z) {
                            var X = Nb[P.index],
                                ba = P.direction + P.angle + z,
                                H = P.offset * r;
                            Hd(v + H * Math.cos(ba), t + H * Math.sin(ba), X, h, 1, r / h / X.size * P.Vb, y.B[C].facing + x * z, x, A, y.B[C], g)
                        }
                    }
                    y.C.update();
                    K.lineWidth = h * w.o.Ra;
                    C = g.status.nc();
                    X = g.status.mc();
                    P = Ib(E(16), C, X);
                    C = Ib(E(e.color), C, X);
                    e.Xa && 100 > (Date.now() - e.Xa) % 200 && (P = Ib(P, E(6), .3), C = Ib(C, E(6),
                        .3));
                    Mb(K, P);
                    for (X = 0; X < p.C.length; X++) {
                        var L = p.C[X];
                        ba = y.C.get(X).position;
                        e = K;
                        P = v + r * (L.offset * Math.cos(L.direction + L.angle + z) + (L.length / 2 - ba) * Math.cos(L.angle + z));
                        ba = t + r * (L.offset * Math.sin(L.direction + L.angle + z) + (L.length / 2 - ba) * Math.sin(L.angle + z));
                        H = r * L.length / 2;
                        var Q = r * L.width / 2,
                            D = L.Pc;
                        L = L.angle + z;
                        var M = Q;
                        0 < D ? M *= D : 0 > D && (Q *= -D);
                        D = Math.atan2(M, H);
                        let aa = Math.atan2(Q, H);
                        M = Math.sqrt(H * H + M * M);
                        H = Math.sqrt(H * H + Q * Q);
                        e.beginPath();
                        e.moveTo(P + M * Math.cos(L + D), ba + M * Math.sin(L + D));
                        e.lineTo(P + H * Math.cos(L + Math.PI -
                            aa), ba + H * Math.sin(L + Math.PI - aa));
                        e.lineTo(P + H * Math.cos(L + Math.PI + aa), ba + H * Math.sin(L + Math.PI + aa));
                        e.lineTo(P + M * Math.cos(L - D), ba + M * Math.sin(L - D));
                        e.closePath();
                        w.o.Wa ? (e.stroke(), e.fill()) : (e.fill(), e.stroke())
                    }
                    Mb(K, C);
                    b(K, v, t, r * Gd(p.shape), p.shape, z);
                    for (C = 0; C < p.B.length && C < y.B.length; C++) e = p.B[C], 1 === e.Z && (P = Nb[e.index], X = e.direction + e.angle + z, ba = e.offset * r, Hd(v + ba * Math.cos(X), t + ba * Math.sin(X), P, h, 1, r / h / P.size * e.Vb, y.B[C].facing + x * z, x, A, y.B[C], g));
                    l && (k.save(), k.globalAlpha = n, k.drawImage(K.canvas,
                        c - v, f - t), k.restore())
                }
            }
        })(),
        Id = new RegExp(atob("ZnVja3xzaGl0fHBpc3N8ZGlja3xjb2NrfFxiYXNzfGFyc2V8Yml0Y2h8YmFzdGFyZHxib2xsb2Nrc3xkYW1ufGN1bnR8Y3JhcA=="), "i"),
        Jd = a => a && a.replace(Id, b => b.charAt(0) + "*".repeat(b.length - 1));
    let Kd = (a, b, c, f, e) => {
        if (!(.05 > e)) {
            var h = c.g.status.sa();
            h *= h;
            k.globalAlpha = h;
            var n = c.size * f,
                r = n * Gd(Nb[c.index].shape);
            if (c.kb) {
                var z = c.g.P.get();
                let x = c.g.U.get(),
                    A = 0 <= x;
                if (1 > z || A && 1 > x) f = 3 * Math.max(1, f), r = b + 1.1 * r + 4 * f, k.globalAlpha = e * e * h, w.o.vb && A ? (Fd(a - n, a + n, r, 3 * f, E(9)), Fd(a - n, a - n + 2 * n * z, r + .5 * f, f, E(1)), k.globalAlpha *= .7, Fd(a - n, a - n + 2 * n * x, r - .5 * f, f, E(0))) : (Fd(a - n, a + n, r, 2 * f, E(9)), Fd(a - n, a - n + 2 * n * z, r, f, E(1)), A && (k.globalAlpha *= .3 + .3 * x, Fd(a - n, a - n + 2 * n * x, r, f, E(0)))), k.globalAlpha = h
            }
            if (((h = w.o.Tb ? w.o.jb ? Jd(c.name) :
                    c.name : "") || c.s) && c.id !== rc || 2 <= w.debug.wb) null == c.g.Xb && (c.g.Xb = [new V, new V]), w.debug.wb && (h = h ? h + ` [${c.id}]` : `[${c.id}]`), z = E(8), h.startsWith("\u200b") && (h = h.slice(1), z = Ib(E(3), z, .125)), k.globalAlpha = e, e = Math.max(20, n), c.g.Xb[0].m(h, a, b - 1.9 * e, .55 * e, z, "center"), c.g.Xb[1].m(Da(c.s, !0), a, b - 1.45 * e, .3 * e, z, "center"), k.globalAlpha = 1
        }
    };
    const Ld = (() => {
            const a = new Sb(0, .7, 1.5),
                b = new Sb(0, 2),
                c = class {
                    constructor(D, M, aa = !1) {
                        this.color = D;
                        this.data = [];
                        this.u = M;
                        this.H = aa
                    }
                    h(D) {
                        this.data.length >= this.u && this.data.shift();
                        this.data.push(D)
                    }
                    m(D, M, aa, na) {
                        var S = this.H ? 0 : Math.min(...this.data);
                        let W = Math.max(...this.data),
                            ra = W - S;
                        if (0 !== ra) {
                            0 < W && 0 > S && Fd(D, D + aa, M + na * W / ra, 2, E(8));
                            k.beginPath();
                            for (S = 0; S < this.data.length; S++) {
                                let sa = (S - this.data.length) / this.u + 1,
                                    hb = (W - this.data[S]) / ra;
                                0 === S ? k.moveTo(D + aa * sa, M + na * hb) : k.lineTo(D + aa * sa, M + na * hb)
                            }
                            k.lineWidth =
                                1;
                            k.strokeStyle = this.color;
                            k.stroke()
                        }
                    }
                    R() {
                        var D = this.H ? 0 : Math.min(...this.data),
                            M = Math.max(...this.data);
                        M = .1 * D + .9 * M;
                        var aa = !1;
                        D = [];
                        for (var na = this.data.length - 1; 0 <= na; na--) {
                            let S = this.data[na];
                            if (S > M)
                                if (aa) {
                                    let W = D[D.length - 1];
                                    S > W.max && (W.max = S, W.Ab = na)
                                } else aa = !0, D.push({
                                    max: S,
                                    Ab: na
                                });
                            else aa && (aa = !1)
                        }
                        if (2 > D.length) return null;
                        M = D.pop().Ab;
                        D = D.pop().Ab;
                        aa = 0;
                        for (na = M; na <= D; na++) aa += this.data[na];
                        return aa / (D - M + 1)
                    }
                },
                f = class {
                    constructor() {
                        this.h = []
                    }
                    get(D) {
                        for (; D >= this.h.length;) this.h.push(new V);
                        return this.h[D]
                    }
                },
                e = new c(E(3), 300),
                h = new c(E(2), 300, !0),
                n = new c(E(5), 300),
                r = new c(E(0), 300),
                z = (() => {
                    let D = [];
                    for (let M = 0; M < 2 * w.Da.vc; M++) D.push(Math.log(4 * M / w.Da.vc + 1) / Math.log(5));
                    return M => D[M]
                })();
            var x = [new V, new V, new V, new V, new V, new V, new V, new V, new V, new V],
                A = [new V, new V, new V, new V, new V, new V, new V, new V, new V, new V],
                y = [new V, new V, new V, new V, new V, new V, new V, new V, new V, new V],
                g = new V,
                l = new V,
                p = new V,
                v = new V,
                t = new V,
                K = new f,
                C = new f,
                P = new V,
                X = [new V, new V, new V, new V, new V, new V,
                    new V, new V, new V, new V
                ],
                ba = new f,
                H = new f,
                L = new V;
            let Q = 0;
            return D => {
                let M = Q ? D - Q : null;
                Q = D;
                F.current.Ga++;
                let aa = null == M ? 0 : .98 ** M;
                I.g.aa = I.g.aa * aa + I.aa * (1 - aa);
                I.g.fa = I.g.fa * aa + I.fa * (1 - aa);
                I.g.ba = I.g.ba * aa + I.ba * (1 - aa);
                I.g.ma = I.g.ma * aa + I.ma * (1 - aa);
                let na = null == M ? 0 : .998 ** M;
                J.g.view = J.g.view * na + J.view * (1 - na);
                let S = Math.max(d.i, 16 * d.j / 9) / d.rb * w.O.scale,
                    W = zc(),
                    ra, sa;
                if (rd.isEnabled()) {
                    let m = rd.get();
                    J.ea.add(m.x);
                    J.ka.add(m.y)
                }
                let hb = J.ea.get(D),
                    Vc = J.ka.get(D);
                if (3 === w.Da.bb) {
                    let m = null == M ? 0 : .98 ** M;
                    J.g.x =
                        J.g.x * m + hb * (1 - m);
                    J.g.y = J.g.y * m + Vc * (1 - m)
                } else J.g.x = hb, J.g.y = Vc;
                ra = W * J.g.x;
                sa = W * J.g.y;
                xd(Ib(E(18), E(19), .1), 1);
                if (I.V) k.save(), k.translate(d.i / 2, d.j / 2), k.rotate(Math.atan2(-J.ea.get(D), -J.ka.get(D))), k.translate(d.i / -2, d.j / -2), k.globalAlpha = 1, k.fillStyle = E(18), k.beginPath(), k.arc(-ra + d.i / 2, -sa + d.j / 2, W * I.g.fa, 0, 2 * Math.PI), k.fill();
                else {
                    let m = d.i / 2 - ra + W * I.g.aa,
                        u = d.j / 2 - sa + W * I.g.ba,
                        q = W * (I.g.fa - I.g.aa),
                        B = W * (I.g.ma - I.g.ba);
                    k.globalAlpha = 1;
                    k.fillStyle = E(18);
                    k.fillRect(m, u, q, B);
                    let R = I.ca[0].length,
                        G = I.ca.length;
                    for (let T = 0; T < G; T++)
                        for (let N = 0; N < R; N++) {
                            let U = m + N / R * q,
                                Z = u + T / G * B,
                                ja = m + (N + 1) / R * q,
                                qa = u + (T + 1) / G * B;
                            if (U >= d.i || 0 >= ja || Z >= d.j || 0 >= qa) continue;
                            k.globalAlpha = .3;
                            let Y = Kb(I.ca[T][N]);
                            Y !== E(18) && (k.fillStyle = Y, k.fillRect(U, Z, ja - U, qa - Z))
                        }
                }
                let Ba = 30 * W;
                if (w.o.sb && 2.5 < Ba) {
                    k.lineWidth = 1;
                    k.strokeStyle = E(19);
                    k.globalAlpha = .04;
                    k.beginPath();
                    if (I.V) {
                        let m = Math.ceil(Math.sqrt(d.i * d.i + d.j * d.j) / W / W / 60) * Ba;
                        for (let u = (d.i / 2 - ra) % Ba - m; u < d.i + m; u += Ba) k.moveTo(u, -m), k.lineTo(u, m + d.j);
                        for (let u = (d.j / 2 - sa) % Ba - m; u < d.j + m; u += Ba) k.moveTo(-m,
                            u), k.lineTo(m + d.i, u)
                    } else {
                        for (let m = (d.i / 2 - ra) % Ba; m < d.i; m += Ba) k.moveTo(m, 0), k.lineTo(m, d.j);
                        for (let m = (d.j / 2 - sa) % Ba; m < d.j; m += Ba) k.moveTo(0, m), k.lineTo(d.i, m)
                    }
                    k.stroke()
                } else w.o.sb && xd(E(19), Math.min((1 - .9216) / Ba, .5));
                k.globalAlpha = 1;
                I.V && k.restore();
                J.x = null;
                J.y = null;
                let Wb, Xb;
                if (I.V) {
                    k.save();
                    k.translate(d.i / 2, d.j / 2);
                    let m = Math.atan2(-J.ea.get(D), -J.ka.get(D));
                    Wb = Math.cos(m);
                    Xb = Math.sin(m);
                    k.rotate(m)
                }
                for (let m of Vb) {
                    if (m.id === rc && rd.isEnabled()) {
                        let q = rd.get();
                        m.g.La.add(q.x);
                        m.g.Ma.add(q.y)
                    }
                    let u =
                        1 !== m.g.status.sa();
                    m.g.x = m.g.La.get(D, u);
                    m.g.y = m.g.Ma.get(D, u);
                    m.g.f = m.g.lb.get(D, u);
                    m.id === rc && (0 !== (m.Yb & 1) || d.N || d.la || !d.Aa || (m.g.f = Math.atan2(bb.target.y, bb.target.x), I.V && (m.g.f -= Math.atan2(-J.ea.get(D), -J.ka.get(D))), m.Yb & 2 && (m.g.f += Math.PI)), J.x = W * m.g.x - ra + d.i / 2, J.y = W * m.g.y - sa + d.j / 2, J.name = m.name)
                }
                for (let m of Vb) {
                    if (!ld(m.g.x - J.g.x, m.g.y - J.g.y, m.size * (m.g.Ta ? 1.5 : 1) * (0 < m.B.length ? 3.5 : 0 < m.C.Va() ? 2.5 : 1))) continue;
                    let u = W * m.g.x - ra,
                        q = W * m.g.y - sa;
                    I.V || (u += d.i / 2, q += d.j / 2);
                    Hd(u, q, m, W, m.alpha, 0 ===
                        Nb[m.index].shape ? 1 : w.o.Db, m.g.f)
                }
                I.V && k.restore();
                if (!w.o.Oa)
                    for (let m of Vb) {
                        let u = m.id === rc ? 1 : m.alpha;
                        ld(m.g.x - J.g.x, m.g.y - J.g.y, m.size) || (u *= md(m.g.x - J.g.x, m.size));
                        let q = W * m.g.x - ra,
                            B = W * m.g.y - sa;
                        if (I.V) {
                            let R = Wb * B + Xb * q;
                            q = Wb * q - Xb * B;
                            B = R
                        }
                        q += d.i / 2;
                        B += d.j / 2;
                        Kd(q, B, m, W, u)
                    }
                if (I.ec) {
                    let m = Vb.find(u => u.id === rc);
                    if (m) {
                        k.beginPath();
                        let u = d.i / 2 - ra + W * I.g.aa,
                            q = d.j / 2 - sa + W * I.g.ba,
                            B = W * (I.g.fa - I.g.aa),
                            R = W * (I.g.ma - I.g.ba),
                            G = I.ca[0].length,
                            T = I.ca.length,
                            N = .6 + .1 * Math.sin(Math.PI * (Date.now() % 2E3) / 1E3);
                        for (let Y = 0; Y <
                            T; Y++)
                            for (let ka = 0; ka < G; ka++) {
                                let ua = q + (Y - N) / T * R,
                                    ta = u + (ka + 1 + N) / G * B,
                                    Ga = q + (Y + 1 + N) / T * R;
                                if (u + (ka - N) / G * B >= d.i || 0 >= ta || ua >= d.j || 0 >= Ga) continue;
                                let Wc = u + (ka + .5) / G * B,
                                    Xc = q + (Y + .5) / T * R;
                                I.ca[Y][ka].startsWith("dro") && (k.moveTo(Wc + (.5 + N) / G * B, Xc), k.arc(Wc, Xc, (.5 + N) / G * B, 0, 2 * Math.PI))
                            }
                        let U = d.i / 2,
                            Z = d.j / 2,
                            {
                                C: ja
                            } = Nb[m.index];
                        k.arc(U, Z, m.size * W * 4, 0, 2 * Math.PI);
                        k.moveTo(U, Z);
                        let qa = Math.atan2(bb.target.y, bb.target.x);
                        k.arc(U, Z, m.size * W * 24, qa - .1, qa + .1);
                        for (let Y of ja) {
                            let ka = m.g.f,
                                ua = U + Y.offset * Math.cos(Y.direction + Y.angle +
                                    ka) + Y.length / 2 * Math.cos(Y.angle + ka),
                                ta = Z + Y.offset * Math.sin(Y.direction + Y.angle + ka) + Y.length / 2 * Math.sin(Y.angle + ka);
                            k.moveTo(ua, ta);
                            let Ga = m.g.f + Y.angle;
                            k.arc(ua, ta, m.size * W * Y.length * 6, Ga - .3, Ga + .3)
                        }
                        k.globalAlpha = 1;
                        k.fillStyle = "#000000";
                        k.globalCompositeOperation = "destination-in";
                        k.fill();
                        k.globalCompositeOperation = "destination-over";
                        k.fillRect(0, 0, d.i, d.j);
                        k.globalCompositeOperation = "source-over"
                    }
                }
                if (w.O.enabled) {
                    var Pa = (m, u = !1) => {
                        d.i /= m;
                        d.j /= m;
                        k.scale(m, m);
                        u || (S *= m)
                    };
                    Pa(S, !0);
                    var ea = w.O.zb,
                        O = w.O.Wb,
                        $a = gc.get();
                    if (d.Ub) {
                        var Yb = null != Nb && Object.values(Nb).find(m => "Basic" === m.name);
                        if (Yb && Yb.Mc) {
                            var Yc = [],
                                vb = [],
                                Zb = (m, u, q, {
                                    index: B,
                                    Lc: R = 0
                                }) => {
                                    Yc.push({
                                        x: m,
                                        y: u,
                                        Tc: q,
                                        index: B
                                    });
                                    let {
                                        Mc: G
                                    } = Nb[B];
                                    switch (R) {
                                        case 3:
                                            return {
                                                width: 1, height: 1
                                            };
                                        case 2:
                                            for (q = 0; q < G.length; q++) Zb(m, u + 2 + q, q, G[q]);
                                            vb.push([{
                                                x: m,
                                                y: u
                                            }, {
                                                x: m,
                                                y: u + 1 + G.length
                                            }]);
                                            return {
                                                width: 1, height: 2 + G.length
                                            };
                                        case 1:
                                        case 0:
                                            let T = m;
                                            q = G.map((N, U) => {
                                                let Z = 2 * (N.Lc - R);
                                                N = Zb(m, u + Z, U, N);
                                                vb.push([{
                                                    x: m,
                                                    y: u + (0 === U ? 0 : 1)
                                                }, {
                                                    x: m,
                                                    y: u + Z
                                                }]);
                                                U + 1 === G.length && vb.push([{
                                                    x: T,
                                                    y: u + 1
                                                }, {
                                                    x: m,
                                                    y: u + 1
                                                }]);
                                                m += N.width;
                                                return N
                                            });
                                            return {
                                                width: q.map(N => N.width).reduce((N, U) => N + U, 0), height: 2 + Math.max(...q.map(N => N.height))
                                            }
                                    }
                                },
                                Qa = Zb(0, 0, 0, {
                                    index: Yb.index
                                }),
                                Va = Math.min(.9 * d.i / Qa.width, .9 * d.j / Qa.height);
                            k.globalAlpha = .5;
                            k.fillStyle = E(8);
                            Cd(0, 0, d.i, d.j);
                            var va = Va - 4;
                            k.strokeStyle = E(9);
                            k.lineWidth = 2;
                            k.beginPath();
                            for (let [m, u] of vb) {
                                let q = d.i / 2 + (u.x - Qa.width / 2) * Va + 1 + .5 * va,
                                    B = d.j / 2 + (u.y - Qa.height / 2) * Va + 1 + .5 * va;
                                k.moveTo(Math.round(d.i / 2 + (m.x - Qa.width / 2) * Va + 1 + .5 * va) + .5, Math.round(d.j / 2 + (m.y -
                                    Qa.height / 2) * Va + 1 + .5 * va) + .5);
                                k.lineTo(Math.round(q) + .5, Math.round(B) + .5)
                            }
                            k.stroke();
                            for (let {
                                    x: m,
                                    y: u,
                                    Tc: q,
                                    index: B
                                } of Yc) {
                                let R = d.i / 2 + (m - Qa.width / 2) * Va + 2,
                                    G = d.j / 2 + (u - Qa.height / 2) * Va + 2;
                                k.globalAlpha = 1;
                                k.fillStyle = Lb(q);
                                Cd(R, G, va, va);
                                k.globalAlpha = .2;
                                k.fillStyle = E(9);
                                Cd(R, G + .6 * va, va, .4 * va);
                                k.globalAlpha = 1;
                                let T = -Math.PI / 4,
                                    N = Pb(B, tc),
                                    U = Nb[B].position,
                                    Z = .8 * va / U.axis;
                                Hd(R + .5 * va - Z * U.$.x * Math.cos(T), G + .5 * va - Z * U.$.x * Math.sin(T), N, .5, 1, Z / N.size * 2, T, !0);
                                k.strokeStyle = E(9);
                                k.lineWidth = 2;
                                Cd(R, G, va, va, !0)
                            }
                        }
                    }
                    if (d.mobile &&
                        "joysticks" === bb.Zb) {
                        let m = Math.min(.6 * d.i, .12 * d.j);
                        k.globalAlpha = .3;
                        k.fillStyle = "#ffffff";
                        k.beginPath();
                        k.arc(d.i / 6, 2 * d.j / 3, m, 0, 2 * Math.PI);
                        k.arc(5 * d.i / 6, 2 * d.j / 3, m, 0, 2 * Math.PI);
                        k.fill()
                    }
                    d.mobile && Pa(1.4);
                    var $b = d.i / 2,
                        wb = O;
                    d.mobile && (wb += (d.na ? (ea / 3 + O) / 1.4 * a.get() : 0) + (d.qa ? (ea / 2 + O) / 1.4 * b.get() : 0));
                    var Nd = Date.now();
                    for (let m = ic.length - 1; 0 <= m; m--) {
                        let u = ic[m],
                            q = Nd - u.time,
                            B = u.duration - q;
                        if (0 >= B) {
                            ic.splice(m, 1);
                            continue
                        }
                        let R = Math.max(0, Math.min(1, q / 300, B / 300));
                        null == u.Kc && (u.Kc = new V, u.uc = Bd(u.text,
                            Math.max(7.5, 15 + w.o.Ua)));
                        k.globalAlpha = .5 * R;
                        Fd($b - u.uc / 2, $b + u.uc / 2, wb + 9, 18, E(9));
                        k.globalAlpha = R;
                        u.Kc.m(u.text, $b, wb + 9, 15, E(8), "center", !0);
                        wb += 22 * (3 - 2 * R) * R * R
                    }
                    k.globalAlpha = 1;
                    d.mobile && Pa(1 / 1.4);
                    if (!d.mobile) {
                        d.na = 0 < pc && oc.some(T => T.amount < T.T);
                        a.set(d.na || d.N || d.Ic ? 1 : 0);
                        d.G.stat.L();
                        let m = ea - 35,
                            u = 0,
                            q = O + (a.get() - 1) * (O + ea) * 2,
                            B = d.j - O - 15,
                            R = 10,
                            G = mc();
                        for (let T of oc) {
                            R--;
                            let N = T.da;
                            if (!N) continue;
                            let U = T.amount,
                                Z = E(T.color),
                                ja = T.T,
                                qa = G[R],
                                Y = ea * z(N);
                            Fd(q + 7.5, q - 7.5 + Y, B + 7.5, 12 + w.O.va, E(9));
                            Fd(q + 7.5, q + 7.5 +
                                m * z(N), B + 7.5, 12, E(16));
                            Fd(q + 7.5, q + 7.5 + m * z(U), B + 7.5, 11.5, Z);
                            k.strokeStyle = E(9);
                            k.lineWidth = 1;
                            for (let ta = 1; ta <= U; ta++) {
                                let Ga = q + m * z(ta);
                                Ed(Ga, B + 1.5, Ga, B - 3 + 15)
                            }
                            let ka = 0 < pc && U < ja && U < N,
                                ua = U < ja ? ka ? E(8) : E(16) : Z;
                            x[R].m(qa, q + Y / 2, B + 7.5, 11, ua, "center", !0);
                            A[R].m(`[${(R+1)%10}]`, q + Y - 3.75 - 2, B + 7.5, 11, ua, "right", !0);
                            U && y[R].m(U < ja ? `+${U}` : "MAX", q + Y + 4, B + 7.5, 11, Z, "left", !0);
                            ka && d.G.stat.Ea(R, q * S, B * S, Y * S, 15 * S);
                            B -= 19;
                            u = Y
                        }
                        d.G.oc.Ea(0, 0, B * S, (.5 * ea + a.get() * u) * S, (d.j - B) * S);
                        1 < pc && g.m(`x${pc}`, q + u - 2, B + 15 - 4, 20, E(8), "right")
                    }
                    if (!d.N) {
                        let m =
                            1.65 * ea,
                            u = 25,
                            q = (d.i - m) / 2,
                            B = d.j - O - u;
                        k.lineWidth = 1;
                        Fd(q, q + m, B + u / 2, u - 3 + w.O.va, E(9));
                        Fd(q, q + m, B + u / 2, u - 3, E(16));
                        Fd(q, q + m * yc.s.progress.get(), B + u / 2, u - 3.5, E(13));
                        v.m(`Level ${Math.floor(yc.s.level.get())+1} ${-1!==nc?Nb[nc].name:"Unknown"}`, q + m / 2, B + u / 2, u - 3, E(8), "center", !0);
                        u = 14;
                        B -= u + 4;
                        Fd(q + .1 * m, q + .9 * m, B + u / 2, u - 3 + w.O.va, E(9));
                        Fd(q + .1 * m, q + .9 * m, B + u / 2, u - 3, E(16));
                        Fd(q + .1 * m, q + m * (.1 + .8 * ($a.max ? Math.min(1, yc.s.s.get() / $a.max) : 1)), B + u / 2, u - 3.5, E(11));
                        l.m(`Score: ${Ea(yc.s.s.get())}`, q + m / 2, B + u / 2, u - 1, E(8), "center",
                            !0);
                        k.lineWidth = 4;
                        p.m(J.name, q + m / 2, B - 10 - 4, 32, E(8), "center")
                    }
                    d.mobile && Pa(.8);
                    var wa = I.g.fa - I.g.aa,
                        ab = I.g.ma - I.g.ba,
                        la = ea * wa * 2 / (wa + ab),
                        ac = ea * ab * 2 / (wa + ab),
                        Ha = d.i - O,
                        Ra = d.j - O,
                        Zc = (m, u, q, B, R) => {
                            k.globalAlpha = .5;
                            k.fillStyle = Ib(E(16), E(6));
                            I.V ? Dd(m + q / 2, u + B / 2, q / 2) : Cd(m, u, q, B);
                            if (!I.V) {
                                let G = I.ca[0].length,
                                    T = I.ca.length,
                                    N = q / G,
                                    U = B / T;
                                for (let Z = 0; Z < T; Z++)
                                    for (let ja = 0; ja < G; ja++) k.globalAlpha = .5, k.fillStyle = Kb(I.ca[Z][ja]), Cd(m + ja * N, u + Z * U, N, U)
                            }
                            k.globalAlpha = 1;
                            k.save();
                            k.beginPath();
                            I.V ? k.arc(m + q / 2, u + B / 2, q / 2, 0, 2 *
                                Math.PI) : k.rect(m, u, q, B);
                            k.clip();
                            for (let G of fc.get()) k.fillStyle = Ib(E(G.color), E(9), .3), k.globalAlpha = G.alpha, 2 === G.type ? Cd(m + (G.x - G.size / wa) * q - .4, u + (G.y * ab / wa - G.size / wa) * q - 1, 2 * G.size / wa * q + .2, 2 * G.size / wa * q + .2) : 1 === G.type ? Dd(m + G.x * q, u + G.y * ab / wa * q, G.size / wa * q + .2) : G.id !== rc && Dd(m + G.x * q, u + G.y * ab / wa * q, R);
                            k.fillStyle = E(9);
                            k.globalAlpha = 1;
                            Dd(m + (J.ea.get(D) - I.g.aa) / wa * q, u + (J.ka.get(D) - I.g.ba) / wa * q, R);
                            k.restore();
                            k.strokeStyle = E(9);
                            k.lineWidth = 3;
                            I.V ? Dd(m + q / 2, u + B / 2, q / 2, !0) : Cd(m, u, q, B, !0)
                        };
                    if (d.mobile) {
                        Ha -=
                            la;
                        let m = (d.na ? (ea / 3 + O + O) * a.get() : 0) + (d.qa ? (ea / 2 + O + O) * b.get() : 0);
                        Zc(O, m + O, la, ac, 4);
                        d.G.Mb.L();
                        let u = (B, R, G, T, N, U) => {
                                d.G.Mb.Ea(B, R * S, G * S, T * S, N * S);
                                k.globalAlpha = .5;
                                k.fillStyle = E(16);
                                Cd(R, G, T, N);
                                k.globalAlpha = .1;
                                k.fillStyle = E(9);
                                Cd(R, G + .6 * N, T, .4 * N);
                                k.globalAlpha = 1;
                                C.get(B).m(U, R + T / 2, G + .725 * N, .6 * N, E(8), "center");
                                k.strokeStyle = E(9);
                                k.lineWidth = 3;
                                Cd(R, G, T, N, !0)
                            },
                            q = (ea - 2 * O) / 3;
                        3 === d.ga && u(0, O, m + 2 * O + la, 2 * q, 2 * q, "\u2756");
                        u(1, 2 * O + la, m + O, q, q, d.Na ? "-" : "+");
                        d.Na && (u(2, 3 * O + la + q, m + O, 6 * q, q, `\u2756 ${["Never","Always",
"Automatic","Manual","Double Tap"][d.ga||0]}`), u(3, 2 * O + la, m + 2 * O + q, 3.5 * q, q, "Autofire"), u(4, 3 * O + la + 3.5 * q, m + 2 * O + q, 3.5 * q, q, "Reverse"), u(5, 2 * O + la, m + 3 * O + 2 * q, 3.5 * q, q, "Autospin"), u(6, 3 * O + la + 3.5 * q, m + 3 * O + 2 * q, 3.5 * q, q, "Override"), 2 === d.Na && (u(7, 2 * O + la, m + 4 * O + 3 * q, 3.5 * q, q, "Action"), u(8, 3 * O + la + 3.5 * q, m + 4 * O + 3 * q, 3.5 * q, q, "???")))
                    } else Ha -= la, Ra -= ac, Zc(Ha, Ra, la, ac, 2);
                    var bc = Ra - 10,
                        $c = od(D);
                    e.h($c);
                    h.h(xc);
                    r.h(F.Dc);
                    n.h(kc());
                    d.cb && (k.fillStyle = E(9), Cd(Ha, Ra - 40, la, 30), e.m(Ha, Ra - 40, la, 30), h.m(Ha, Ra - 40, la, 30), r.m(Ha, Ra - 40,
                        la, 30), n.m(Ha, Ra - 40, la, 30), bc -= 40);
                    var xa = [];
                    if (!w.o.Oa) {
                        xa.push({
                            line: `${lc().toFixed(1)} ms  ${d.D.code} #${d.D.id}`
                        });
                        xa.push({
                            line: `${F.Ga} FPS / ${sc.toFixed(1)} mspt`,
                            jd: 15 > F.Ga || 25 < sc
                        });
                        if (d.cb) {
                            xa.push({
                                line: `Prediction: ${$c.toFixed(2)} (${["off","new","old","smoothCamera"][w.Da.bb]})`
                            });
                            var ad = 0,
                                bd = 0,
                                cd = 0,
                                dd = 0;
                            let u = (B, R) => {
                                R ? bd += B.length : ad += B.length;
                                for (let G of B)(G.name || G.s) && dd++, cd += G.C.Va(), u(G.B, !0)
                            };
                            u(Vb, !1);
                            xa.push({
                                line: `Rendering: e ${ad} t ${bd} g ${cd} n ${dd}`
                            });
                            xa.push({
                                line: `Network: ${F.v.Fb}/${F.v.$b}/${F.v.Hb} @ ${F.Pa}Hz`
                            });
                            xa.push({
                                line: `Bandwidth: tx ${F.oa} rx ${F.Fa}`
                            });
                            xa.push({
                                line: `Coordinates: (${J.ea.get(D).toFixed(2)}, ${J.ka.get(D).toFixed(2)})`
                            });
                            let q = h.R();
                            xa.push({
                                line: `Speed: ${xc.toFixed(2)} gu/s${q&&.005<=q?` (${q.toFixed(2)} gu/s)`:""}`
                            })
                        }
                        let m = Lc();
                        1 < m && xa.push({
                            line: `${Ea(m)} players`
                        })
                    }
                    for (let m = 0; m < xa.length; m++) {
                        let {
                            line: u,
                            jd: q = !1
                        } = xa[m];
                        K.get(m).m(u, Ha + la, bc - 14 * m, 11, E(q ? 2 : 8), "right")
                    }
                    t.m("arras.io", Ha + la, bc - 14 * xa.length - 2, 16, E(8), "right");
                    d.mobile && Pa(1.25);
                    d.mobile && Pa(1.4);
                    if (!w.o.Oa && w.O.Lb) {
                        let m =
                            d.i - ea - O,
                            u = O + 14 + 14;
                        d.mobile && (u += (d.na ? ea / 3 / 1.4 * a.get() : 0) + (d.qa && 2 * O + qc.length * (.5 * ea + 14) > 1.4 * m ? ea / 2 / 1.4 * b.get() : 0));
                        0 < $a.data.length && P.m("Leaderboard", m + ea / 2, u - 10, 19, E(8), "center");
                        for (let q = 0; q < $a.data.length && (!d.mobile || 6 > q); q++) {
                            let B = $a.data[q];
                            Fd(m, m + ea, u + 7, 11 + w.O.va, E(9));
                            Fd(m, m + ea, u + 7, 11, E(16));
                            Fd(m, m + ea * Math.min(1, B.s / $a.max), u + 7, 10.5, B.Rc);
                            let R = B.name ? `${w.o.jb?Jd(B.name):B.name} - ` : "",
                                G = B.label.includes("#") ? B.label.replace("##", Math.round(B.s).toString()).replace("#s", 1 === Math.round(B.s) ?
                                    "" : "s") : `${B.label}: ${Da(B.s)}`;
                            X[q].m(R + G, m + ea / 2, u + 7, 10, E(8), "center", !0);
                            let T = 14 / B.position.axis;
                            Hd(m - 21 - T * B.position.$.x * Math.SQRT1_2, u + 7 + T * B.position.$.x * Math.SQRT1_2, B.image, .5, 1, 2 * T / B.image.size, -Math.PI / 4, !0);
                            u += 18
                        }
                    }
                    d.mobile && Pa(1 / 1.4);
                    d.qa = 0 < qc.length && !(d.mobile && d.N) && !d.za;
                    b.set(+d.qa);
                    var ed = b.get();
                    d.G.upgrade.L();
                    if (d.qa) {
                        let m = .5 * ea,
                            u = .5 * ea,
                            q = 2 * ed * O - O,
                            B = O,
                            R = q,
                            G = 0,
                            T = B;
                        hc += .01;
                        let N = 0;
                        for (let qa of qc) {
                            B > T && (T = B);
                            G = q;
                            d.G.upgrade.Ea(N, q * S, B * S, m * S, u * S);
                            k.globalAlpha = .5;
                            k.fillStyle = Lb(N);
                            Cd(q, B, m, u);
                            k.globalAlpha = .1;
                            k.fillStyle = E(9);
                            Cd(q, B + .6 * u, m, .4 * u);
                            k.globalAlpha = 1;
                            let Y = Pb(qa, tc),
                                ka = Nb[qa].position,
                                ua = .6 * m / ka.axis;
                            Hd(q + .5 * m - ua * ka.$.x * Math.cos(hc), B + .5 * u - ua * ka.$.x * Math.sin(hc), Y, 1, 1, ua / Y.size, hc, !0);
                            let ta = (d.K[`KEY_CHOOSE_${N+1}`] || "").toLowerCase().trim();
                            !d.mobile && ta ? (ba.get(N).m(Y.name, q + .9 * m / 2, B + u - 6, u / 8 - 2, E(8), "center"), H.get(N).m(`[${ta}]`, q + m - 4, B + u - 6, u / 8 - 2, E(8), "right")) : ba.get(N).m(Y.name, q + m / 2, B + u - 6, u / 8 - 2, E(8), "center");
                            k.strokeStyle = E(9);
                            k.globalAlpha = 1;
                            k.lineWidth =
                                3;
                            Cd(q, B, m, u, !0);
                            N++;
                            0 === N % (d.mobile ? 10 : 3) ? (q = R, B += u + 14) : q += ed * (m + 14)
                        }
                        let U = Bd("Don't Upgrade", Math.max(6, 12 + w.o.Ua)) + 4,
                            Z = (G + m + 14 + R - 15) / 2,
                            ja = T + u + 14;
                        Fd(Z - U / 2, Z + U / 2, ja + 7, 14 + w.O.va, E(9));
                        Fd(Z - U / 2, Z + U / 2, ja + 7, 14, E(18));
                        L.m("Don't Upgrade", Z, ja + 7, 12, E(8), "center", !0);
                        d.G.za.Ea(0, (Z - U / 2) * S, ja * S, U * S, 14 * S)
                    } else d.G.upgrade.L(), d.G.za.L();
                    if (d.mobile) {
                        d.na = 0 < pc && oc.some(G => G.amount < G.T) && !d.qa;
                        a.set(d.na || d.N ? 1 : 0);
                        let m = a.get();
                        d.G.stat.L();
                        let u = ea / 2,
                            q = ea / 3,
                            B = 2 * m * O - O,
                            R = mc();
                        if (d.na) {
                            for (let G = 0; G < oc.length; G++) {
                                let T =
                                    oc[G],
                                    N = T.da;
                                if (0 >= N) continue;
                                let U = T.amount,
                                    Z = E(T.color),
                                    ja = T.T,
                                    qa = R[9 - G].split(/\s+/),
                                    Y = Math.floor(qa.length / 2),
                                    [ka, ua] = 1 === qa.length ? [qa, null] : [qa.slice(0, Y).join(" "), qa.slice(Y).join(" ")];
                                k.globalAlpha = .5;
                                k.fillStyle = Z;
                                Cd(B, O, u, 2 * q / 3);
                                k.globalAlpha = .1;
                                k.fillStyle = E(9);
                                Cd(B, O + 2 * q / 3 * 2 / 3, u, 2 * q / 3 / 3);
                                k.globalAlpha = 1;
                                k.fillStyle = E(8);
                                Cd(B, O + 2 * q / 3, u, q / 3);
                                k.fillStyle = Z;
                                Cd(B, O + 2 * q / 3, u * U / N, q / 3);
                                k.strokeStyle = E(9);
                                k.lineWidth = 1;
                                for (let ta = 1; ta < ja; ta++) {
                                    let Ga = B + ta / N * u;
                                    Ed(Ga, O + 2 * q / 3, Ga, O + q)
                                }
                                U === ja || !pc ||
                                    N !== ja && U === N || d.G.stat.Ea(9 - G, B * S, O * S, u * S, q * S);
                                ua ? (x[G].m(ua, B + u / 2, O + .55 * q, q / 5, E(8), "center"), x[G].m(ka, B + u / 2, O + .3 * q, q / 5, E(8), "center")) : x[G].m(ka, B + u / 2, O + .425 * q, q / 5, E(8), "center");
                                0 < U && y[G].m(U < N ? `+${U}` : "MAX", B + u / 2, O + 1.3 * q, q / 4, Z, "center");
                                k.strokeStyle = E(9);
                                k.globalAlpha = 1;
                                k.lineWidth = 3;
                                Ed(B, O + 2 * q / 3, B + u, O + 2 * q / 3);
                                Cd(B, O, u, q, !0);
                                B += m * (u + 14)
                            }
                            1 < pc && g.m(`x${pc}`, B, O + 20, 20, E(8), "left")
                        }
                    }
                    Pa(1 / S, !0)
                }
            }
        })(),
        Md = (() => {
            var a = new V,
                b = new V,
                c = new V,
                f = new V,
                e = new V,
                h = new V,
                n = new V,
                r = new V,
                z = new V,
                x = new V,
                A = new V,
                y = new V;
            let g = () => {
                    let p = d.A.sc;
                    var v = p[0] + .5 * p[1] + 3 * p[2];
                    v = 0 === v ? "\ud83c\udf3c" : 4 > v ? "\ud83c\udfaf" : 8 > v ? "\ud83d\udca5" : 15 > v ? "\ud83d\udca2" : 25 > v ? "\ud83d\udd25" : 50 > v ? "\ud83d\udca3" : 75 > v ? "\ud83d\udc7a" : 100 > v ? "\ud83c\udf36\ufe0f" : "\ud83d\udcaf";
                    let t = [],
                        K = 0;
                    for (let [C, P] of [
                            [" kill", ""],
                            [" assist", ""],
                            [" visitor", " defeated"],
                            [" polygon", " destroyed"]
                        ]) {
                        let X = p[K++];
                        0 < X && t.push(X + C + (1 === X ? "" : "s") + P)
                    }
                    return `${v} ${Aa(t)||"A true pacifist"}`
                },
                l = () => -1 === d.A.ra.type ? "\ud83d\udd2a Death information unavailable" :
                1 === d.A.ra.type ? "\ud83e\udd37 Well, that was kinda dumb" : 2 === d.A.ra.type ? "\ud83d\udca3 Self-destruct completed" : 3 === d.A.ra.type ? `\ud83c\udf9b\ufe0f Surrendered control of the ${Nb[nc].name}` : 4 === d.A.ra.type ? "\ud83d\udce6 Your score has been saved" : 0 === d.A.ra.sources.length ? "\u26c8\ufe0f Vanished into thin air" : `\ud83d\udd2a Succumbed to ${Aa(d.A.ra.sources.map(p=>{p.name?p=`${p.name}'s ${p.Bb}`:(p=p.Bb,p=`${/^[aeiou]/i.test(p)?"an":"a"} ${p}`);return p}))}`;
            return () => {
                let p = Math.max(d.i, 16 * d.j / 9) /
                    d.rb * w.O.scale,
                    v = (Q, D = !1) => {
                        d.i /= Q;
                        d.j /= Q;
                        k.scale(Q, Q);
                        D || (p *= Q)
                    };
                v(p, !0);
                let t = d.jc.get();
                var K = (Q, D) => {
                    Q = (t - Q) / (D - Q);
                    return 0 >= Q ? 0 : 1 <= Q ? 1 : Q * Q * (3 - 2 * Q)
                };
                xd(E(9), .1 + .15 * K(0, .5));
                let C = d.i / 2,
                    P = Math.min(d.j / 2 - 60, d.j - 500) - 800 * (1 - K(0, 1)),
                    X = Pb(nc, tc);
                var ba = Nb[nc].position;
                let H = 140 / ba.axis,
                    L = C - H * ba.$.x * Math.SQRT1_2;
                ba = P + 25 + H * ba.$.x * Math.SQRT1_2;
                Hd(L - 190 - 70, ba - 10, X, 1.5, 1, .5 * H / X.size, -Math.PI / 4, !0);
                a.m(d.A.Gb ? (new Date(1E3 * d.A.Gb)).toISOString().replace(".000", "") : "", C, P - 160, 10, E(8), "center");
                b.m(`Level ${Math.floor(yc.s.level.get())+
1}`, L - 190 - 70, ba + 60, 13, E(8), "center");
                c.m(Nb[nc].name, L - 190 - 70, ba + 85, 19, E(8), "center");
                f.m(J.name ? `${J.name}'s Score:` : "Your Score:", C - 170, P - 50, 25, E(8));
                e.m(Ea(d.A.s * (1 - (1 - K(0, 2)) ** 5)), C - 170, P + 10, 50, E(8));
                k.globalAlpha = K(1, 1.25);
                h.m(`\u231a Survived for ${Ca()}`, C - 170, P + 50, 15, E(8));
                k.globalAlpha = K(1.25, 1.5);
                n.m(g(), C - 170, P + 75, 15, E(8));
                k.globalAlpha = K(1.5, 1.75);
                r.m(-1 === d.A.ta.type ? "\u2753 Kill information unavailable" : 1 === d.A.ta.type ? `\ud83c\udf6a ${Math.round(100*d.A.ta.ratio)}% of your score came from polygons` :
                    2 === d.A.ta.type ? `\ud83d\udd2a ${Math.round(100*d.A.ta.ratio)}% of your score came from ${d.A.ta.name?`the player ${d.A.ta.name}`:"an unnamed player"}` : 1E5 < d.A.s ? "\ud83d\udc80 lol you died" : 2E4 < d.A.s ? "\u2753 Kill players and polygons to get more score" : `\u2753 Press ${d.K.KEY_LEVEL_UP} to get to level 45`, C - 170, P + 100, 15, E(8));
                k.globalAlpha = K(1.75, 2);
                z.m(l(), C - 170, P + 125, 15, E(8));
                k.globalAlpha = K(2, 2.25);
                x.m(`${.75<=d.A.fb?"\ud83d\udccb":"\u2620\ufe0f"} ${null==d.A.fb?"Server activity information unavailable":
`The server was alive for ${Math.round(100*d.A.fb)}% of the run`}`, C - 170, P + 150, 15, E(8));
                k.globalAlpha = K(3, 3.25);
                K = Math.ceil((d.tb - Date.now()) / 1E3);
                A.m(0 < K ? `(you may respawn in ${K} second${1===K?"":"s"})` : "joysticks" === bb.Zb ? "(tap to respawn)" : "(press enter to respawn)", C, P + 225, 15, E(8), "center");
                k.globalAlpha = 1;
                d.ia ? (d.G.ia.Ea(0, (C - 60) * p, (P + 170) * p, 120 * p, 32 * p), k.globalAlpha = .5, k.fillStyle = E(16), Cd(C - 60, P + 170, 120, 32), k.globalAlpha = .1, k.fillStyle = E(9), Cd(C - 60, P + 189.2, 120, 12.8), k.globalAlpha = 1, y.m(d.ia.ic ?
                    "Copied!" : "Copy Code", C, P + 194, 16, E(8), "center"), k.strokeStyle = E(9), k.lineWidth = 3, Cd(C - 60, P + 170, 120, 32, !0)) : d.G.ia.L();
                v(1 / p, !0)
            }
        })();
    window.addEventListener("beforeunload", a => {
        d.Aa && !d.N && (a.preventDefault(), a.returnValue = "")
    });
    const Od = (() => {
            var a = new V,
                b = new V,
                c = [
                    ["Tip: You can view and edit your keybinds in the options menu.", "Tip: You can play on mobile by just going to arras.io on your phone!"],
                    ["Tip: You can have the shield and health bar be separated by going to the options menu.", "Tip: If arras.io is having a low frame rate, you can try enabling low graphics in the options menu.", "Tip: You can make traps rounded with the classic trap setting in the options menu.", "Tip: You can create your own private server with the template in the link on the options menu.",
                        "Tip: You can create your own theme with the custom theme maker in the link on the options menu."
                    ],
                    ["Teaming in FFA or FFA Maze is frowned upon, but when taken to the extremes, you can be punished.", "Witch hunting is when you continuously target someone and follow them. This is frowned upon, but when taken to the extremes, you can be punished.", "Multiboxing is when you use a script to control multiple tanks at the same time. This is considered CHEATING and will result in a ban."]
                ];
            c = c[Math.floor(Math.random() *
                c.length)];
            let f = c[Math.floor(Math.random() * c.length)];
            return () => {
                let e = Math.max(d.i, 16 * d.j / 9) / d.rb * w.O.scale,
                    h = (n, r = !1) => {
                        d.i /= n;
                        d.j /= n;
                        k.scale(n, n);
                        r || (e *= n)
                    };
                h(e, !0);
                xd(E(18), .5);
                a.m("Connecting...", d.i / 2, d.j / 2, 32, E(8), "center");
                b.m(d.message, d.i / 2, d.j / 2 + 30, 16, E(1), "center");
                b.m(f, d.i / 2, d.j / 2 + 75, 16, E(8), "center");
                h(1 / e, !0)
            }
        })(),
        Pd = (() => {
            var a = new V,
                b = new V;
            return () => {
                let c = Math.max(d.i, 16 * d.j / 9) / d.rb * w.O.scale,
                    f = (e, h = !1) => {
                        d.i /= e;
                        d.j /= e;
                        k.scale(e, e);
                        h || (c *= e)
                    };
                f(c, !0);
                xd(Ib(E(12), E(19), .3), .25);
                a.m("\ud83d\udc80 Disconnected \ud83d\udc80", d.i / 2, d.j / 2, 32, E(8), "center");
                b.m(d.message, d.i / 2, d.j / 2 + 30, 16, E(2), "center");
                f(1 / c, !0)
            }
        })();
    let wd = a => {
        rb(wd);
        k.lineCap = "round";
        k.lineJoin = w.o.$a ? "miter" : "round";
        d.ya && !d.la && 1E3 < a - F.tc && (F.tc = a, F.Ga = F.current.Ga, F.Pa = F.current.Pa, F.oa = F.current.oa, F.Fa = F.current.Fa, F.current.Ga = 0, F.current.Pa = 0, F.current.oa = 0, F.current.Fa = 0);
        d.ya && Nb ? Ld(a) : d.la || Od();
        d.N ? Md() : d.G.ia.L();
        d.la && Pd();
        if (d.N || d.la) a = E(18), d.N && (a = Ib(a, E(9), .25)), d.la && (a = Ib(a, Ib(E(12), E(19), .3), .25)), Ma(a)
    };
    window.$createProfile = (() => {
        var a = new V;
        return (b, c = -1, f = 200, e = -Math.PI / 4) => {
            let h = k.canvas.width,
                n = k.canvas.height,
                r = k.canvas.width = f;
            f = k.canvas.height = f; - 1 === c ? k.clearRect(0, 0, r, f) : (k.fillStyle = E(18), k.fillRect(0, 0, r, f), k.globalAlpha = .5, k.fillStyle = Lb(c), Cd(0, 0, r, f), k.globalAlpha = .1, k.fillStyle = E(9), Cd(0, .6 * f, r, .4 * f), k.globalAlpha = 1);
            let z = Pb(b, tc);
            b = Nb[b].position;
            let x = .6 * r / b.axis;
            Hd(.5 * r - x * b.$.x * Math.cos(e), .5 * f - x * b.$.x * Math.sin(e), z, 1, 1, x / z.size, e, !0); - 1 !== c && (a.m(z.name, .9 * r / 2, f - 6, f / 8 - 2, E(8),
                "center"), k.strokeStyle = E(9), k.globalAlpha = 1, k.lineWidth = 3, Cd(0, 0, r, f, !0));
            c = k.canvas.toDataURL();
            k.canvas.width = h;
            k.canvas.height = n;
            return c
        }
    })();
    let Qd = new Date,
        Rd = d.mobile ? 0 : Math.max(0, 1 - Math.abs(Qd.getTime() - new Date(Qd.getFullYear() - (6 > Qd.getMonth() ? 1 : 0), 11, 25)) / 20736E5);
    if (Rd) {
        let a = document.createElement("canvas");
        a.style.position = "absolute";
        a.style.top = "0";
        document.body.insertBefore(a, document.body.firstChild);
        let b = a.getContext("2d"),
            c = [],
            f = () => {
                a.width !== window.innerWidth && (a.width = window.innerWidth);
                a.height !== window.innerHeight && (a.height = window.innerHeight);
                b.clearRect(0, 0, a.width, a.height);
                b.fillStyle = "#ffffff";
                for (let e of c) {
                    e.x += 5 / e.r + Math.random();
                    e.y += 12.5 / e.r + Math.random();
                    let h = 2 * Math.min(.4, .9 - e.y / a.height);
                    0 < h ? (b.globalAlpha = h, b.beginPath(), b.arc(e.x,
                        e.y, e.r, 0, 2 * Math.PI), b.fill()) : e.Ja = !0
                }
                .001 * a.width * Rd > Math.random() && c.push({
                    x: a.width * (1.5 * Math.random() - .5),
                    y: -50 - 100 * Math.random(),
                    r: 2 + Math.random() * Math.random() * 4
                });
                d.ya ? a.remove() : rb(f)
            };
        setInterval(() => {
            c = c.filter(e => !e.Ja)
        }, 2E3);
        f()
    }
    let Sd = "en-US" === navigator.language && -7 <= d.pa && -4 >= d.pa,
        Td = 6 === Qd.getMonth() && 4 === Qd.getDate(),
        Ud = 11 === Qd.getMonth() && 31 === Qd.getDate() || 0 === Qd.getMonth() && 3 >= Qd.getDate();
    if (!d.mobile && (Td && Sd || Ud)) {
        let a = document.createElement("canvas");
        a.style.position = "absolute";
        a.style.top = "0";
        document.body.insertBefore(a, document.body.firstChild);
        let b = a.getContext("2d"),
            c = () => {
                let h = "164,14,14 230,80,0 230,119,0 47,127,51 23,78,166 123,31,163".split(" ");
                return h[Math.floor(Math.random() * h.length)]
            },
            f = [],
            e = () => {
                if (a.width !== window.innerWidth || a.height !== window.innerHeight) a.width = window.innerWidth, a.height = window.innerHeight, f = [], b.clearRect(0, 0, a.width, a.height), b.fillStyle =
                    "rgba(255,255,255,0.01)", b.fillRect(0, 0, a.width, a.height), b.lineWidth = 2.5, b.lineCap = "round";
                b.globalCompositeOperation = "destination-out";
                b.fillStyle = "rgba(0,0,0,0.15)";
                b.fillRect(0, 0, a.width, a.height);
                b.globalCompositeOperation = "lighter";
                for (var h of f) {
                    var n = h.x,
                        r = h.y;
                    h.J += .2;
                    h.x += h.M;
                    h.y += h.J;
                    h.J *= .99;
                    h.M *= .99;
                    h.time--;
                    var z = 0 < h.time ? h.Rb ? 1 : 10 <= h.time ? 1 : h.time / 10 : 0;
                    if (0 < z) b.strokeStyle = `rgba(${h.color},${z})`, b.beginPath(), b.moveTo(n, r), b.lineTo(h.x, h.y), b.stroke();
                    else {
                        if (h.Rb && !h.Ja) {
                            n = Math.floor(5 *
                                Math.random()) + 30;
                            r = .5 * Math.random() + 3;
                            z = 25 + 5 * Math.random();
                            for (var x = 0; 2 > x; x++) {
                                let A = c();
                                for (let y = 0; y < n; y++) {
                                    let g = (y + Math.random()) / n * Math.PI * 2,
                                        l = r + .5 * Math.random();
                                    f.push({
                                        color: A,
                                        x: h.x,
                                        y: h.y,
                                        M: Math.cos(g) * l,
                                        J: -.8 + Math.sin(g) * l,
                                        time: z + 2 * Math.random(),
                                        Rb: !1,
                                        Ja: !1
                                    })
                                }
                            }
                        }
                        h.Ja = !0
                    }
                }
                3E-5 * a.width > Math.random() && (h = a.width * Math.random(), n = a.height - 10, r = 4 * Math.random() - 2, z = 5 * Math.random() - 15, x = 30 + 10 * Math.random(), f.push({
                    color: c(),
                    x: h,
                    y: n,
                    M: r,
                    J: z,
                    time: x,
                    Rb: !0,
                    Ja: !1
                }));
                d.ya ? a.remove() : rb(e)
            };
        setInterval(() => {
            f = f.filter(h => !h.Ja)
        }, 2E3);
        e()
    };
}();


// FOV
const canvasElement = document.getElementById("canvas");
window.fov = 1.0;
let zoomdisplay = document.createElement("div");
zoomdisplay.id = "zoomdisplay";
zoomdisplay.style["font"] = 'bold 15px Ubuntu';
zoomdisplay.style["text-align"] = 'center';
zoomdisplay.style["color"] = 'black';
zoomdisplay.style["position"] = 'absolute';
zoomdisplay.style["top"] = '0px';
zoomdisplay.style["right"] = '20px';
zoomdisplay.innerText = `ZOOM : ${window.fov}`;
document.body.append(zoomdisplay);
// 設定
let view = 0;
Object.defineProperty(window.Arras().player, "view", {
    "set": (_view) => {
        view = _view;
        return view;
    },
    "get": () => {
        return (view * fov);
    }
});

// マウスホイール操作時
window.addEventListener("wheel", (event) => {
    const element = document.elementFromPoint(event.clientX, event.clientY);
    if (!element.isEqualNode(canvasElement)) {
        return;
    }
    if (event.wheelDelta > 0) {
        //window.fov /= 1.1;
        window.fov = Number((window.fov - 0.05).toFixed(2));
    } else {
        //window.fov *= 1.1;
        window.fov = Number((window.fov + 0.05).toFixed(2));
    }
    window.fov = Math.max(fov, 0.0);
    window.fov = Math.min(fov, 30.0);
    zoomdisplay.innerText = `ZOOM : ${window.fov}`;
});


// Fast respawn
setInterval(function(){
    window.Arras().tb = 0;
}, 10);
